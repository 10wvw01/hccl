...
{
      if (opName == nullptr || opName[0] == '\0') return nullptr;
      auto it = opRegistries_.find(opName);
      return (it == opRegistries_.end()) ? nullptr : it->second.get();
}

bool PluginBrokerContext::CheckDirTrusted(const std::string& dir) const
{
    /*
     * 安全校验：
     *   1) 拒绝根目录自身是符号链接；
     *   2) 拒绝根目录world-writable（其他用户可写），防止被篡改注入恶意.so；
     *   3) realpath解析失败（目录不存在/无权限）视为不可信。
     */
    struct stat lst{};
    if (lstat(dir.c_str(), &lst) != 0) {
    ...
    if ((lst.st_mode & S_IWOTH) != 0) {
        PLUGIN_LOG_ERROR("HCCL_PLUGIN_ALG_DIR is world-writable, refuse to load: %s", dir.c_str());
        return false;
    }
...
bool PluginBrokerContext::LoadSelectorEntries(OpRegistry& reg)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (reg.FindByName(entries[i].algName) != nullptr) {
            PLUGIN_LOG_WARN("duplicate algName [%s] in %s, keep the first one",
                entries[i].algName, reg.selectorSoPath.c_str());
            continue;
        }
...
bool PluginBrokerContext::ScanOpDir(const std::string& opDirPath, const std::string& opDirName)
{
    ...
    PLUGIN_LOG_INFO("op [%s] registered %zu custom algorithm(s) from %s",
        opDirName.c_str(), reg->algs.size(), selectorSoPath.c_str());
    opRegistries_[opDirName] = std::move(reg);
    return true;
}
...
void PluginBrokerContext::AutoInit()
{
    ...
    std::string rootDir = rootDirEnv;
    if (!IsDirectory(rootDir)) {
        PLUGIN_LOG_ERROR("HCCL_PLUGIN_ALG_DIR does not exist or is not a directory: %s", rootDir.c_str());
        ready_ = false;
        return;
    }
    if (!CheckDirTrusted(rootDir)) {
        PLUGIN_LOG_ERROR("HCCL_PLUGIN_ALG_DIR failed trust check, refuse to load any plugin algorithm: %s",
            rootDir.c_str());
        ready_ = false;
        return;
    }
    ...
    while ((ent = readdir(dir)) != nullptr) {
        ...
        if (ScanOpDir(subPath, name)) {
            anyLoaded = anyLoaded || (opRegistries_.find(name) != opRegistries_.end());
        }
    }
    closedir(dir);

    ready_ = true; /* 只要扫描流程本身未出现致命错误即视为Ready，具体算子有没有算法不影响Ready状态 */
    PLUGIN_LOG_INFO("PluginBroker auto init done, rootDir=%s, opCount=%zu, anyAlgLoaded=%d",
        rootDir.c_str(), opRegistries_.size(), static_cast<int>(anyLoaded));
}
...
int PluginBrokerContext::ExecuteAlg(const char* algName, const char* opName, const HcclAlgoPluginParam* param, void* comm)
{
    if (!ready_ || algName == nullptr || param == nullptr) {
    ...
    /* 懒加载集合通信算法实现动态库，含loadFailed标记的重试拦截 */
    {
        std::lock_guard<std::mutex> lock(alg->loadMutex);
        if (alg->loadFailed) {
            PLUGIN_LOG_ERROR("ExecuteAlg: algName [%s] previously failed to load, refuse to retry",
                algName);
            return HCCL_E_INTERNAL;
        }
        if (alg->implHandle == nullptr) {
            alg->implHandle = dlopen(alg->entry.soPath, RTLD_NOW | RTLD_GLOBAL);
            if (alg->implHandle == nullptr) {
                PLUGIN_LOG_ERROR("ExecuteAlg: dlopen impl so failed: %s, dlerror=%s",
                    alg->entry.soPath, dlerror());
                alg->loadFailed = true;
                return HCCL_E_INTERNAL;
            }
            alg->fnPtr = dlsym(alg->implHandle, alg->entry.fnSymbol);
            if (alg->fnPtr == nullptr) {
                PLUGIN_LOG_ERROR("ExecuteAlg: dlsym [%s] failed in %s: %s",
                    alg->entry.fnSymbol, alg->entry.soPath, dlerror());
                alg->loadFailed = true;
                return HCCL_E_INTERNAL;
            }
        }
    ...
    if (std::strcmp(opName, "Send") == 0) {
        ...
        return reinterpret_cast<Fn>(alg->fnPtr)(param->sendBuf, param->count, param->dataType,
            param->remoteRank, static_cast<HcclComm>(comm), param->stream);
    }
    if (std::strcmp(opName, "Recv") == 0) {
        using Fn = HcclResult (*)(void*, uint64_t, HcclDataType, uint32_t, HcclComm, aclrtStream);
        return reinterpret_cast<Fn>(alg->fnPtr)(param->recvBuf, param->count, param->dataType,
            param->remoteRank, static_cast<HcclComm>(comm), param->stream);
    }
    if (std::strcmp(opName, "Broadcast") == 0) {
        using Fn = HcclResult (*)(void*, uint64_t, HcclDataType, uint32_t, HcclComm, aclrtStream);
        return reinterpret_cast<Fn>(alg->fnPtr)(param->sendBuf, param->count, param->dataType,
            param->root, static_cast<HcclComm>(comm), param->stream);
    }
    if (std::strcmp(opName, "AllReduce") == 0) {
        using Fn = HcclResult (*)(void*, void*, uint64_t, HcclDataType, HcclReduceOp, HcclComm, aclrtStream);
        return reinterpret_cast<Fn>(alg->fnPtr)(param->sendBuf, param->recvBuf, param->count,
            param->dataType, param->reduceOp, static_cast<HcclComm>(comm), param->stream);
    }
    if (std::strcmp(opName, "Reduce") == 0) {
        using Fn = HcclResult (*)(void*, void*, uint64_t, HcclDataType, HcclReduceOp, uint32_t, HcclComm, aclrtStream);
        return reinterpret_cast<Fn>(alg->fnPtr)(param->sendBuf, param->recvBuf, param->count,
            param->dataType, param->reduceOp, param->root, static_cast<HcclComm>(comm), param->stream);
    }
    if (std::strcmp(opName, "AllGather") == 0) {
        using Fn = HcclResult (*)(void*, void*, uint64_t, HcclDataType, HcclComm, aclrtStream);
        return reinterpret_cast<Fn>(alg->fnPtr)(param->sendBuf, param->recvBuf, param->count,
            param->dataType, static_cast<HcclComm>(comm), param->stream);
    }
    ...
    if (std::strcmp(opName, "AllToAll") == 0) { // 非V，等长场景
        using Fn = HcclResult (*)(const void*, uint64_t, HcclDataType, const void*, uint64_t, HcclDataType,
            HcclComm, aclrtStream);
        return reinterpret_cast<Fn>(alg->fnPtr)(param->sendBuf, param->count, param->dataType,
            param->recvBuf, param->count, param->dataType, static_cast<HcclComm>(comm), param->stream);
    }
    if (std::strcmp(opName, "Scatter") == 0) {
        using Fn = HcclResult (*)(void*, void*, uint64_t, HcclDataType, uint32_t, HcclComm, aclrtStream);
        return reinterpret_cast<Fn>(alg->fnPtr)(param->sendBuf, param->recvBuf, param->count,
            param->dataType, param->root, static_cast<HcclComm>(comm), param->stream);
    }

    PLUGIN_LOG_ERROR("ExecuteAlg: opName=%s dispatch not implemented, please extend per "
        "sdk/hccl_algo_plugin_sdk.h standard signatures", opName);
...
int PluginBrokerContext::QueryAlgs(const char* opName, char* buf, size_t bufLen)
{
    ...
    for (const auto& alg : reg->algs) {
        ...
        std::memcpy(buf + offset, alg->entry.algName, nameLen);
        offset += nameLen;
        buf[offset++] = '\0';
    }
    return HCCL_SUCCESS;
