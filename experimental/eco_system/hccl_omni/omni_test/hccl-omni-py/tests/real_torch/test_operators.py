...
Run:
    ...
    torchrun --nproc_per_node=4 tests/real_torch/test_operators.py -k test_allreduce

    # All tests, distributed
    torchrun --nproc_per_node=4 tests/real_torch/test_operators.py
"""
...
def test_allgather():
    ...
    cfg = hccl_omni.HcclOmniConfig(
        ins_file_path='xml_example/4pfullmesh.xml',
    ...
    inp = (torch.arange(N, dtype=torch.float32, device=device) + rank * N).contiguous()
    out_list = [torch.empty(N, dtype=torch.float32, device=device) for _ in range(w)]
    ...
    # torch native
    native_list = [torch.empty(N, dtype=torch.float32, device=device) for _ in range(w)]
...
def test_alltoall():
    ...
    send_sizes = [(i + 1) * 2 for i in range(w)]   # [2, 4, 6, 8] for w=4
    recv_sizes = [send_sizes[rank]] * w              # each source sends the same chunk
    ...
    inp = (torch.arange(send_total, dtype=torch.float32, device=device) + rank * 100).contiguous()
