...
#include "all_gather_executor_base.h"
...
AllGatherExecutorBase::AllGatherExecutorBase() : ExecutorBase()
{
}

// 执行入口
HcclResult AllGatherExecutorBase::Orchestrate(const OpParam &param, AlgResourceCtx* resCtx)
{
    ...
    HCCL_DEBUG("[AllGatherExecutorBase][Orchestrate] slaveThreadNum[%u]", algResource_->slaveThreadNum);
    for (u32 i = 0; i < algResource_->slaveThreadNum + 1; i++) {
        HCCL_DEBUG("[AllGatherExecutorBase][Orchestrate] threadHandle[%u]=[%llu]", i, threadHandlePtr[i]);
        if (i == 0) {
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[AllGatherExecutorBase][Orchestrate]errNo[0x%016llx]ReduceScatter executor kernel run failed",
            HCCL_ERROR_CODE(ret)), ret);
    HCCL_INFO("[AllGatherExecutorBase][Orchestrate]tag[%s] ReduceScatter executor orchestrate success, take time [%lld]us.",
        param.tag, DURATION_US(TIME_NOW() - startut));
    return HCCL_SUCCESS;
}

bool AllGatherExecutorBase::IsHugeData(u64 curSize) const
{
...
HcclResult AllGatherExecutorBase::RunLoop(const OpParam &param)
{
    ...
    CHK_PRT_RET((cclInputMem.size == 0), HCCL_ERROR("[AllGatherExecutorBase][RunLoop]cclBuffer size is zero"), HCCL_E_PARA);
    ...
    HCCL_DEBUG("[AllGatherExecutorBase][RunLoop] curUserInputPtr[%p], curUserOutputPtr[%p], "
...
HcclResult AllGatherExecutorBase::CalcResRequest(HcclComm comm, const OpParam& param, TopoInfo* topoInfo,
    AlgHierarchyInfo& algHierarchyInfo, AlgResourceRequest& resourceRequest, AlgType& algType)
{
    return HCCL_SUCCESS;
}

HcclResult AllGatherExecutorBase::PrepareDataSlice(u64 dataCount, u32 unitSize, u32 sliceNum,
    std::vector<Slice> &dataSlice)
{
    CHK_PRT_RET((sliceNum == 0), HCCL_ERROR("[AllGatherExecutorBase][PrepareDataSlice]sliceNum is zero."), HCCL_E_PARA);
