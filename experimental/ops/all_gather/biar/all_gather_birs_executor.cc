...
#include <numeric>
#include "all_gather_birs_executor.h"
#include "topo_experimental.h"

namespace ops_hccl_experimental {

AllGatherBIRSExecutor::AllGatherBIRSExecutor() : AllGatherExecutorBase()
{
...
HcclResult AllGatherBIRSExecutor::CalcResRequest(HcclComm comm, const OpParam& param, TopoInfo* topoInfo,
    AlgHierarchyInfo& algHierarchyInfo, AlgResourceRequest& resourceRequest, AlgType& algType)
{
    ...
    HCCL_INFO("[AllGatherBIRSExecutor][CalcResRequest]slaveThreadNum[%u] notifyNumPerThread[%u] notifyNumOnMainThread[%u]"
        " level0Channels[%u]",
...
HcclResult AllGatherBIRSExecutor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    HCCL_CONFIG_INFO(HCCL_ALG, "[AllGatherBIRSExecutor][KernelRun] starts.");

    CHK_RET(KernelRunLevel0(param, execMem));

    HCCL_INFO("all gather BIRS run success");
    return HCCL_SUCCESS;
}

HcclResult AllGatherBIRSExecutor::SelectAndPrepareBirsTemplate(bool isSingleServer,
                                                                u32 localRank, u32 localRankSize,
                                                                std::unique_ptr<AlgTemplateBase>& templatePtr)
{
    TemplateType tplType = TEMPLATE_ALL_GATHER_BIRS;
    
    // TemplateType tplType = isSingleServer ? TEMPLATE_ALL_GATHER_BIRS
...
HcclResult AllGatherBIRSExecutor::KernelRunLevel0(const OpParam &param, ExecMem &execMem)
{
...
REGISTER_EXEC("AllGatherBIRSExecutor", AllGatherBIRS, AllGatherBIRSExecutor);
}
