...
#include <numeric>
#include "all_gather_mesh_executor.h"
#include "topo_experimental.h"

namespace ops_hccl_experimental {

AllGatherMeshExecutor::AllGatherMeshExecutor() : AllGatherExecutorBase()
{
...
HcclResult AllGatherMeshExecutor::CalcResRequest(HcclComm comm, const OpParam& param, TopoInfo* topoInfo,
    AlgHierarchyInfo& algHierarchyInfo, AlgResourceRequest& resourceRequest, AlgType& algType)
{
    ...
    u32 threadNum = topoInfo->userRankSize;
    ...
    HCCL_INFO("[AllGatherMeshExecutor][CalcResRequest]slaveThreadNum[%u] notifyNumPerThread[%u] notifyNumOnMainThread[%u]"
        " level0Channels[%u]",
...
HcclResult AllGatherMeshExecutor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    HCCL_CONFIG_INFO(HCCL_ALG, "[AllGatherMeshExecutor][KernelRun] starts.");

    CHK_RET(KernelRunLevel0(param, execMem));

    HCCL_INFO("all gather mesh run success");
    return HCCL_SUCCESS;
}

HcclResult AllGatherMeshExecutor::SelectAndPrepareTemplate(bool isSingleServer,
                                                                u32 localRank, u32 localRankSize,
                                                                std::unique_ptr<AlgTemplateBase>& templatePtr)
{
    TemplateType tplType = TEMPLATE_ALL_GATHER_MESH;
    ...
    HCCL_CONFIG_INFO(HCCL_ALG, "[%s] Run TEMPLATE_ALL_GATHER_MESH in COMM_LEVEL0", __func__);

    return HCCL_SUCCESS;
}

HcclResult AllGatherMeshExecutor::KernelRunLevel0(const OpParam &param, ExecMem &execMem)
{
    ...
    CHK_RET(SelectAndPrepareTemplate(isSingleServer, level0LocalRank, level0LocalRankSize, level0TempAlg));
...
REGISTER_EXEC("AllGatherMeshExecutor", AllGatherMesh, AllGatherMeshExecutor);
}
