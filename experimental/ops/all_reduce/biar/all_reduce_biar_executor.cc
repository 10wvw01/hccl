...
#include <numeric>
#include "all_reduce_biar_executor.h"
#include "topo_experimental.h"

namespace ops_hccl_experimental {

AllReduceBIARExecutor::AllReduceBIARExecutor() : AllReduceExecutorBase()
{
...
HcclResult AllReduceBIARExecutor::CalcResRequest(HcclComm comm, const OpParam& param, TopoInfo* topoInfo,
    AlgHierarchyInfo& algHierarchyInfo, AlgResourceRequest& resourceRequest, AlgType& algType)
{
    ...
    HCCL_INFO("[AllReduceBIARExecutor][CalcResRequest]slaveThreadNum[%u] notifyNumPerThread[%u] notifyNumOnMainThread[%u]"
        " level0Channels[%u]",
...
HcclResult AllReduceBIARExecutor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    HCCL_CONFIG_INFO(HCCL_ALG, "[AllReduceBIARExecutor][KernelRun] starts.");
...
HcclResult AllReduceBIARExecutor::SelectAndPrepareBirsTemplate(bool isSingleServer,
                                                                u32 localRank, u32 localRankSize,
                                                                std::unique_ptr<AlgTemplateBase>& templatePtr)
{
    TemplateType RStplType = isSingleServer ? TEMPLATE_ALL_REDUCE_BIAR
                                    : TEMPLATE_REDUCE_SCATTER_BIRS_INTER;
    templatePtr = AlgTemplateRegistry::Instance().GetAlgTemplate(RStplType);
    CHK_SMART_PTR_NULL(templatePtr);
    if (isSingleServer){
        CHK_RET(templatePtr->Prepare(localRank, localRankSize));
        HCCL_CONFIG_INFO(HCCL_ALG, "[%s] Run TEMPLATE_ALL_REDUCE_BIAR in COMM_LEVEL0", __func__);
    } else {
    ...
    // TemplateType AGtplType = TEMPLATE_ALL_GATHER_BIRS;
    // AGtemplatePtr = AlgTemplateRegistry::Instance().GetAlgTemplate(AGtplType);
    // CHK_RET(AGtemplatePtr->Prepare(localRank, localRankSize));
    // HCCL_CONFIG_INFO(HCCL_ALG, "[%s] Run TEMPLATE_ALL_GATHER_BIRS in COMM_LEVEL0", __func__);

    return HCCL_SUCCESS;
}

HcclResult AllReduceBIARExecutor::KernelRunLevel0(const OpParam &param, ExecMem &execMem)
{
    ...
    std::unique_ptr<AlgTemplateBase> tempAlg = std::make_unique<AlgTemplateBaseExperimental>();

    bool isSingleServer = (topoInfo_->serverNum == 1);
    CHK_RET(SelectAndPrepareBirsTemplate(isSingleServer, level0LocalRank, level0LocalRankSize, tempAlg));
    ...
    if (auto Exp = dynamic_cast<AlgTemplateBaseExperimental*>(tempAlg.get())) {
...
REGISTER_EXEC("AllReduceBIARExecutor", AllReduceBIAR, AllReduceBIARExecutor);
}
