...
#ifndef OPS_HCCL_ALLREDUCE_SERIALIZATION
#define OPS_HCCL_ALLREDUCE_SERIALIZATION
...
public:
    ...
    std::string SplitStream(uint64_t& start, uint64_t& end){
        ...
        if (end > temp.length()) {
            HCCL_ERROR("[SplitStream]end[%lu] is bigger than stream length[%lu]", end, temp.length());
            end = temp.length();
        }
...
#endif // OPS_HCCL_ALLREDUCE_SERIALIZATION
