                                      ...
                                      std::unique_ptr<AlgResourceCtxSerializable> &resCtxHost)
{
    ...
    if (HcclEngineCtxGet(comm, param.tag, param.engine, &ctx, &size) == HCCL_SUCCESS) {
        HCCL_INFO("[HcclAllReduceCustom] Engine context already exists, reuse it");
        param.ctxSize = size;
        ...
        HCCL_INFO("[HcclAllReduceCustom] Creating engine context");
        ...
        errno_t memRet = memcpy_s(newCtx, ctxSize, seq.data(), ctxSize);
        if (memRet != 0) {
            HCCL_ERROR("memcpy_s failed, errno: %d", memRet);
...
HcclResult HcclAllReduceCustom(void *sendBuf, void *recvBuf, uint64_t count, HcclDataType dataType,
                                HcclReduceOp opType, HcclComm comm, aclrtStream stream)
{
    HCCL_INFO("Start to execute HcclAllReduceCustom");
    ...
    if (dataType >= HCCL_DATA_TYPE_RESERVED) {
        HCCL_ERROR("[HcclAllReduceCustom] Invalid dataType [%d]", dataType);
        return HCCL_E_PARA;
    }
    uint32_t perDataSize = SIZE_TABLE[dataType];
    uint64_t inputSize = count * perDataSize;
    // AllReduce的output大小等于input大小（与AllGather不同，AllGather的output是input的rankSize倍）
    uint64_t outputSize = inputSize;
    int ret = sprintf_s(param.tag, sizeof(param.tag), "%s", "hccl_custom_allreduce");
    if (ret <= 0) {
        HCCL_ERROR("[HcclAllReduceCustom] Failed to fill param.tag");
        return HCCL_E_INTERNAL;
    }
    CHK_RET(GetDeviceType(&param.devType));
    if (param.devType != DEVICE_TYPE_A5) {
        HCCL_ERROR("[HcclAllReduceCustom] Not Support Device Type [%u]", param.devType);
        return HCCL_E_INTERNAL;
    }

    param.stream = stream;
    CHK_RET(HcclGetCommName(comm, param.commName));
    HCCL_INFO("[HcclAllReduceCustom] commName: %s", param.commName);
    ...
    HCCL_INFO("HcclAllReduceCustom executed successfully");
    return HCCL_SUCCESS;
}
