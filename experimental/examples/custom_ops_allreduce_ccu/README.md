# 自定义通信算子 - AllReduce（CCU）

## 样例介绍

本样例展示如何基于 HCCL 通信编程接口开发 AllReduce 通信算子，包含以下功能点：

1. 基于 CCU_SCHED 通信引擎实现 AllReduce 集合通信算子
2. 使用 `ccu::ReadReduce` 原语从远端读取数据并与本地数据做归约（Reduce）操作
3. 支持自定义算子包的独立构建、独立部署

### 与 AllGather CCU 示例的区别

本样例参考 `examples/05_custom_ops_allgather/ccu/` 的 AllGather CCU 示例进行开发，核心区别如下：

| 对比项 | AllGather | AllReduce |
|--------|-----------|-----------|
| 通信原语 | `ccu::Write`（本卡数据写入远端） | `ccu::ReadReduce`（从远端读取并归约到本地） |
| 输出大小 | input 的 rankSize 倍 | 与 input 相同 |
| 输出偏移 | `dataSize * myRank` | `0` |
| 算法逻辑 | 每个rank将自己的input写入所有远端rank的output | 每个rank先本地拷贝input到output，再从每个远端rank读取input并归约（如SUM）到本地output |
| 前同步交换 | 交换output地址 | 交换input地址 |

`ccu::ReadReduce` 的函数签名如下：

```cpp
CcuResult ReadReduce(ChannelHandle ch, LocalAddr local, RemoteAddr remote, Variable len,
                     HcclDataType dataType, HcclReduceOp opType, Event event, uint16_t mask = 1);
```

该接口从远端地址 `remote` 读取数据，与本地地址 `local` 的数据按 `opType`（如 `HCCL_REDUCE_SUM`）进行归约，结果写回本地地址 `local`。遍历所有远端 rank 后，本地 output 即为所有 rank input 的归约结果。
...
├── op_host/
│   ├── allreduce.cc                    # HcclAllReduceCustom 算子实现源文件
│   ├── utils.cc                        # 工具模块（通道获取、线程获取、Kernel注册）
│   ├── utils.h                         # 工具模块头文件
│   └── CMakeLists.txt
├── op_kernel_ccu/
│   ├── ccu_kernel.cc                   # CCU Kernel 实现逻辑（使用 ccu::ReadReduce）
│   ├── ccu_kernel.h                    # CCU Kernel 头文件
│   ├── exec_op.cc                      # CCU 算子编排逻辑
│   ├── exec_op.h                       # CCU 算子编排头文件
│   └── CMakeLists.txt
├── inc/
│   ├── hccl_custom_allreduce.h         # 自定义 allreduce 算子接口头文件
│   ├── common.h                        # 公共类型头文件
│   ├── binary_stream.h                 # 序列化工具头文件
│   └── log.h                           # 日志宏定义
└── testcase/
    ├── main.cc                         # 样例实现源文件
    └── Makefile                        # 编译/构建配置文件
...
# 编译自定义算子包
cd hccl
bash build.sh --vendor=cust --ops=allreduce_ccu --custom_ops_path=./experimental/examples/custom_ops_allreduce_ccu
```
...
```bash
./build_out/cann-hccl_custom_allreduce_ccu_linux-<arch>.run --install --install-path=<ascend_cann_path>
```
...
- 头文件：`${ASCEND_HOME_PATH}/opp/vendors/cust/include/hccl_custom_allreduce.h`
- 动态库：`${ASCEND_HOME_PATH}/opp/vendors/cust/lib64/libhccl_custom_allreduce.so`
...
在 `experimental/examples/custom_ops_allreduce_ccu/testcase` 代码目录下执行如下命令：
...
./custom_allreduce_ccu
```

### 3. 样例结果示例

所有节点的输入数据初始化为该节点的 DeviceId。使用 SUM 归约操作运行 AllReduce 后，每个 rank 的输出为所有 rank 输入之和。终端将输出类似以下的日志信息（以 2 卡运行为例）：
...
> 说明：2卡场景下，rank 0 输入为 0，rank 1 输入为 1，SUM 归约后所有 rank 的输出均为 0 + 1 = 1。
