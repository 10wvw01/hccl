...
// 单次通信允许的最大字节数：256 MB（与 broadcast 一致）
constexpr uint64_t UB_MAX_DATA_SIZE = 256ULL * 1024 * 1024;
...
HcclResult ExecOp(const OpParam &param, const AlgResourceCtx &resCtx)
{
    ...
    uint64_t maxDataSizePerLoop = std::min(UB_MAX_DATA_SIZE, resCtx.localBuffer.size);
    ...
    CHK_PRT_RET(maxDataCountPerLoop == 0,
        HCCL_ERROR("ExecOp: ccl buffer too small for one element, maxDataSizePerLoop[%llu] dataTypeSize[%u]",
            static_cast<unsigned long long>(maxDataSizePerLoop), dataTypeSize), HCCL_E_INTERNAL);
    ...
    for (uint32_t j = 0; j < param.rankSize; j++) {
        ...
        for (uint64_t k = 0; k < loopCount; k++) {
            ...
            CHK_RET(static_cast<HcclResult>(
                HcommLocalCopyOnThread(thread, recvAddr, cclBuffAddr, subBytes)));
