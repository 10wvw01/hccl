...
This sample demonstrates how to perform a ReduceScatter operation using the `HcclReduceScatter()` interface. It covers the following features:
...
- Call the `HcclReduceScatter()` interface to add up the input data at corresponding positions of all ranks, evenly distribute the result to each rank in rank ID order, and print the result.
