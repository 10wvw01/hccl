...
- Call `HcclAlltoAllV()` to split the input data into a specific number of blocks along a give dimension, send the blocks sequentially to other ranks, receive data from other ranks, concatenate the received data along the same dimension in order, and display the result.
...
Each rank initializes its data with its own `rank_id`. After the `AlltoAllV` operation, the content on each node becomes the concatenation of the input data from all nodes.
