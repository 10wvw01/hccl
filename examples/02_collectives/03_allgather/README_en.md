...
This sample demonstrates how to perform an AllGather operation using the `HcclAllGather()` interface. It covers the following features:
...
- Call the `HcclAllGather()` interface to concatenate the input data of all ranks in the communication domain in rank ID order, send the result to all ranks, and print the result.
