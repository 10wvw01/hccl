...
bool Select(const HcclAlgoPluginParam* param, char* algName, size_t algNameLen)
{
    ...
    if (param->root == 0) {
        int written = std::snprintf(algName, algNameLen, "BroadcastAlgoTree");
    ...
    std::fprintf(stderr, "[BroadcastSelector][Select] root=%u, not hit, fallback to HCCL original logic\n",
        param->root);
    return false;
}
