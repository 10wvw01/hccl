...
bool Select(const HcclAlgoPluginParam* param, char* algName, size_t algNameLen)
{
    ...
    int written = std::snprintf(algName, algNameLen, "%s", hit);
    std::fprintf(stderr, "[AllReduceSelector][Select] totalBytes=%llu, hit=%s\n",
        static_cast<unsigned long long>(totalBytes), hit);
    return written > 0 && static_cast<size_t>(written) < algNameLen;
}
