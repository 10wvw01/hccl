...
extern "C" HcclResult HcclAlgoPluginAllReduceSmall(void* sendBuf, void* recvBuf, uint64_t count,
    HcclDataType dataType, HcclReduceOp op, HcclComm comm, aclrtStream stream)
{
    return FakeExecute("AllReduceAlgoSmall", sendBuf, recvBuf, count, dataType, op, comm, stream);
}

extern "C" HcclResult HcclAlgoPluginAllReduceLarge(void* sendBuf, void* recvBuf, uint64_t count,
    HcclDataType dataType, HcclReduceOp op, HcclComm comm, aclrtStream stream)
{
