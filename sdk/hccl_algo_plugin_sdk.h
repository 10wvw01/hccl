...
public:
    ...
    void Add(const char* algName, const char* soPath, const char* fnSymbol)
    {
        ...
        SafeCopy(entry.algName, sizeof(entry.algName), algName);
        SafeCopy(entry.soPath, sizeof(entry.soPath), soPath);
        SafeCopy(entry.fnSymbol, sizeof(entry.fnSymbol), fnSymbol);
...
/* AllReduce 标准执行函数签名 */
typedef HcclResult (*HcclAlgoPluginAllReduceFn)(void *sendBuf, void *recvBuf, uint64_t count, 
    HcclDataType dataType,HcclReduceOp op, HcclComm comm, aclrtStream stream);

/* AllGather 标准执行函数签名 */
typedef HcclResult (*HcclAlgoPluginAllGatherFn)(void *sendBuf, void *recvBuf, uint64_t sendCount, 
    HcclDataType dataType, HcclComm comm, aclrtStream stream);

/* Broadcast 标准执行函数签名 */
typedef HcclResult (*HcclAlgoPluginBroadcastFn)(void *buf, uint64_t count, HcclDataType dataType, 
    uint32_t root, HcclComm comm, aclrtStream stream);

/* Reduce 标准执行函数签名 */
typedef HcclResult (*HcclAlgoPluginReduceFn)(void *sendBuf, void *recvBuf, uint64_t count,
    HcclDataType dataType, HcclReduceOp op, uint32_t root, HcclComm comm, aclrtStream stream);

/* ReduceScatter 标准执行函数签名 */
typedef HcclResult (*HcclAlgoPluginReduceScatterFn)(void *sendBuf, void *recvBuf, uint64_t recvCount, 
    HcclDataType dataType, HcclReduceOp op, HcclComm comm, aclrtStream stream);

/* AlltoAll类算子标准执行函数签名 */
typedef HcclResult (*HcclAlgoPluginAllToAllFn)(const void *sendBuf, uint64_t sendCount, HcclDataType sendType,
    const void *recvBuf, uint64_t recvCount, HcclDataType recvType, HcclComm comm, aclrtStream stream);

/* Scatter 标准执行函数签名 */
typedef HcclResult (*HcclAlgoPluginScatterFn)(void *sendBuf, void *recvBuf, uint64_t recvCount, 
    HcclDataType dataType, uint32_t root, HcclComm comm, aclrtStream stream);

/* Send/Recv 标准执行函数签名 */
typedef HcclResult (*HcclAlgoPluginSendFn)(void* sendBuf, uint64_t count, HcclDataType dataType, 
    uint32_t destRank, HcclComm comm, aclrtStream stream);
typedef HcclResult (*HcclAlgoPluginRecvFn)(void* recvBuf, uint64_t count, HcclDataType dataType,
