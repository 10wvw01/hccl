...
HcclResult ProfilingManager::ReportTaskExecTimeLine(AicpuComProf *acprof, u32 turnOffset)
{
    if (!GetProfL1State()) {
        return HCCL_SUCCESS;
    }
...
HcclResult ProfilingManager::ReportTaskInfo(s32 streamId, void* ctxPtr)
{
    ...
    bool isSupportBatchReport = (AdprofReportBatchAdditionalInfo != nullptr);
    HCCL_INFO("AdprofReportBatchAdditionalInfo != nullptr: %s", isSupportBatchReport ? "true" : "false");
