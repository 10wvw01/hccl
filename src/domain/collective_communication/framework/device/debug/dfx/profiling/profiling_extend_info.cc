    ...
    const SqeInfo &sqeInfo, MsprofAicpuHcclTaskInfo &msprofAicpuMC2HcclInfo)
{
    ...
    return AssembleProfInfoByType(sqeInfo, msprofAicpuMC2HcclInfo);
}
...
void ProfilingExtendInfoHelper::Ctx2MsprofAicpuMC2HcclInfo(
    const AicpuComContext *ctx, MsprofAicpuHcclTaskInfo &msprofAicpuMC2HcclInfo)
{
    InitHcclInfo(msprofAicpuMC2HcclInfo);
...
std::string ProfilingExtendInfoHelper::AicpuKfcProfCommTurnToString(const AicpuKfcProfCommTurn &aicpuKfcProfCommTurn)
{
    std::stringstream ss;
...
void ProfilingExtendInfoHelper::Init(const AicpuComContext *ctx)
{
    if (AdprofGetHashId == nullptr) {
