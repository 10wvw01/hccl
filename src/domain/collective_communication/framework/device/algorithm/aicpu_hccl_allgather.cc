...
#include "aicpu_hccl_allgather.h"

#include "common/aicpu_hccl_common.h"
#include "framework/aicpu_rpc_server.h"

bool AicpuHcclAllgather::isMCFirstCall = true;

HcclResult AicpuHcclAllgather::RunAlgorithm(HcclReduceOp opType, void *sendBuffer, void *recvBuffer, u64 dataCount,
...
HcclResult AicpuHcclAllgather::RunAllGatherv(HcclReduceOp opType, void *sendBuffer, void *recvBuffer, u64 dataCount,
...
u64 AicpuHcclAllgather::GetWindowOffset(u32 curTurnCnt, u64 curSize, u64 strideCnt, u64 recvBuffer)
{
    u64 windowOffset = 0;
...
HcclResult AicpuHcclAllgather::RunAllGathervMC(HcclReduceOp opType, void *sendBuffer, void *recvBuffer, u64 dataCount,
    HcclDataType dataType, u64 strideCnt, AivAicpuOpParam *nextTask)
{
    ...
    HCCL_INFO("current task: g_proxLoopCnt[%u], windowSize[%llu], windowOffset[%llu], dataCount[%llu], "
        "curSize[%llu], strideCnt[%llu], gatherOut [%#llx]",
        g_proxLoopCnt, ctx_->windowSize, windowOffset, dataCount, curSize, strideCnt, ctx_->gatherOut);
    ...
    if (nextTask != nullptr) {
        ...
        HCCL_INFO("next task: g_proxLoopCnt[%u], workCnt[%u], curTurnCnt[%u], windowSize[%llu], "
            "windowOffset[%llu], dataCount[%llu], nextSize[%llu]",
            g_proxLoopCnt, ctx_->acprof[g_proxLoopCnt].workCnt, ctx_->curTurnCnt, ctx_->windowSize, nextWindowOffset,
            nextTask->count, nextSize);
...
HcclResult AicpuHcclAllgather::GenRingTask(HcclReduceOp opType, u64 sndAddr, u64 rcvAddr, u64 gatherSize,
    HcclDataType dataType, uint32_t streamId, bool isClockwise, uint32_t step, bool isWindowLast) const
{
    ...
    if (step == 1U) { // 首轮send->winIn
        ret = DispatcherAicpu::CopyData(streamId, sndAddr, winIn, gatherSize, dataType, HCCL_REDUCE_RESERVED, rankId_);
    ...
    ret = DispatcherAicpu::CopyData(streamId, evenStep ? preWinOut : preWinIn, recvBufAddr, gatherSize,
    ...
    if (isClockwise) {
        if (!((step == rankNum_ - 1U) && isWindowLast)) {
            CHK_RET(DispatcherAicpu::SignalWait(streamId, (rankId_ + 1U) % rankNum_, DispatcherAicpu::NO_IPC,
                DispatcherAicpu::POST_SYNC));
        }
        // ccore notify
        if ((step < rankNum_ - 1U) && isWindowLast) {
            ret = DispatcherAicpu::AddCcoreNotify(streamId, turn_ * (rankNum_ - 1U) + step);
            CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u add ccore notify failed", turn_, step), ret);
        }
    } else {
        if (!((step == rankNum_ - 1U) && isWindowLast)) {
            CHK_RET(DispatcherAicpu::SignalRecord(streamId, (rankId_ + 1U) % rankNum_, DispatcherAicpu::NO_IPC,
                DispatcherAicpu::POST_SYNC));
        }
    }
    // recv->windowout
    if (step < rankNum_ - 1U){
        ret = DispatcherAicpu::CopyData(streamId, recvBufAddr, evenStep ? winIn : winOut, gatherSize, dataType,
...
HcclResult AicpuHcclAllgather::RunDoubleRingAllGather(HcclReduceOp opType, u64 sendBuffer, u64 recvBuffer,
    ...
    while (countLeft > 0) {
        ...
        for (uint32_t step = 1U; step <= rankNum_ - 1U; step++) {
            ...
            if ((step == rankNum_ - 1U) && isWindowLast) { // 拷贝全量本卡直接从send->recv
                if (opType != HCCL_REDUCE_PROD) {
                    ret = DispatcherAicpu::CopyData(mainStream, sendBuffer, recvBuffer + rankId_ * rankDataSize, rankDataSize,
