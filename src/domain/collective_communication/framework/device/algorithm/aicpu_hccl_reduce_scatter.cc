...
#include "aicpu_hccl_reduce_scatter.h"
#include "common/aicpu_hccl_common.h"

HcclResult AicpuHcclReduceScatter::RunAlgorithm(HcclReduceOp opType, void *sendBuffer, void *recvBuffer, u64 dataCount,
...
HcclResult AicpuHcclReduceScatter::RunDeterministicReduceScatterLocal(HcclReduceOp opType, void *sendBuffer,
...
HcclResult AicpuHcclReduceScatter::RunReduceScatterWriteMode(HcclReduceOp opType, void *sendBuffer, void *recvBuffer,
...
HcclResult AicpuHcclReduceScatter::RunReduceScatterReadMode(HcclReduceOp opType, void *sendBuffer, void *recvBuffer,
...
std::vector<Slice> AicpuHcclReduceScatter::PrepareMeshSlice(u64 dataSize, uint32_t rankNum_)
{
...
HcclResult AicpuHcclReduceScatter::RunDeterministicReduceScatter(HcclReduceOp opType, void *sendBuffer,
...
HcclResult AicpuHcclReduceScatter::RunCurrentDeterministicReduceScatter(HcclReduceOp opType, uint8_t *curInputPtr,
    uint8_t *curOutputPtr, u64 strideSize, u64 curSize, HcclDataType dataType)
{
    ...
    // 1. 片内send->recv
    DispatcherAicpu::CopyData(rankId_, static_cast<void *>(curInputPtr + rankId_ * strideSize), curOutputPtr, curSize,
...
HcclResult AicpuHcclReduceScatter::GenRingTask(HcclReduceOp opType, u64 sndAddr, u64 rcvAddr, u64 curSize,
    u64 scatterSize, HcclDataType dataType, uint32_t streamId, bool isClockwise, uint32_t step) const
{
    ...
    if (step == 1U) { // 首轮send->winIn winOut
        ret = DispatcherAicpu::CopyData(streamId, sndAddr, winIn, curSize, dataType, HCCL_REDUCE_RESERVED, rankId_);
        CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u send to clock winIn failed", turn_, step), ret);

        ret = DispatcherAicpu::CopyData(streamId, sndAddr + scatterSize, winOut, curSize, dataType,
        ...
        ret = DispatcherAicpu::CopyData(streamId, sndAddr, evenStep ? winIn : winOut, curSize, dataType,
            HCCL_REDUCE_RESERVED, rankId_);
        CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u send to clock winOut failed", turn_, step), ret);
    }
    // 片间同步 notify后卡 wait前卡
    ret = DispatcherAicpu::SignalRecord(streamId, postRankId, DispatcherAicpu::IPC, DispatcherAicpu::PRE_SYNC);
    CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u notify post rank failed", turn_, step), ret);

    ret = DispatcherAicpu::SignalWait(streamId, preRankId, DispatcherAicpu::IPC, DispatcherAicpu::PRE_SYNC);
    CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u wait pre rank failed", turn_, step), ret);

    // 片间memcpy 前卡window->window
    ret = DispatcherAicpu::CopyData(streamId, evenStep ? preWinOut : preWinIn, evenStep ? winIn : winOut, curSize,
    ...
    if (step == rankNum_ - 1U) { // 最后一轮 win输出至recv
        ret = DispatcherAicpu::CopyData(streamId, evenStep ? winIn : winOut, rcvAddr, curSize, dataType,
            HCCL_REDUCE_RESERVED, rankId_);
        CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u window to recv failed", turn_, step), ret);
    }
    ret = DispatcherAicpu::SignalRecord(streamId, preRankId, DispatcherAicpu::IPC, DispatcherAicpu::POST_SYNC);
    CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u notify pre rank failed", turn_, step), ret);
    // 片间同步 notify前卡 wait后卡
    ret = DispatcherAicpu::SignalWait(streamId, postRankId, DispatcherAicpu::IPC, DispatcherAicpu::POST_SYNC);
    CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u wait post rank failed", turn_, step), ret);
    return HCCL_SUCCESS;
}

HcclResult AicpuHcclReduceScatter::RunDoubleRingReduceScatter(HcclReduceOp opType, u64 sendBuffer, u64 recvBuffer,
    ...
    while (countLeft > 0) {
        ...
        for (uint32_t step = 1U; step <= rankNum_ - 1U; step++) {
            if (isWindowFirst) {
                ...
                ret = DispatcherAicpu::AddCcoreWait(mainStream, waitAddr, turn_ * (rankNum_ - 1U) + step,
            ...
            // 主->从
            CHK_RET(
                DispatcherAicpu::SignalRecord(mainStream, subStream, DispatcherAicpu::NO_IPC, DispatcherAicpu::PRE_SYNC));
            CHK_RET(DispatcherAicpu::SignalWait(subStream, subStream, DispatcherAicpu::NO_IPC, DispatcherAicpu::PRE_SYNC));
            ...
            // 从->主
            CHK_RET(
                DispatcherAicpu::SignalRecord(subStream, subStream, DispatcherAicpu::NO_IPC, DispatcherAicpu::POST_SYNC));
            CHK_RET(
                DispatcherAicpu::SignalWait(mainStream, subStream, DispatcherAicpu::NO_IPC, DispatcherAicpu::POST_SYNC));
            if (isWindowLast) {
                ...
                // ccore notify
                ret = DispatcherAicpu::AddCcoreNotify(mainStream, turn_ * (rankNum_ - 1U) + step);
...
HcclResult AicpuHcclReduceScatter::RunSwitchReduceScatter(HcclReduceOp opType, u64 sendBuffer, u64 recvBuffer,
    u64 dataCount, HcclDataType dataType) const
{
    const u64 scatterSize = dataCount / rankNum_ * unitSize_;
    ...
    while (countLeft > 0) {
        ...
        for (uint32_t step = 1U; step <= rankNum_; step++) {
            ...
            if (isWindowFirst) {
                // ccore wait
                u64 waitAddr = ctx_->workSpaceAddr + ctx_->notifyOff + offsetof(AivAicpuOpParam, sendCnt);
                ret = DispatcherAicpu::AddCcoreWait(mainStream, waitAddr, turn_ * rankNum_ + step,
            ...
            if (step != rankNum_) {
                // 片间同步 notify前卡 wait后卡
                ret = DispatcherAicpu::SignalRecord(mainStream, preRankId, DispatcherAicpu::IPC, DispatcherAicpu::PRE_SYNC);
                CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u notify pre rank failed", turn_, step), ret);

                ret = DispatcherAicpu::SignalWait(mainStream, postRankId, DispatcherAicpu::IPC, DispatcherAicpu::PRE_SYNC);
                CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u wait post rank failed", turn_, step), ret);

                // 片间memcpy snd->后序卡win 首轮采用覆盖
                ret = DispatcherAicpu::CopyData(mainStream, stepSndAddr, ctx_->rankInfo[postRankId].window, curSize,
                ...
                // 片间同步 notify后卡 wait前卡
                ret =
                    DispatcherAicpu::SignalRecord(mainStream, postRankId, DispatcherAicpu::IPC, DispatcherAicpu::POST_SYNC);
                CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u notify post rank failed", turn_, step), ret);

                ret = DispatcherAicpu::SignalWait(mainStream, preRankId, DispatcherAicpu::IPC, DispatcherAicpu::POST_SYNC);
                CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u wait pre rank failed", turn_, step), ret);
            } else { // 最后一轮仅作本卡内拷贝
                // snd->recv
                ret = DispatcherAicpu::CopyData(mainStream, stepSndAddr, rcvAddr, curSize, dataType,
                                                HCCL_REDUCE_RESERVED, rankId_);
                CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u cpy snd to rcv failed", turn_, step), ret);
                stepSndAddr += scatterSize;

                // win->recv
                ret = DispatcherAicpu::CopyData(mainStream, ctx_->rankInfo[rankId_].window, rcvAddr, curSize, dataType,
                    opType, rankId_);
            ...
            if (isWindowLast) {
                ...
                // ccore notify
                ret = DispatcherAicpu::AddCcoreNotify(mainStream, turn_ * rankNum_ + step);
                CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("Turn %u step %u add ccore notify failed", turn_, step), ret);
            }
        }
        isWindowFirst = false;
