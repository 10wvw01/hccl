...
HcclResult HcclAlgoPluginMgr::Init()
{
    ...
    std::call_once(initFlag_, [this]() {
         ...
         */
        soHandle_ = dlopen(pluginPath, RTLD_NOW | RTLD_GLOBAL);
        if (soHandle_ == nullptr) {
            HCCL_WARNING("[HcclAlgoPluginMgr] dlopen PluginBroker failed: %s, dlerror=%s. "
                "Fallback to original HCCL algorithm selection/execution logic.", pluginPath, dlerror());
            loaded_ = false;
            return;
        }
        ...
        if (getTableFn == nullptr) {
            HCCL_WARNING("[HcclAlgoPluginMgr] dlsym GetHcclAlgoPlugin failed: %s. Reject this PluginBroker, "
                "fallback to original logic.", dlerror());
            Unload();
            return;
        }
        ...
        /* 接口版本校验：版本不匹配则拒绝加载，降级为原有选择逻辑 */
        if (table->version != HCCL_PLUGIN_API_VERSION) {
            HCCL_WARNING("[HcclAlgoPluginMgr] PluginBroker version mismatch: expect=%u, actual=%u. "
                "Reject this PluginBroker to avoid loading illegal/corrupted module.",
                HCCL_PLUGIN_API_VERSION, table->version);
            Unload();
            return;
        }
        if (table->IsReady == nullptr || table->FetchContext == nullptr ||
            table->SelectAlg == nullptr || table->ExecuteAlg == nullptr || table->QueryAlgs == nullptr) {
    ...
    return HCCL_SUCCESS; /* Init()本身不因Plugin加载失败而返回错误，失败即静默降级 */
}
...
HcclAlgoPlugin_t* HcclAlgoPluginMgr::GetPlugin()
{
    return loaded_ ? pluginTable_ : nullptr;
}

void* HcclAlgoPluginMgr::GetContext()
{
    return loaded_ ? ctx_ : nullptr;
}

bool HcclAlgoPluginMgr::IsLoaded() const
{
    return loaded_;
}
...
void ExtractCountAndDataType(const OpParam& param, uint64_t& count, HcclDataType& dataType)
{
    ...
    if (param.opType == HcclCMDType::HCCL_CMD_ALLTOALLV || param.opType == HcclCMDType::HCCL_CMD_ALLTOALLVC ||
        param.opType == HcclCMDType::HCCL_CMD_ALLGATHER_V || param.opType == HcclCMDType::HCCL_CMD_REDUCE_SCATTER_V ||
...
const char* OpTypeToName(HcclCMDType opType)
{
    switch (opType) {
        ...
        case HcclCMDType::HCCL_CMD_RECEIVE:          return "Recv";
...
void FillHcclAlgoPluginParam(const OpParam& param, const TopoInfoWithNetLayerDetails* topoInfo,
    HcclAlgoPluginParam& pluginParam)
{
    ...
    if (topoInfo != nullptr) {
        ...
        pluginParam.isAsymmetricTopo = topoInfo->isDiffDeviceModule ||
