                ...
                for (int oneDid = 0; oneDid < yRankSize; oneDid++) {
                    if (oneDid == rooty) {
                        ...
                        for (u64 connerDataSlice = 0; connerDataSlice < xRankSize; connerDataSlice++) {
                            if (connerDataSlice == rootx)
                                continue;
                            u64 currentDataSliceId = rootz * xRankSize * yRankSize + connerDataSlice * yRankSize
                                                     + oneDid; // 算算和oneDid节点同y轴的rank
                            u64 pieceId = currentDataSliceId;
                            u64 sliceSizeOnePiece
                                = DataSliceCut(ySDataSize[root][osn][0], xySOffset[root][0] + ySOffset[root][osn][0],
                                    omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                            u64 inputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + ySOffset[root][osn][0],
                                                         omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                                                     + omniPipeSplitSliceInfoListPerLoop[pieceId].offset;
                            u64 outputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + ySOffset[root][osn][0],
                                                          omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                                                      + omniPipeSplitSliceInfoListPerLoop[pieceId].offset;
                        ...
                        for (u64 connerDataSlice = 0; connerDataSlice < yRankSize; connerDataSlice++) {
                            ...
                            u64 sliceSizeOnePiece
                                = DataSliceCut(xSDataSize[root][osn][0], xySOffset[root][0] + xSOffset[root][osn][0],
                                    omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                            u64 inputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + xSOffset[root][osn][0],
                                                         omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                                                     + omniPipeSplitSliceInfoListPerLoop[pieceId].offset;
                            u64 outputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + xSOffset[root][osn][0],
                                                          omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                                                      + omniPipeSplitSliceInfoListPerLoop[pieceId].offset;
                            sliceSizeMultRankPiece1.push_back(sliceSizeOnePiece);
                            sliceCountMultRankPiece1.push_back(sliceSizeOnePiece / dataTypeSize);
                    ...
                    if (oneDid == rootx) {
                        ...
                        for (u64 connerDataSlice = 0; connerDataSlice < yRankSize; connerDataSlice++) {
                            if (connerDataSlice == rooty)
                                continue;
                            u64 currentDataSliceId
                                = rootz * xRankSize * yRankSize + oneDid * xRankSize + connerDataSlice;
                            u64 pieceId = currentDataSliceId;
                            u64 sliceSizeOnePiece = DataSliceCut(ySDataSize[root][osn][isn],
                                xySOffset[root][osn] + ySOffset[root][osn][isn],
                                omniPipeSplitSliceInfoListPerLoop[pieceId].size);
