        ...
        for (u64 isn = 0; isn < xInCornerStep; isn++) { 
            ...
            for (int oneDid = 0; oneDid < xRankSize; oneDid++) {
                ...
                PushStepFields(stepSliceInfotmp, sliceSizeMultRankPiece, sliceCountMultRankPiece,
                               inputOmniPipeSliceStrideMultRankPiece, outputOmniPipeSliceStrideMultRankPiece, 0,
                               xyzDataSizeStep[0][osn * innerStepNum + isn] * oneDid);
            }
            dataSliceLevelx.insert(dataSliceLevelx.end(), stepSliceInfotmp);
        }
        for (u64 isn = xInCornerStep; isn < innerStepNum; isn++) {
        ...
        for (u64 osn = xyConnerStep; osn < outerStepNum; osn++) {
            ...
            for (u64 isn = 0; isn < xInCornerStep; isn++) {
                ...
                for (int oneDid = 0; oneDid < xRankSize; oneDid++) {
                    ...
                    for (u64 connerDataSlice = 0; connerDataSlice < yRankSize; connerDataSlice++) {
                        u64 currentDataSliceId = rootz * xRankSize * yRankSize + connerDataSlice * xRankSize
                                                 + oneDid; // 斜对角先算算是哪一片
                        if (connerDataSlice != rooty) {
                            ...
                            //     omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                            // u64 inputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + xSOffset[root][osn][isn],
                            //                              omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                            //                          + omniPipeSplitSliceInfoListTotal[pieceId].offset;
                            // u64 outputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + xSOffset[root][osn][isn],
                            //                               omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                            ...
                            CalcAndPushPiece(pieceId, xySOffset[root][osn] + xSOffset[root][osn][isn],
                                             ...
                                             omniPipeSplitSliceInfoListTotal, dataTypeSize,
                ...
                for (int oneDid = 0; oneDid < yRankSize; oneDid++) {
                    ...
                    //     for (u64 connerDataSlice = 0; connerDataSlice < sliceSizeMultRankPiece.size();
                    //         connerDataSlice++) {
            ...
            for (u64 isn = xInCornerStep; isn < innerStepNum; isn++) {
                ...
                for (int oneDid = 0; oneDid < xRankSize; oneDid++) {
                    ...
                    // u64 sliceSizeOnePiece
                    //     = DataSliceCut(xSDataSize[root][osn][isn], xySOffset[root][osn] + xSOffset[root][osn][isn],
                    //         omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                    // u64 inputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + xSOffset[root][osn][isn],
                    //                              omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                    //                          + omniPipeSplitSliceInfoListTotal[pieceId].offset;
                ...
                for (int oneDid = 0; oneDid < yRankSize; oneDid++) {
                    if (oneDid == rooty) {
                        ...
                        // stepSliceInfotmp.stepInputSliceStride.push_back(0);
                        // stepSliceInfotmp.stepOutputSliceStride.push_back(
                        //     xyzDataSizeStep[0][osn * innerStepNum + isn] * oneDid);
                        // stepSliceInfotmp.inputOmniPipeSliceStride.push_back(inputOmniPipeSliceStrideMultRankPiece);
                        // stepSliceInfotmp.outputOmniPipeSliceStride.push_back(outputOmniPipeSliceStrideMultRankPiece);
                        // stepSliceInfotmp.stepCount.push_back(sliceCountMultRankPiece);
        ...
        for (u64 osn = xyConnerStep; osn < outerStepNum; osn++) {
            for (u64 isn = 0; isn < xInCornerStep + 1; isn++) {
                ...
                for (int oneDid = 0; oneDid < xRankSize; oneDid++) {
                    ...
                    for (u64 connerDataSlice = 0; connerDataSlice < yRankSize; connerDataSlice++) {
                        u64 currentDataSliceId = zAxis * xRankSize * yRankSize + connerDataSlice * xRankSize
                                                 + oneDid; // 斜对角先算算是哪一片
                        if (connerDataSlice != rooty) {
                            u64 pieceId = currentDataSliceId;
                            // u64 sliceSizeOnePiece = DataSliceCut(xSDataSize[root][osn][isn],
            ...
            for (u64 isn = xInCornerStep + 1; isn < innerStepNum; isn++) {
                ...
                for (int oneDid = 0; oneDid < xRankSize; oneDid++) {
                    ...
                    u64 pieceId = zAxis * xRankSize * yRankSize + rooty * xRankSize + oneDid;
                    // u64 sliceSizeOnePiece
                    //     = DataSliceCut(xSDataSize[root][osn][isn], xySOffset[root][osn] + xSOffset[root][osn][isn],
                    //         omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                    // u64 outputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + xSOffset[root][osn][isn],
                    ...
                    if (oneDid == rooty) {
                        ...
                        for (u64 connerDataSlice = 0; connerDataSlice < xRankSize; connerDataSlice++) {
                            if (connerDataSlice == rootx)
                                continue;
                            u64 currentDataSliceId = rootz * xRankSize * yRankSize + oneDid * xRankSize
                                                     + connerDataSlice; // 算算和oneDid节点同y轴的rank
                            u64 pieceId = currentDataSliceId;
                            // u64 sliceSizeOnePiece
                            //     = DataSliceCut(ySDataSize[root][osn][0], xySOffset[root][0] + ySOffset[root][osn][0],
                            //         omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                            // u64 inputPieceIdOffset = sliceOffsetCut(xySOffset[root][0] + ySOffset[root][osn][0],
                            //                              omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                            //                          + omniPipeSplitSliceInfoListPerLoop[pieceId].offset;
                            // u64 outputPieceIdOffset = sliceOffsetCut(xySOffset[root][0] + ySOffset[root][osn][0],
                            //                               omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                            //                           + omniPipeSplitSliceInfoListPerLoop[pieceId].offset;
                            ...
                            CalcAndPushPiece(pieceId, xySOffset[root][0] + ySOffset[root][osn][0],
                                             ...
                                             omniPipeSplitSliceInfoListPerLoop, dataTypeSize,
                                             sliceSizeMultRankPiece1, sliceCountMultRankPiece1,
    ...
    for (u64 osn = 0; osn < xyConnerStep; osn++) {
        for (u64 isn = 0; isn < yInCornerStep; isn++) {
            ...
            for (int oneDid = 0; oneDid < yRankSize; oneDid++) {
                ...
                PushStepFields(stepSliceInfotmp, sliceSizeMultRankPiece, sliceCountMultRankPiece,
                               inputOmniPipeSliceStrideMultRankPiece, outputOmniPipeSliceStrideMultRankPiece, 0,
                               xyzDataSizeStep[1][osn * innerStepNum + isn] * oneDid);
            }
            dataSliceLevely.insert(dataSliceLevely.end(), stepSliceInfotmp);
        }
        for (u64 isn = yInCornerStep; isn < innerStepNum; isn++) {
        ...
        for (u64 osn = xyConnerStep; osn < outerStepNum; osn++) {
                ...
                for (int oneDid = 0; oneDid < yRankSize; oneDid++) {
                    ...
                    for (u64 connerDataSlice = 0; connerDataSlice < xRankSize; connerDataSlice++) {
                        ...
                        if (connerDataSlice != rootx) {
                            ...
                            //     omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                            // u64 inputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + ySOffset[root][osn][isn],
                            //                              omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                            //                          + omniPipeSplitSliceInfoListTotal[pieceId].offset;
                            // u64 outputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + ySOffset[root][osn][isn],
                            //                               omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                            //                           + omniPipeSplitSliceInfoListPerLoop[pieceId].offset;
            ...
            for (u64 isn = yInCornerStep + 1; isn < innerStepNum; isn++) {
                ...
                for (int oneDid = 0; oneDid < yRankSize; oneDid++) {
                    ...
                    u64 pieceId = zAxis * xRankSize * yRankSize + oneDid * xRankSize + rootx;
                    // u64 sliceSizeOnePiece
                    //     = DataSliceCut(ySDataSize[root][osn][isn], xySOffset[root][osn] + ySOffset[root][osn][isn],
                    //         omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                    // u64 outputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + ySOffset[root][osn][isn],
                    //                               omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                    //                           + omniPipeSplitSliceInfoListPerLoop[pieceId].offset;
                    ...
                    if (oneDid == rootx) {
                        ...
                        for (u64 connerDataSlice = 0; connerDataSlice < yRankSize; connerDataSlice++) {
                            if (connerDataSlice == rooty)
                                continue;
                            u64 currentDataSliceId = zAxis * xRankSize * yRankSize + connerDataSlice * xRankSize
                                                     + oneDid; // 算算和oneDid节点同y轴的rank
                            u64 pieceId = currentDataSliceId;
                            // u64 sliceSizeOnePiece
                            //     = DataSliceCut(xSDataSize[root][osn][0], xySOffset[root][0] + xSOffset[root][osn][0],
                            //         omniPipeSplitSliceInfoListPerLoop[pieceId].size);
                            // u64 inputPieceIdOffset = sliceOffsetCut(xySOffset[root][0] + xSOffset[root][osn][0],
                            //                              omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                            //                          + omniPipeSplitSliceInfoListPerLoop[pieceId].offset;
                            // u64 outputPieceIdOffset = sliceOffsetCut(xySOffset[root][0] + xSOffset[root][osn][0],
                            //                               omniPipeSplitSliceInfoListPerLoop[pieceId].size)
                            //                           + omniPipeSplitSliceInfoListPerLoop[pieceId].offset;
                            // sliceSizeMultRankPiece1.push_back(sliceSizeOnePiece);
                            // sliceCountMultRankPiece1.push_back(sliceSizeOnePiece / dataTypeSize);
                            // inputOmniPipeSliceStrideMultRankPiece1.push_back(inputPieceIdOffset);
                            // outputOmniPipeSliceStrideMultRankPiece1.push_back(outputPieceIdOffset);
                            CalcAndPushPiece(pieceId, xySOffset[root][0] + xSOffset[root][osn][0],
