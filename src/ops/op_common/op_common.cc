...
HcclResult HcclGetChannelForCcu(HcclComm comm, const OpParam &param, AlgResourceRequest &resRequest)
{
    ...
    for (CcuKernelInfo& kernelInfo: resRequest.ccuKernelInfos) {
        ...
        if (channelNum > 0) {
            ...
            DieIdType dieId = 0;
            CHK_RET(HcclRankGraphGetEndpointInfo(comm, param.userRank, &localEndpoint,
...
// 资源聚合规则：同 resGroup 内逐 kernel 相加、不同 resGroup 之间取最大
static HcclResult CalcKernelsAggregatedResReq(AlgResourceRequest &resRequest, HcommCcuResDescHandle finalReq)
{
    ...
    for (u32 i = 0; i < totalKernelNum; i++) {
        ...
        CcuResult createRet = HcommCcuInsResDescCreate(0, &kernelDesc);
                                     ...
                                     std::unique_ptr<AlgResourceCtxSerializable> &resCtxHost)
{
    ...
    while (currentResGroup <= maxResGroup) {
        ...
        for (u32 i = 0; i < totalKernelNum; i++) {
            ...
            HCCL_DEBUG("[RegisterCcuKernels] kernelFuncName[%s] dieId[%u]", kernelInfo.kernelFuncName, kernelInfo.dieId);
                                          ...
                                          std::unique_ptr<AlgResourceCtxSerializable> &resCtxHost)
{
    ...
    CcuResult createRet = HcommCcuInsResDescCreate(0, &reqDesc);
    if (createRet != CCU_SUCCESS) {
