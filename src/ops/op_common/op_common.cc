    ...
    std::map<uint32_t, std::map<u32, std::map<HcommCcuResType, uint32_t>>> &groupedResMap)
{
    ...
    for (HcommCcuResType resType : GetCcuResTypes()) {
        ...
        HCCL_INFO("[AccumulateKernelRes] kernel[%s] dieId[%u] resGroup[%u] resType[%d] resNum[%u].",
                  kernelInfo.kernelFuncName, kernelInfo.dieId, kernelInfo.resGroup, resType, resNum);
...
static bool IsResCapSufficient(HcommCcuResDescHandle resCap, HcommCcuResDescHandle resReq)
{
    ...
    for (HcommCcuResType resType : GetCcuInsCreateResTypes()) {
        ...
        HCCL_INFO("[IsResCapSufficient] resType[%d] cap[%u] req[%u] %s.",
                   resType, capNum, reqNum, capNum >= reqNum ? "sufficient" : "insufficient");
        if (capNum < reqNum) {
...
static HcclResult BuildAggregatedResReq(AlgResourceRequest &resRequest, ResDescByDie &reqDescs)
{
    ...
    for (auto &dieEntry : groupedResMap) {
        ...
        for (HcommCcuResType resType : GetCcuInsCreateResTypes()) {
            ...
            HCCL_INFO("[BuildAggregatedResReq] dieId[%u] resType[%d] aggregated maxNum[%u].",
                      dieId, resType, maxNum);
        }
