    ...
    ThreadHandle exportedCpuTsThread, u32 notifyNumOnMainThread, void *resCtxSequence, std::string &algName, ThreadHandle unfoldThread)
{
    ...
    if (isCapture) {
        ...
        HcclOpbaseLaunchInOrderToOrderStream(comm, param, unfoldThread, UNFLOD_THREAD_NOTIFY_IDX, execTimeout);
    } else if (param.opMode == OpMode::OFFLOAD) {
        HcclHcommLaunchInOrderToOrderStream(comm, param, unfoldThread, UNFLOD_THREAD_NOTIFY_IDX, execTimeout);
    }
    /*************************/
    ...
    if (isCapture) {
        ...
        HcclOpbaseLaunchInOrderToKernelStream(comm, HOST_ORDER_THREAD_NOTIFY_IDX, execTimeout);
    } else if (param.opMode == OpMode::OFFLOAD) {
        HcclHcommLaunchInOrderToKernelStream(comm, HOST_ORDER_THREAD_NOTIFY_IDX, execTimeout);
    }
    /**************************/
