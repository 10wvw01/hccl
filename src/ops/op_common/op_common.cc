                      ...
                      std::unique_ptr<TopoInfoWithNetLayerDetails> &topoInfo, std::string &algName, const ResPackGraphMode &resPack)
{
    ...
    if ((param.engine == COMM_ENGINE_AICPU_TS) || (param.engine == COMM_ENGINE_CPU)) {
        ...
        uint32_t notifyNum = (isGroupEnabled && (param.engine == COMM_ENGINE_AICPU_TS)) ? 3 : 1;
        CHK_RET(HcclThreadAcquireWithStream(comm, COMM_ENGINE_CPU_TS, param.stream, notifyNum, &cpuTsThread));
        // Export cpuTsThread
