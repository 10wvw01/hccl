...
HcclResult AivFlagClear(HcclComm comm, HcclCommStateOp state, void* userPtr)
{
    if (state != HcclCommStateOp::HCCL_COMM_STATE_RESUME_POST) {
        HCCL_INFO("[%s] CommStateOp is not STATE_RESUME_POST, skip aiv flag clear.", __func__);
        return HCCL_SUCCESS;
    }
    ...
    HcclComm comm, const OpParam &param, AlgResourceRequest &resRequest, AlgResourceCtxSerializable* resCtxHost)
{
    ...
    if (ret == HCCL_E_NOT_FOUND || ret == HCCL_E_PARA) {
        ...
        CHK_RET(HcclCommRegCommStateCallBack(param.commModeTag, AivFlagClear, resCtxHost->aivCommInfoPtr));
        // 注册到通信域，支持建链时交换
