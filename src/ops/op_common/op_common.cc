...
HcclResult NegotiateCcuResFallback(HcclComm comm, const OpParam &param, bool localResOk)
{
    ...
    if (needCreate) {
        ...
        // 按文档示例构造HcclCommConfig，设置AICPU引擎和ccl_buffer
        HcclCommConfig subCommConfig;
        HcclCommConfigInit(&subCommConfig);
        subCommConfig.hcclBufferSize = NEGOTIATION_CCL_BUFFER_SIZE;
        subCommConfig.hcclOpExpansionMode = static_cast<uint32_t>(HcclOpExpansionMode::HCCL_OP_EXPANSION_MODE_AI_CPU);
        auto nameRet = sprintf_s(subCommConfig.hcclCommName, sizeof(subCommConfig.hcclCommName), "%s",
            negTag.c_str());
        CHK_PRT_RET(nameRet <= 0,
            HCCL_ERROR("[NegotiateCcuResFallback] sprintf_s for hcclCommName failed."), HCCL_E_INTERNAL);

        HcclResult subRet = HcclCreateSubCommConfig(&comm, rankSize, rankIds.data(), 0,
            static_cast<uint32_t>(devId), &subCommConfig, &negCtx->subComm);
