    ...
    ThreadHandle exportedCpuTsThread, u32 notifyNumOnMainThread, void *resCtxSequence, std::string &algName, ThreadHandle unfoldThread)
{
    ...
    HcclRtEvent event0 = nullptr;
    HcclRtEvent event1 = nullptr;
    if (isCapture) {
        aclError eventRet = aclrtCreateEventExWithFlag(&event0, ACL_EVENT_SYNC);
        CHK_PRT_RET(eventRet != ACL_SUCCESS, HCCL_ERROR("[%s]aclrtCreateEventExWithFlag failed, ret[%d] event[%p].",
                __func__, eventRet, event0), HCCL_E_RUNTIME);
        eventRet = aclrtCreateEventExWithFlag(&event1, ACL_EVENT_SYNC);
        if (eventRet != ACL_SUCCESS) {
            (void)aclrtDestroyEvent(event0);
            event0 = nullptr;
            HCCL_ERROR("[%s]aclrtCreateEventExWithFlag failed, ret[%d] event[%p].", __func__, eventRet, event1);
            return HCCL_E_RUNTIME;
        }
        CHK_RET(HcclAclgraphLaunchInOrderToOrderStream(comm, param, unfoldThread,UNFLOD_THREAD_NOTIFY_IDX, execTimeout, event0));
    ...
    /************TODO:第二阶段 *************/
    if (isCapture) {
        CHK_RET(HcclAclgraphLaunchInOrderToKernelStream(comm, unfoldThread, HOST_ORDER_THREAD_NOTIFY_IDX, execTimeout, event1));
    } else if (param.opMode == OpMode::OPBASE) {
    ...
    if (ret != HCCL_SUCCESS) {
        ...
        if (event0 != nullptr) { (void)aclrtDestroyEvent(event0); }
        if (event1 != nullptr) { (void)aclrtDestroyEvent(event1); }
        return ret;
    }
