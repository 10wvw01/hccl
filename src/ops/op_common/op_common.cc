...
HcclResult HcclGetChannelImpl(const u32 level, HcclComm comm, const OpParam &param, std::vector<HcclChannelDesc>& channelRequest,
    ...
    for (u32 idx = 0; idx < channelNum; idx++) {
        ...
        HCCL_INFO("[%s]remoteRank[%u] protocol[%u] portGroupSize[%u] dieId[%u] remoteCclBufferAddr[0x%llx] remoteCclBufferSize[%u]",
...
HcclResult SetMultipleDimensionSplitRatio(HcclComm comm, OpParam &param) {
    ...
    if (isCommConfigured) {
        param.multipleDimensionSplitRatio = commRatio;
        param.multipleDimensionSplitRatioSource = MultipleDimensionSplitRatioSource::COMM_CONFIG;
    ...
    if (GetExternalInputMultipleDimensionSplitRatio(envRatio)) {
        ...
        param.multipleDimensionSplitRatio = envRatio;
        param.multipleDimensionSplitRatioSource = MultipleDimensionSplitRatioSource::ENV_CONFIG;
        HCCL_INFO("[SetMultipleDimensionSplitRatio] ratioSource[ENV_CONFIG], configuredRatio[%f]", envRatio);
        return HCCL_SUCCESS;
    }

    param.multipleDimensionSplitRatio = defaultRatio;
    param.multipleDimensionSplitRatioSource = MultipleDimensionSplitRatioSource::BUILTIN_FORMULA;
