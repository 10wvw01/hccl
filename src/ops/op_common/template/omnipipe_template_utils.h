...
namespace ops_hccl {
inline HcclResult FillOmniPipeTemplateAlgParams(TemplateDataParams &tempAlgParams,
...
HcclResult PrepareOmniPipeDataSplitForMultiChannel(AlgTemplate &algTemplate,
    const TemplateDataParams &tempAlgParams, HcclDataType dataType, const TemplateResource &templateResource,
    ...
    std::vector<std::vector<std::vector<u64>>> &dataOffsetVec)
{
    ...
    for (uint32_t idx = 0; idx < tempAlgParams.stepSliceInfo.stepSliceSize.size(); idx++) {
        ...
        for (uint32_t rpt = 0; rpt < tempAlgParams.stepSliceInfo.stepSliceSize[0].size(); rpt++) {
            ...
            CHK_RET(algTemplate.CalcDataSplitByPortGroup(totalDataCount, dataTypeSize,
    ...
    HCCL_INFO("stepSliceSize.size()[%d], stepSliceSize[0].size()[%d]",
        tempAlgParams.stepSliceInfo.stepSliceSize.size(), tempAlgParams.stepSliceInfo.stepSliceSize[0].size());
    return HCCL_SUCCESS;
}
} // namespace ops_hccl
