...
// 共享内存排布：|stop flag[1]|HcclResult[2]|DpuTaskexceptionInfo| 其中，HcclResult为aicpu侧读取是否有错的标志位, stop flag为aicpu侧读取是否停止的标志位
    ...
    if (UNLIKELY(hcclRet != HCCL_SUCCESS)) {
        if (hcclRet == HCCL_E_AGAIN) {
            ...
            if (taskexpShmem != nullptr) {
                ...
                uint8_t *hcclRetPtr = stopFlagPtr + sizeof(uint8_t); // todo magicnum
                uint8_t *dstDataPtr = hcclRetPtr + sizeof(uint16_t);
                HCCL_INFO("[ChkRetAndTaskexception] stopFlagPtr[0x%llx].", stopFlagPtr); //test
                HCCL_INFO("[ChkRetAndTaskexception] hcclRetPtr[0x%llx].", hcclRetPtr); //test
                auto ret = memcpy_s(hcclRetPtr, sizeof(uint16_t), &hcclRet, sizeof(uint16_t));
                if (ret != 0) {
                ...
                ret = memcpy_s(dstDataPtr, sizeof(DpuTaskexceptionInfo), &dpuTaskexceptionInfo, sizeof(DpuTaskexceptionInfo));
                if (ret != 0) {
                    HCCL_ERROR("[ChkRetAndTaskexception] memcpy dpuTaskexceptionInfo failed.");
                }
                HCCL_INFO("[ChkRetAndTaskexception] after cpy flag[%u].", *(reinterpret_cast<uint16_t *>(hcclRetPtr))); //test
