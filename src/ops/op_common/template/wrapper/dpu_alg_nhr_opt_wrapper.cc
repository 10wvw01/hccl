    ...
    u32 templateRankSize)
{
    ...
    // ====== Phase 1: Step 同步（sync 放在最前面）======
    // 对所有通信对端先批量 Record，再批量 Wait，减少串行阻塞窗口
    if (hasTx) {
    ...
    // ====== Phase 3: 直接使用底层原语执行数据传输 ======
    // （step sync 已替代 ACK 握手，Fence 替代 FIN_ACK 后同步，
    //   此处仅做 DATA 传输 + Fence）
        ...
        if (samePeer) {
            ...
            for (u32 i = 0; i < txSrcSlices.size(); i++) {
                ...
                CHK_RET(static_cast<HcclResult>(HcommWriteWithNotifyNbiOnThread(
                    0, txCh->handle, dst, src, txSrcSlices[i].size_, NOTIFY_IDX_DATA_SIGNAL)));
                CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(
                    0, rxCh->handle, NOTIFY_IDX_DATA_SIGNAL, STEP_SYNC_TIMEOUT)));
            }
            ...
            if (txSrcSlices.size() != 0) {
                for (u32 i = 0; i < txSrcSlices.size(); i++) {
                    ...
                    CHK_RET(static_cast<HcclResult>(HcommWriteWithNotifyNbiOnThread(
                        0, txCh->handle, dst, src, txSrcSlices[i].size_, NOTIFY_IDX_DATA_SIGNAL)));
                    CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(
                        0, txCh->handle, NOTIFY_IDX_DATA_SIGNAL, STEP_SYNC_TIMEOUT)));
                }
            }
            if (rxSrcSlices.size() != 0) {
                for (u32 i = 0; i < rxSrcSlices.size(); i++) {
                    CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(
                        0, rxCh->handle, NOTIFY_IDX_DATA_SIGNAL)));
                    CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(
                        0, rxCh->handle, NOTIFY_IDX_DATA_SIGNAL, STEP_SYNC_TIMEOUT)));
                }
            }
