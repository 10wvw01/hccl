...
 */
#include "scatter_omnipipe_data_slice_calc.h"
...
{
    HCCL_INFO("[CalcScatterScratchSize] start");
    ...
    HCCL_INFO(
        "[CalcScatterScratchSize] xRankSize=[%llu],yRankSize=[%llu],zRankSize=[%llu],", xRankSize, yRankSize, zRankSize);
    ...
    HCCL_INFO("[CalcScatterScratchSize] xTopo=[%llu],yTopo=[%llu],zTopo=[%llu],", xTopo, yTopo, zTopo);
    ...
    HCCL_INFO("[CalcScatterScratchSize] scratchSize[0]=[%llu],scratchSize[1]=[%llu],scratchSize[2]=[%llu]",
    ...
    HCCL_INFO("[CalcScatterScratchSize] end");
    return scratchSize;
}
...
void PushScatterZDiagSteps(std::vector<StepSliceInfo> &dataSliceLevelz, u64 zSDataSize[][MAX_STEP_NUM],
    ...
    u64 yRankSize, u64 zRankSize, u64 xAxis, u64 yAxis, u64 zCclBufferBaseOff, u64 zConnerStep,
...
void PushScatterZSameAxisSteps(std::vector<StepSliceInfo> &dataSliceLevelz, u64 zSDataSize[][MAX_STEP_NUM],
    u64 zSOffset[][MAX_STEP_NUM], const std::vector<OmniPipeSplitSliceInfo> &perLoop,
    const std::vector<OmniPipeSplitSliceInfo> &total, u64 dataTypeSize, u64 maxDataPieceId, u64 xRankSize,
    u64 yRankSize, u64 zRankSize, u64 xAxis, u64 yAxis, u64 zCclBufferBaseOff, u64 zConnerStep, u64 outerStepNum,
    const std::vector<std::vector<u64>> &xyzDataSizeStep)
{
    for (u64 osn = zConnerStep; osn < outerStepNum; osn++) {
        ...
        for (int oneDid = 0; oneDid < zRankSize; oneDid++) {
            ...
            u64 inputPieceIdOffset = sliceOffsetCut(zSOffset[maxDataPieceId][osn], perLoop[pieceId].size)
        ...
        if (outSliceNum != zAxis) {
            for (u64 connerDataSlice = 0; connerDataSlice < yRankSize; connerDataSlice++) {
                u64 currentInnerStepDataSliceId
                    = outSliceNum * xRankSize * yRankSize + connerDataSlice * xRankSize + oneDid;
...
void PushScatterXInnerSameAxisOneOsn(std::vector<StepSliceInfo> &dataSliceLevelx, u64 osn,
    ...
    u64 xSOffset[][MAX_STEP_NUM][MAX_STEP_NUM], const std::vector<OmniPipeSplitSliceInfo> &perLoop,
    const std::vector<OmniPipeSplitSliceInfo> &total, u64 dataTypeSize, u64 maxDataPieceId, u64 xRankSize,
    u64 yRankSize, u64 zRankSize, u64 yAxis, u64 zAxis, const std::vector<u64> &dataSizePerLoop,
    u64 xCclBufferBaseOff, u64 xInCornerStep, u64 innerStepNum)
{
        ...
        for (int oneDid = 0; oneDid < xRankSize; oneDid++) {
            ...
            for (u64 outSliceNum = 0; outSliceNum < zRankSize; outSliceNum++) {
                ...
                if (outSliceNum != zAxis) {
                    ...
                    outputslicestride = outSliceNum * dataSizePerLoop[maxDataPieceId]
                                        + xSOffset[maxDataPieceId][osn][isn];
                    sliceSizeMultRankPiece.push_back(sliceSizeOnePiece);
                    sliceCountMultRankPiece.push_back(sliceSizeOnePiece / dataTypeSize);
                    inputOmniPipeSliceStrideMultRankPiece.push_back(inputPieceIdOffset);
...
// scatter x轴外层 xB<=yB 斜对角段（单个osn，isn∈[0,xInCornerStep)）：root发斜对角数据，root数据放index=rootx，其余x轴rank塞0
        ...
        if (isn == innerStepNum - 1) {
            if (xSDataSize[root][osn][isn - 1] < ySDataSize[root][osn][isn - 2]) {
                inputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + ySOffset[root][osn][isn - 2]
                                                        + xSDataSize[root][osn][isn - 1], perLoop[pieceId].size)
                                     + perLoop[pieceId].offset;
                ...
                inputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + ySOffset[root][osn][isn - 2]
                                                        + ySDataSize[root][osn][isn - 2], perLoop[pieceId].size)
                                     + perLoop[pieceId].offset;
                outputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + ySOffset[root][osn][isn - 2]
                                                         + ySDataSize[root][osn][isn - 2], perLoop[pieceId].size)
                                      + perLoop[pieceId].offset;
            ...
            inputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + ySOffset[root][osn][isn - 1],
                                     perLoop[pieceId].size)
                                 + perLoop[pieceId].offset;
            outputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + ySOffset[root][osn][isn - 1],
                                      perLoop[pieceId].size)
                                  + perLoop[pieceId].offset;
        }
    }
            ...
            if (oneDid == rooty) {
                ...
                for (u64 connerDataSlice = 0; connerDataSlice < xRankSize; connerDataSlice++) {
                    if (connerDataSlice == rootx)
                        continue;
                    u64 currentDataSliceId = rootz * xRankSize * yRankSize + oneDid * xRankSize + connerDataSlice;
                    u64 pieceId = currentDataSliceId;
        ...
        for (int one = 0; one < yRankSize; one++) {
            if (one == rooty) {
                ...
                for (u64 connerDataSlice = 0; connerDataSlice < xRankSize; connerDataSlice++) {
                    if (connerDataSlice == rootx)
                        continue;
                    u64 currentDataSliceId = rootz * xRankSize * yRankSize + one * xRankSize + connerDataSlice;
                    u64 pieceId = currentDataSliceId;
...
void PushScatterYInnerSameAxisOneOsn(std::vector<StepSliceInfo> &dataSliceLevely, u64 osn,
    ...
    u64 ySOffset[][MAX_STEP_NUM][MAX_STEP_NUM], const std::vector<OmniPipeSplitSliceInfo> &perLoop,
    const std::vector<OmniPipeSplitSliceInfo> &total, u64 dataTypeSize, u64 maxDataPieceId, u64 xRankSize,
    u64 yRankSize, u64 zRankSize, u64 xAxis, u64 zAxis, const std::vector<u64> &dataSizePerLoop,
...
// scatter y轴外层 xB<=yB 斜对角段（单个osn，isn∈[0,yInCornerStep+1)）：root发斜对角数据，root数据放index=rootx，其余x轴rank塞0
...
// scatter y轴外层 xB>yB 斜对角段（单个osn，isn∈[0,yInCornerStep)）：第一步只有root发斜对角数据，buffer用xCclBufferBaseOff
        ...
        if (isn == innerStepNum - 1) {
            if (ySDataSize[root][osn][isn - 1] < xSDataSize[root][osn][isn - 2]) {
                inputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + xSOffset[root][osn][isn - 2]
                                                         + ySDataSize[root][osn][isn - 1], perLoop[pieceId].size)
                                     + perLoop[pieceId].offset;
                ...
                inputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + xSOffset[root][osn][isn - 2]
                                                         + xSDataSize[root][osn][isn - 2], perLoop[pieceId].size)
                                     + perLoop[pieceId].offset;
                outputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + xSOffset[root][osn][isn - 2]
                                                          + xSDataSize[root][osn][isn - 2], perLoop[pieceId].size)
                                      + perLoop[pieceId].offset;
            ...
            inputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + xSOffset[root][osn][isn - 1],
                                     perLoop[pieceId].size)
                                 + perLoop[pieceId].offset;
            outputPieceIdOffset = sliceOffsetCut(xySOffset[root][osn] + xSOffset[root][osn][isn - 1],
                                      perLoop[pieceId].size)
                                  + perLoop[pieceId].offset;
        }
    }
        ...
        for (int rankx = 0; rankx < xRankSize; rankx++) {
            if (rankx == rootx) {
                ...
                for (u64 connerDataSlice = 0; connerDataSlice < yRankSize; connerDataSlice++) {
                    if (connerDataSlice == rooty)
                        continue;
                    u64 currentDataSliceId = rootz * xRankSize * yRankSize + connerDataSlice * xRankSize + rankx;
                    u64 pieceId = currentDataSliceId;
...
OmniPipeSliceInfo CalcScatterOmniPipeSliceInfo(OmniPipeSliceParam &omniPipeSliceParam, uint32_t root)
{
    ...
    for (int rs = 0; rs < topo.rankSize; rs++) {
        ...
        if (ifroot || isSameAxis) {
            ...
            for (u64 i = 0; i < state.outerStepNum; i++) {
                ...
                state.innerStepNum = CalScatterDataSize2D(innerSlowDataSize, innerFastDataSize, innerSlowBw, innerFastBw,
                    innerSlowRankSize, innerFastRankSize, state.isZSlowAxis ? fastDataSize[i] : slowDataSize[i],
                    MAX_STEP_NUM);
            ...
            CalScatter2DOffset(slowOffset, fastOffset, state.outerStepNum, slowRankSize, fastRankSize, slowDataSize,
                fastDataSize);
        }
    }
    if (state.outerStepNum > finStepMark) {
        if (state.isZSlowAxis) {
            state.zConnerStep = 1;
            state.xyConnerStep = state.outerStepNum - finStepMark;
        } else {
            state.zConnerStep = state.outerStepNum - finStepMark;
            state.xyConnerStep = 1;
        }
    }
    std::vector<u64> levelRankSize = omniPipeSliceParam.levelRankSize;
    scratchSizexyz = CalcScatterScratchSize((u64 *)xSDataSize[topo.maxDataPieceId], (u64 *)ySDataSize[topo.maxDataPieceId],
        zSDataSize[topo.maxDataPieceId], levelRankSize, state.zConnerStep, state.outerStepNum, state.innerStepNum,
        MAX_STEP_NUM, omniPipeSliceParam.levelAlgType, omniPipeSliceParam.engine, topo.xB, topo.yB);
    xyzDataSizeStep = CalScatterDataSizeStep((u64 *)xSDataSize[topo.maxDataPieceId], (u64 *)ySDataSize[topo.maxDataPieceId],
        zSDataSize[topo.maxDataPieceId], levelRankSize, state.zConnerStep, state.outerStepNum, state.innerStepNum,
        MAX_STEP_NUM, topo.xB, topo.yB);
    ...
    HCCL_DEBUG("zConnerStep[%d] outerStepNum[%d] xyConnerStep[%d] xInCornerStep[%d] yInCornerStep[%d] innerStepNum[%d]",
        state.zConnerStep, state.outerStepNum, state.xyConnerStep, state.xInCornerStep, state.yInCornerStep,
        state.innerStepNum);
    ...
    PushScatterZDiagSteps(dataSliceLevelz, zSDataSize, zSOffset, omniPipeSplitSliceInfoListPerLoop,
        ...
        topo.zRankSize, topo.xAxis, topo.yAxis, zCclBufferBaseOff, state.zConnerStep, xyzDataSizeStep);
    PushScatterZSameAxisSteps(dataSliceLevelz, zSDataSize, zSOffset, omniPipeSplitSliceInfoListPerLoop,
        omniPipeSplitSliceInfoListTotal, topo.dataTypeSize, topo.maxDataPieceId, topo.xRankSize, topo.yRankSize,
        topo.zRankSize, topo.xAxis, topo.yAxis, zCclBufferBaseOff, state.zConnerStep, state.outerStepNum,
        xyzDataSizeStep);

    std::vector<StepSliceInfo> dataSliceLevelx;
    for (u64 osn = 0; osn < state.xyConnerStep; osn++) {
        PushScatterXInnerCornerOneOsn(dataSliceLevelx, osn, xSDataSize, xySOffset, xSOffset,
            ...
            topo.xRankSize, topo.yRankSize, topo.zRankSize, topo.yAxis, topo.zAxis, dataSizePerLoop, xCclBufferBaseOff,
            state.xInCornerStep);
    ...
    if (topo.xB <= topo.yB) {
        HCCL_DEBUG("xB <= yB");
        for (u64 osn = state.xyConnerStep; osn < state.outerStepNum; osn++) {
    ...
    for (u64 osn = 0; osn < state.xyConnerStep; osn++) {
        ...
        PushScatterYInnerSameAxisOneOsn(dataSliceLevely, osn, ySDataSize, xySOffset, ySOffset,
            omniPipeSplitSliceInfoListPerLoop, omniPipeSplitSliceInfoListTotal, topo.dataTypeSize, topo.maxDataPieceId,
            topo.xRankSize, topo.yRankSize, topo.zRankSize, topo.xAxis, topo.zAxis, dataSizePerLoop, yCclBufferBaseOff,
