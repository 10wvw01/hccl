...
HcclResult HcclOpbaseLaunchInOrderToOrderStream(HcclComm comm, OpParam &param, ThreadHandle unfoldThread, u32 notifyIdx, u32 timeout)
{
    ...
    HcclDedicatedThreadAcquire(comm, DED_THREAD_TYPE_AICPU_ORDER_LAUNCH_OPBASE, HOST_ORDER_THREAD_NOTIFY_NUM, &hostOrderThread);
    // 导出Host侧保序流
    HcclThreadExportToCommEngine(comm, 1, &hostOrderThread, COMM_ENGINE_AICPU_TS, exportHostOrderThread);
    param.exportHostOrderThread = exportHostOrderThread;

    // 获取Device侧保序流
    ThreadHandle deviceOrderThread;
    HcclDedicatedThreadAcquire(comm, DED_THREAD_TYPE_AICPU_ORDER_LAUNCH_DEVICE, DEVICE_ORDER_THREAD_NOTIFY_NUM, &deviceOrderThread);
    param.deviceOrderThread = deviceOrderThread;

    // notify0
    HcommThreadNotifyRecordOnThread(hostOrderThread, unfoldThread, notifyIdx);
...
HcclResult HcclOpbaseLaunchInOrderToKernelStream(HcclComm comm, u32 notifyIdx, u32 timeout)
{
    ...
    // wait notify1
    HcommThreadNotifyWaitOnThread(hostOrderThread, notifyIdx, timeout);
 ...
 * 执行流程：
 */
HcclResult HcclHcommLaunchInOrderToOrderStream(HcclComm comm, OpParam &param, ThreadHandle unfoldThread, u32 graphId, u32 notifyIdx, u32 timeout)
{
 ...
 * 执行流程：
 */
HcclResult HcclHcommLaunchInOrderToKernelStream(HcclComm comm, u32 graphId, u32 notifyIdx, u32 timeout)
{
