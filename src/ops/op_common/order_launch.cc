...
HcclResult HcclAclgraphLaunchInOrderToOrderStream(HcclComm comm, OpParam &param, ThreadHandle unfoldThread, u32 notifyIdx, u32 timeout, HcclRtEvent event)
{
    ...
    // TODO：thread--->stream[host侧保序流&展开流]
    // HcommThreadResGetInfo(ThreadHandle thread, ThreadResType resType, uint32_t infoLen, void **info);
    // Note:GetUnfoldStream()----WARING    然后ret参照上面的
