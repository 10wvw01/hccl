...
// algo 模块根据 priorityOrder 过滤掉不会选择到的引擎算法
__attribute__((weak)) HcclResult FilterCmByHcclAlgo(CostModel &cm, const std::vector<OpExecuteConfig> &priorityOrder)
{
    (void)cm;
    (void)priorityOrder;
                                                               ...
                                                               OpExecuteConfig opExecuteConfig)
{
    ...
    switch (opExecuteConfig) {
        ...
        case OpExecuteConfig::CCU_SCHED:
            // CCU_SCHED → AIV → AICPU 回退链(不回退 CCU_MS)
            return {OpExecuteConfig::CCU_SCHED,
                    OpExecuteConfig::AIV,
                    OpExecuteConfig::AICPU_TS};
...
HcclResult SelectorEngine::GetOrInitCostModel(HcclComm comm, const std::vector<OpExecuteConfig> &priorityOrder,
                                              CostModel *&cm)
{
    ...
    // 调用 algo 模块过滤 costModel(传入 priorityOrder,过滤不会选择到的引擎算法)
    CHK_RET(FilterCmByHcclAlgo(mutableCm, priorityOrder));
                               ...
                               std::string &algName)
{
    ...
    // step 0: 计算引擎优先级(正常流程用 param.opExecuteConfig;回退流程 ReSelector 已设 AICPU_TS)
    std::vector<OpExecuteConfig> priorityOrder = GetEnginePriority(topoInfo, param.opExecuteConfig);

    // step 1: 从通信域 ctx 获取或初始化 costModel(传 priorityOrder 给 algo 模块过滤)
    CostModel *cm = nullptr;
    CHK_RET(GetOrInitCostModel(comm, priorityOrder, cm));
    ...
    // step 3: min(ct)
    HcclResult ret = SelectMinCost(ct, priorityOrder, param, algName);
                                                   ...
                                                   CostTable &ct, CostModel &cm)
{
    ...
    // step 2.2: tuner(外部模块,优先级最高,可覆盖 costTable)
    CHK_RET(Tuner(ct, param));

    return HCCL_SUCCESS;
}

HcclResult SelectorEngine::SelectMinCost(const CostTable &ct, const std::vector<OpExecuteConfig> &priorityOrder,
                                         OpParam &param, std::string &algName)
{
    for (OpExecuteConfig engine : priorityOrder) {
        int minIdx = -1;
        float minCost = 0.0f;
