...
private:
    ...
    HcclResult GetOrInitCostModel(HcclComm comm, const std::vector<OpExecuteConfig> &candidateEngines, CostModel *&cm);

    HcclResult GenAndProcessCostTable(OpParam &param, TopoInfoWithNetLayerDetails *topoInfo,
                                      CostTable &ct, CostModel &cm);

    HcclResult SelectMinCost(const CostTable &ct, const std::vector<OpExecuteConfig> &candidateEngines,
