    ...
    const std::vector<OmniPipeSplitSliceInfo> &omniPipeSplitSliceInfoListPerLoop)
{
    ...
    // TODO
    // 多 rank 数据量不一致场景：push 阶段所有 rank 共用 maxDataPieceId 的 step size/offset，
    // 若各 rank 按自己的 perLoop.size 计算 step，较小 rank 的 step 与 maxDataPieceId 不一致，
    // DataSliceCut 截断后数据量不足。此处统一用所有 rank 中最大的 perLoop.size 计算 step。
    // u64 maxPerLoopSize = 0;
    // for (size_t i = 0; i < omniPipeSplitSliceInfoListPerLoop.size(); i++) {
    //     if (omniPipeSplitSliceInfoListPerLoop[i].size > maxPerLoopSize) {
    //         maxPerLoopSize = omniPipeSplitSliceInfoListPerLoop[i].size;
    //     }
    // }
    // std::vector<OmniPipeSplitSliceInfo> unifiedPerLoop = omniPipeSplitSliceInfoListPerLoop;
    // for (size_t i = 0; i < unifiedPerLoop.size(); i++) {
    //     if (unifiedPerLoop[i].size < maxPerLoopSize) {
    //         unifiedPerLoop[i].size = maxPerLoopSize;
    //         unifiedPerLoop[i].count = maxPerLoopSize / topo.dataTypeSize;
    //     }
    // }
    ...
    for (u64 rs = 0; rs < topo.rankSize; rs++) {
        ...
        if (ifroot || isSameAxis) {
            CalcScatterOneRankDataSize(topo, state, rs, finStepMark, slowBw, fastBw, slowRankSize, fastRankSize,
                ...
                omniPipeSplitSliceInfoListPerLoop); // unifiedPerLoop omniPipeSplitSliceInfoListPerLoop
        }
    }
...
OmniPipeSliceInfo CalcScatterOmniPipeSliceInfo(OmniPipeSliceParam &omniPipeSliceParam, uint32_t root)
{
    ...
    // 尾块适配
    // if (omniPipeSliceParam.loopStartCount.size() == omniPipeSplitSliceInfoListTotal.size()) {
    //     for (size_t i = 0; i < omniPipeSplitSliceInfoListTotal.size(); i++) {
    //         omniPipeSplitSliceInfoListTotal[i].offset += omniPipeSliceParam.loopStartCount[i] * topo.dataTypeSize;
    //     }
    // }
