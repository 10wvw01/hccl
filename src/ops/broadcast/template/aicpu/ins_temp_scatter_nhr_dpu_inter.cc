...
HcclResult InsTempScatterNHRDPUInter::GetStepInfo(u32 step, u32 nSteps, AicpuNHRStepInfo &stepInfo)
{
    ...
    if (deltaRoot < nRanks) {  // 需要发
        ...
        HCCL_INFO("[InsTempScatterNHRDPUInter][GetStepInfo] rankSize[%zu], myAlgRank[%u], sendTo Idx[%u]", subCommRanks_[0].size(), myAlgRank, sendTo);
        ...
        HCCL_INFO("[InsTempScatterNHRDPUInter][GetStepInfo] rankSize[%zu], myAlgRank[%u], recvFrom Idx[%u]", subCommRanks_[0].size(), myAlgRank, recvFrom);
    ...
    TemplateResource& templateResource)
{
    ...
    HCCL_INFO("[InsTempScatterNHRDPUInter] queNum_ =  [%u], threads size = [%zu]", threadNum_, templateResource.threads.size());
    ...
    const TemplateResource& templateResource)
{
    ...
    for (uint64_t rpt = 0; rpt < tempAlgParams.repeatNum; ++rpt) {
        ...
        HCCL_INFO("[InsTempScatterNHRDPUInter][LocalCopy] LocalDataCopy RankID [%u] algRankIdx [%u] "
    ...
    const TemplateResource& templateResource)
{
    ...
    for (u32 rpt = 0; rpt < tempAlgParams.repeatNum; ++rpt) {
        ...
        for (auto rank : subCommRanks_[0]) {
            ...
            HCCL_INFO("[InsTempScatterNHRDPUInter][LocalCopy] LocalDataCopy RankID [%u] dataRank [%u] dataAlgRank[%u] "
