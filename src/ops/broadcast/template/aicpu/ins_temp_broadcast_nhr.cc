    ...
    const std::map<u32, std::vector<ChannelInfo>> &channels, const std::vector<ThreadHandle> &threads, u32 channelIdx)
{
    ...
    for (u32 step = 0; step < nSteps; step++) {
        ...
        CHK_PRT_RET(BatchTxRx(stepInfo, channels, threads, sliceInfoVec, channelIdx),
    ...
    const std::map<u32, std::vector<ChannelInfo>> &channels, const std::vector<ThreadHandle> &threads, u32 channelIdx)
{
    ...
    for (u32 step = 0; step < nSteps; step++) {
        ...
        for (u32 i = 0; i < stepInfo.nSlices; i++) {
            ...
            u64 txPartialOffset = (txIdx == templateRankSize_ - 1) ? dataOffsetTail_[channelIdx] : dataOffset_[channelIdx];
            u64 txPartialSize = (txIdx == templateRankSize_ - 1) ? dataSplitTail_[channelIdx] : dataSplit_[channelIdx];
            u64 rxPartialOffset = (rxIdx == templateRankSize_ - 1) ? dataOffsetTail_[channelIdx] : dataOffset_[channelIdx];
            u64 rxPartialSize = (rxIdx == templateRankSize_ - 1) ? dataSplitTail_[channelIdx] : dataSplit_[channelIdx];
            u64 txOffset   = sliceInfoVec[txIdx][0].offset + txPartialOffset + memOffset;
            u64 txSize     = txPartialSize;
            u64 rxOffset   = sliceInfoVec[rxIdx][0].offset + rxPartialOffset + memOffset;
            u64 rxSize     = rxPartialSize;
...
HcclResult InsTempBroadcastNHR::BatchTxRx(AicpuNHRStepInfo &stepInfo, const std::map<u32, std::vector<ChannelInfo>> &channels, const std::vector<ThreadHandle> &threads,
    const RankSliceInfo &sliceInfoVec, u32 channelIdx)
{
    ...
    if (stepInfo.txSliceIdxs.size() > 0 && stepInfo.rxSliceIdxs.size() == 0) {
        CHK_RET(BatchSend(stepInfo, channels, threads, sliceInfoVec, memOffset, channelIdx));
    }
    // 只有Rx，使用recv指令
    else if (stepInfo.txSliceIdxs.size() == 0 && stepInfo.rxSliceIdxs.size() > 0) {
        CHK_RET(BatchRecv(stepInfo, channels, threads, sliceInfoVec, memOffset, channelIdx));
    }
    // 既有Tx又有Rx，使用SendRecv指令
    else if (stepInfo.txSliceIdxs.size() > 0 && stepInfo.rxSliceIdxs.size() > 0) {
        CHK_RET(BatchSR(stepInfo, channels, threads, sliceInfoVec, memOffset, channelIdx));
    }
    return HcclResult::HCCL_SUCCESS;
}
...
HcclResult InsTempBroadcastNHR::BatchSend(AicpuNHRStepInfo &stepInfo, const std::map<u32, std::vector<ChannelInfo>> &channels, const std::vector<ThreadHandle> &threads,
    const RankSliceInfo &sliceInfoVec, u64 memOffset, u32 channelIdx) const
{
    u32 toRankKey = GetRankFromMap(stepInfo.toRank);
    auto itSend = channels.find(toRankKey);
    ...
    for (u32 i = 0; i < stepInfo.txSliceIdxs.size(); i++) {
        ...
        DataSlice txSrcSlice = DataSlice(srcBuffAddr, memOffset + sliceInfoVec[txId][0].offset + partialOffset, partialSize);
        DataSlice txDstSlice = DataSlice(remoteBuffAddr, memOffset + sliceInfoVec[txId][0].offset + partialOffset, partialSize);
        txSrcSlices.push_back(txSrcSlice);
        txDstSlices.push_back(txDstSlice);
    }
    ...
    const RankSliceInfo &sliceInfoVec, u64 memOffset, u32 channelIdx) const
{
    ...
    for (u32 i = 0; i < stepInfo.rxSliceIdxs.size(); i++) {
        ...
        DataSlice rxSrcSlice = DataSlice(remoteBuffAddr, memOffset + sliceInfoVec[rxId][0].offset + partialOffset, partialSize);
        DataSlice rxDstSlice = DataSlice(BuffAddr, memOffset + sliceInfoVec[rxId][0].offset + partialOffset, partialSize);
        rxSrcSlices.push_back(rxSrcSlice);
        rxDstSlices.push_back(rxDstSlice);
    }
    ...
    const RankSliceInfo &sliceInfoVec, u64 memOffset, u32 channelIdx) const
{
    ...
    for (u32 i = 0; i < stepInfo.txSliceIdxs.size(); i++) {
        ...
        void* BuffAddr = (!enableRemoteMemAccess_) ? buffInfo_.hcclBuff.addr : buffInfo_.inputPtr;
        DataSlice txSrcSlice = DataSlice(BuffAddr, memOffset + sliceInfoVec[txId][0].offset + partialOffset, partialSize);
        DataSlice txDstSlice = DataSlice(remoteSendBuffAddr, memOffset + sliceInfoVec[txId][0].offset + partialOffset, partialSize);
        txSrcSlices.push_back(txSrcSlice);
        txDstSlices.push_back(txDstSlice);
    }
    ...
    for (u32 i = 0; i < stepInfo.rxSliceIdxs.size(); i++) {
        ...
        void* BuffAddr = (!enableRemoteMemAccess_) ? buffInfo_.hcclBuff.addr : buffInfo_.inputPtr;
        DataSlice rxSrcSlice = DataSlice(remoteRecvBuffAddr, memOffset + sliceInfoVec[rxId][0].offset + partialOffset, partialSize);
        DataSlice rxDstSlice = DataSlice(BuffAddr, memOffset + sliceInfoVec[rxId][0].offset + partialOffset, partialSize);
        rxSrcSlices.push_back(rxSrcSlice);              
        rxDstSlices.push_back(rxDstSlice);
    }
...
HcclResult InsTempBroadcastNHR::PrepareDataSplitForMultiChannel(const TemplateResource &templateResource)
{
    ...
    u64 dataSize = tempAlgParams_.sliceSize;
    u64 chunkSize = RoundUp(dataSize, (templateRankSize_ * dataTypeSize_)) * dataTypeSize_;
    std::vector<u64> elemCountOut;
    u64 totalDataCount = chunkSize / dataTypeSize_;
    ...
    u64 lastRankChunkSize = dataSize - chunkSize * (templateRankSize_ - 1);
    if (lastRankChunkSize > chunkSize) {
        lastRankChunkSize = chunkSize;
    }
    if (lastRankChunkSize != chunkSize && lastRankChunkSize > 0) {
        std::vector<u64> elemCountOutTail;
        u64 totalDataCountTail = lastRankChunkSize / dataTypeSize_;
                                          ...
                                          TemplateResource& templateResource)
{
    ...
    for (u32 channelIdx = 0; channelIdx < channelsPerRank_; channelIdx++) {
        CHK_RET(RunScatter(sliceInfoVec, templateResource.channels, templateResource.threads, channelIdx));
        CHK_RET(RunAllGather(sliceInfoVec, templateResource.channels, templateResource.threads, channelIdx));
    }
    if (threadNum_ > 1) {
        std::vector<ThreadHandle> subThreads(templateResource.threads.begin() + 1,
                                             templateResource.threads.begin() + threadNum_);
