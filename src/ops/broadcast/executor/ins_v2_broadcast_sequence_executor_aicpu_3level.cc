    ...
    InsAlgTemplate3, InsAlgTemplate4, InsAlgTemplate5>::OrchestrateLoop(const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    algTemplateScatterL1->SetRoot((param.root / rankSizeLevel0_) * rankSizeLevel0_ + (myRank_ % rankSizeLevel0_));
    ...
    algTemplateScatterL2->SetRoot(param.root / podSize * podSize + myRank_ % podSize);
    ...
    CHK_RET(GenTempResource(resCtx, 0, algTemplateScatterL1, templateResourceL1));
    ...
    CHK_RET(GenTempResource(resCtx, 0, algTemplateScatterL2, templateResourceL2));
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        if (tempAlgParamsScatterL1.count != 0) {
            if (myRank_ >= rootPodStartRank && myRank_ <= rootPodEndRank) {
...
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_BROADCAST,
    ...
    TopoMatchMultilevel,
    InsTempAllGatherMesh1D1DZAxisDetour,      // Scatter L0 (框内)
    ...
    AicpuTempScatterMesh1DZAxisDetour);  // AllGather L0 (框内, Z 轴绕路)
}  // namespace ops_hccl