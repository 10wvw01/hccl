...
HcclResult InsV2BroadcastSoleExecutor<AlgTopoMatch, InsAlgTemplate>::OrchestrateLoop(const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    if (param.engine == CommEngine::COMM_ENGINE_AICPU_TS && std::string(param.algName) == "InsBroadcastNHRMultiChannel") {
...
REGISTER_EXEC_V2(HcclCMDType::HCCL_CMD_BROADCAST, InsBroadcastNHR, InsV2BroadcastSoleExecutor, TopoMatch1D,
                InsTempBroadcastNHR);
REGISTER_EXEC_V2(HcclCMDType::HCCL_CMD_BROADCAST, InsBroadcastNHRMultiChannel, InsV2BroadcastSoleExecutor, TopoMatch1D,
                InsTempBroadcastNHR);
