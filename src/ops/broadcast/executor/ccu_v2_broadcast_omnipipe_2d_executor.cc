            ...
            const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    u64 maxCountPerLoop = std::min(transportBoundDataSize, maxTmpMemSize_) / HCCL_MIN_SLICE_ALIGN
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        if (myRank_ != param.root) {                        
            ...
            tempAlgParamLocalCopy.buffInfo.inBuffBaseOff = processedDataCount * dataTypeSize_; // TODO 从这个地方考虑 ccubuffer切分多块  》myRank_ * allRankSplitData[0] * dataTypeSize_ + 
            tempAlgParamLocalCopy.buffInfo.outBuffBaseOff = processedDataCount * dataTypeSize_; // TODO 检查下
