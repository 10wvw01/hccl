...
#include <execinfo.h>
#include "ccu_v2_broadcast_omnipipe_2d_executor.h"
...
CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::CcuV2BroadcastOmniPipeExecutor()
{
}

template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::CalcAlgHierarchyInfo(HcclComm comm, 
...
#elif T_DESC("2x3用例", false)
	HCCL_DEBUG("[%s] 2x3-TestCase", __func__);
	if (userrank == 0 || userrank == 1 ) {
...
#elif T_DESC("3x2用例", false)
	HCCL_DEBUG("[%s] 3x2-TestCase", __func__);
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::InitCommInfo(
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::CalcResLevel(
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::InitSubCommRanks(
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::CalcRes(
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::Orchestrate(
    const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    HCCL_INFO("[CcuV2BroadcastOmniPipeExecutor][Orchestrate] Orchestrate Start");
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[CcuV2BroadcastOmniPipeExecutor][Orchestrate]errNo[0x%016llx] excutor kernel run failed",
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::InitTemplate(
            ...
            const std::vector<std::vector<u32>>& subCommRanks0, const std::vector<std::vector<u32>>& subCommRanks1)
{
    ...
    HCCL_DEBUG("[CcuV2BroadcastOmniPipeExecutor][%s] tempMap.size:%u", __func__, tempMap.size());
    
    HCCL_DEBUG("[CcuV2BroadcastOmniPipeExecutor][%s] threads_.size:%d", __func__, threads_.size());
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::GenTempAlgParamsIn2HCCLBuff(
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::GenTempAlgParamsHCCLBuff2HCCLBuff(
	TemplateDataParams &tempAlgParams, StepSliceInfo &stepSliceInfo, u64 processedDataCount,
	const AlgResourceCtxSerializable &resCtx, const OpParam &param)
{
	tempAlgParams.count = 0;
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::InitOmniPipeScratchParam(
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::InitOmniPipeSliceParam(
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::GenTemplateAlgParamsByDimData(
            TemplateDataParams &tempAlgParams, StepSliceInfo &stepSliceInfo, u64 processedDataCount)
{
    tempAlgParams.count = 0;
    tempAlgParams.stepSliceInfo = stepSliceInfo;
...
template <typename AlgTopoMatch, typename CcuScatterAlgTemplateX, typename CcuScatterAlgTemplateY, typename CcuAgAlgTemplateX, typename CcuAgAlgTemplateY>
HcclResult CcuV2BroadcastOmniPipeExecutor<AlgTopoMatch, CcuScatterAlgTemplateX, CcuScatterAlgTemplateY, CcuAgAlgTemplateX, CcuAgAlgTemplateY>::OrchestrateLoop(
            const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    // TOOD 设置下offsetStart、设置下offsetEnd
    ...
    HCCL_INFO("[%s] myRank[%u] transportBoundDataSize[%u] scratchBoundDataSize[%u]", __func__, myRank_, transportBoundDataSize, scratchBoundDataSize);
    ...
    HCCL_DEBUG("[%s] zjq>>> maxCountPerLoop[%u], loopTimes[%u]", __func__, maxCountPerLoop, loopTimes);
#else
    ...
    OmniPipeSliceInfo alignSliceInfoSC = CalcScatterOmniPipeSliceInfo(sliceParam, param.root);
    OmniPipeSliceInfo alignSliceInfoAG = CalcAGOmniPipeSliceInfo(sliceParam);
    HCCL_INFO("[jjy]alignSliceInfoSC.dataSliceLevel0.size()=%d", alignSliceInfoSC.dataSliceLevel0.size());
 
    // 3.2 计算第n次的loop的slice信息
    OmniPipeSliceInfo tailSliceInfoSC;
    ...
    if (allRankSplitData[0] % maxCountPerLoop != 0) {
        ...
        tailSliceInfoSC = CalcScatterOmniPipeSliceInfo(sliceParam, param.root);
        tailSliceInfoAG = CalcAGOmniPipeSliceInfo(sliceParam);
    }
 
    for(int i=0;i<alignSliceInfoSC.dataSliceLevel0.size();i++){
        for(int j=0;j<alignSliceInfoSC.dataSliceLevel0[i].inputOmniPipeSliceStride.size();j++){
            for(int k=0;k<alignSliceInfoSC.dataSliceLevel0[i].inputOmniPipeSliceStride[j].size();k++){
                HCCL_INFO("[jjy][dataSliceLevel0]myRank[%d][inputOmniPipeSliceStride][%d][%d][%d]:%d",myRank_,i,j,k,alignSliceInfoSC.dataSliceLevel0[i].inputOmniPipeSliceStride[j][k]);
            }
        } 
    }
    for(int i=0;i<alignSliceInfoSC.dataSliceLevel1.size();i++){
        for(int j=0;j<alignSliceInfoSC.dataSliceLevel1[i].inputOmniPipeSliceStride.size();j++){
            for(int k=0;k<alignSliceInfoSC.dataSliceLevel1[i].inputOmniPipeSliceStride[j].size();k++){
                HCCL_INFO("[jjy][dataSliceLevel1]myRank[%d][inputOmniPipeSliceStride][%d][%d][%d]:%d",myRank_,i,j,k,alignSliceInfoSC.dataSliceLevel1[i].inputOmniPipeSliceStride[j][k]);
            }
        } 
    }
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {//loopTimes
        CHK_PRT_RET(
            ...
            HCCL_ERROR("[CcuV2BroadcastOmniPipeExecutor][Orchestrate] multiLoopAllRankSplitData.size() <= loop"),
            HCCL_E_PARA);
 
        // 4.1 确定当前是前n-1次loop的slice结果，还是存在尾块时最后一次loop的slice结果
        if (loop == loopTimes - 1 && !tailSliceInfoSC.isEmpty()) {
            omniPipeSliceInfoSC = tailSliceInfoSC;
            omniPipeSliceInfoAG = tailSliceInfoAG;
        } else {
            omniPipeSliceInfoSC = alignSliceInfoSC;
        ...
        // for (int i=0;i<multiLoopAllRankSplitData.size();i++){
        //     for(int j=0;j<multiLoopAllRankSplitData[i].size();j++){
        //         HCCL_INFO("rankId[%d], multiLoopAllRankSplitData[%d][%d]:%d", myRank_, i, j, multiLoopAllRankSplitData[i][j]);
        //     }
        // }
        ...
        // 4.2 RS的通信步数
        auto level0StepCountRS = omniPipeSliceInfoSC.dataSliceLevel0.size();
        ...
        for (auto i = 0; i < level0StepCountRS; ++i) {
            ...
            if (i == level0StepCountRS - 1) {
				...
				if (isSameXAxisAsRoot) {
                    ...
                    scatterAlgTempY.SetRoot(myRank_);
			...
			// 中间步骤：逐步转发阶段。- root节点：快轴发对角数据，慢轴发同轴数据  - 同x轴非root节点：不发送任何数据（等待接收）同y轴非root节点：往x轴方向发送部分转发数据
			else if(i != 0 && i != level0StepCountRS - 1) {
				HCCL_DEBUG("[%s] myRank[%u] StepNum[%u]", __func__, myRank_, i);
			...
			scatterAlgTempX.isLastStep_ = (i == level0StepCountRS - 1);
            templateResourceScatterX.threads.clear();
            templateResourceScatterX.threads.emplace_back(threads_[0]);
			CHK_RET(scatterAlgTempX.KernelRun(param, tempScatterAlgParamsX, templateResourceScatterX));
			// // 执行Y维度通信
			scatterAlgTempY.isStepOne_ = (i == 0);
			scatterAlgTempY.isLastStep_ = (i == level0StepCountRS - 1);
            scatterAlgTempY.xRankSize_ = rankSizeLevel0_;
            templateResourceScatterY.threads.clear();
            templateResourceScatterY.threads.emplace_back(threads_[1]);
        ...
        if (myRank_ != param.root) {                        
            ...
            templateResourceScatterX.threads.clear();
            templateResourceScatterX.threads.emplace_back(threads_[1]);
        ...
        for (u32 i = 0; i < level0StepCountAG; i++) {
            ...
            // 执行机内template任务
            templateResourceAgX.threads.clear();
            templateResourceAgX.threads.emplace_back(threads_[1]);
            CHK_RET(agAlgTempX.KernelRun(param, tempAgAlgParamsX, templateResourceAgX));
            templateResourceAgY.threads.clear();
            templateResourceAgY.threads.emplace_back(threads_[0]);
...
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_BROADCAST, CcuBroadcastOmniPipe2D,
                                CcuV2BroadcastOmniPipeExecutor, TopoMatchUBX, 
