            ...
            const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    u64 scratchBoundDataSize = maxTmpMemSize_/ HCCL_MIN_SLICE_ALIGN * HCCL_MIN_SLICE_ALIGN / rankSize_;
    ...
    u64 maxCountPerLoop = std::min(transportBoundDataSize, scratchBoundDataSize) / dataTypeSize_; // TODO：优化下，分别计算每张卡 两个值的大小，再取最小值
    ...
    perLoopSize = dataSize_ > perLoopSize ? perLoopSize : dataSize_;
    HCCL_INFO("[%s] perLoopSize[%u] dataSize_[%u]", __func__, perLoopSize, dataSize_);
    ...
    // std::vector<u64> dataWholeSize(rankSize_, dataSize_); // TODO: allRankSplitData[myRank_] * dataTypeSize_  用输入的
    std::vector<u64> dataWholeSize(rankSize_, allRankSplitData[myRank_] * dataTypeSize_);
    ...
    // HCCL_DEBUG("[jjy]alignSliceInfoSC.dataSliceLevel0.size()=%d", alignSliceInfoSC.dataSliceLevel0.size());
    ...
    //     // HCCL_INFO("[%s] myRank[%u] tailLoopSize[%u]", __func__, myRank_, tailLoopSize);
    //     // std::vector<u64> dataSizePerLoop(rankSize_, tailLoopSize);
    //     // sliceParam.dataSizePerLoop = dataSizePerLoop;
    //     // tailSliceInfoSC = CalcScatterOmniPipeSliceInfo(sliceParam, param.root);
    //     // tailSliceInfoAG = CalcAGOmniPipeSliceInfo(sliceParam);
    // }
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        // if (loop == loopTimes - 1 && dataCount_ > maxCountPerLoop && dataCount_ % maxCountPerLoop != 0) {
    ...
    // }
        ...
        if (myRank_ != param.root) { // TODO 这块的搬运细节需要验证下                        
            ...
            tempAlgParamLocalCopy.buffInfo.hcclBuff = resCtx.cclMem; // 用不到
            tempAlgParamLocalCopy.count = currDataCount;
            tempAlgParamLocalCopy.sliceSize = currDataCount * dataTypeSize_;
            tempAlgParamLocalCopy.buffInfo.inBuffBaseOff =  multiLoopAllRankSplitData[loop][myRank_] * myRank_ * dataTypeSize_; // TODO 调测下 》》》 myRank_ * perLoopSize; // TODO 从这个地方考虑 ccubuffer切分多块  》myRank_ * allRankSplitData[0] * dataTypeSize_ + 
...
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_BROADCAST, CcuBroadcastOmniPipe2D,
                                ...
                                CcuTempAllGatherOmniPipeMesh1DMem2Mem, CcuTempAllGatherOmniPipeNHR1DMem2Mem); // CcuTempAllGatherOmniPipeNHR1DMem2Mem
} // namespace ops_hccl