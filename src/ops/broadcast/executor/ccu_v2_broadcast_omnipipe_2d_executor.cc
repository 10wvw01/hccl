...
{
    HCCL_DEBUG("[CcuV2BroadcastOmniPipeExecutor][Orchestrate] Orchestrate Start");
            ...
            const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    const char* scatterBwRatioXEnv = std::getenv("OMNIPIPE_BW_X_SC");
    const char* scatterBwRatioYEnv = std::getenv("OMNIPIPE_BW_Y_SC");
    const char* allGatherBwRatioXEnv = std::getenv("OMNIPIPE_BW_X_AG");
    const char* allGatherBwRatioYEnv = std::getenv("OMNIPIPE_BW_Y_AG");
    double bwXSC = std::stod(std::string(scatterBwRatioXEnv));
    double bwYSC = std::stod(std::string(scatterBwRatioYEnv));
    double bwXAG = std::stod(std::string(allGatherBwRatioXEnv));
    double bwYAG = std::stod(std::string(allGatherBwRatioYEnv));
    HCCL_DEBUG("[%s] rankId[%d], bwXSC[%f]", __func__, myRank_, bwXSC);
    HCCL_DEBUG("[%s] rankId[%d], bwYSC[%f]", __func__, myRank_, bwYSC);
    HCCL_DEBUG("[%s] rankId[%d], bwXAG[%f]", __func__, myRank_, bwXAG);
    HCCL_DEBUG("[%s] rankId[%d], bwYAG[%f]", __func__, myRank_, bwYAG);

    double eqBwLevel0SC = BW_OMNI_DEFAULT; // 50
    double eqBwLevel1SC = BW_OMNI_DEFAULT; // 50
    double eqBwLevel0AG = BW_OMNI_UBX_CCU_SCHED_AG_MESH; // 47
    double eqBwLevel1AG = BW_OMNI_UBX_CCU_SCHED_AG_CLOS; // 180
    eqBwLevel0SC = bwXSC;
    eqBwLevel1SC = rankSizeLevel1_ > 1 ? bwYSC / (rankSizeLevel1_ - 1) : bwYSC;
    eqBwLevel0AG = bwXAG;
    eqBwLevel1AG = rankSizeLevel1_ > 1 ? bwYAG / (rankSizeLevel1_ - 1) : bwYAG;
    HCCL_DEBUG("[%s] rankId[%d], eqBwLevel0SC[%f]", __func__, myRank_, eqBwLevel0SC);
    HCCL_DEBUG("[%s] rankId[%d], eqBwLevel1SC[%f]", __func__, myRank_, eqBwLevel1SC);
    HCCL_DEBUG("[%s] rankId[%d], eqBwLevel0AG[%f]", __func__, myRank_, eqBwLevel0AG);
    HCCL_DEBUG("[%s] rankId[%d], eqBwLevel1AG[%f]", __func__, myRank_, eqBwLevel1AG);
    std::vector<double> endpointAttrBwAvgSC = {eqBwLevel0SC, eqBwLevel1SC, 1.0};
    ...
    for (int i=0;i<allRankSplitData.size();i++){
        HCCL_DEBUG("[jjy]rankId[%d], allRankSplitData[%d]:%d", myRank_, i, allRankSplitData[i]);
    }
    ...
    for (int i=0;i<multiLoopAllRankSplitData.size();i++){
        for(int j=0;j<multiLoopAllRankSplitData[i].size();j++){
            HCCL_DEBUG("[jjy]rankId[%d], multiLoopAllRankSplitData[%d][%d]:%d", myRank_, i, j, multiLoopAllRankSplitData[i][j]);
        }
    }
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        if (myRank_ != param.root) { // TODO 这块的搬运细节需要验证下                        
            CHK_RET(PreSyncInterThreads(mainThread, syncThreads, notifyIdxesMainToSub));
            ...
            tempAlgParamLocalCopy.buffInfo.inputSize = resCtx.cclMem.size; // 使用到的输入是cclbuffer
            ...
            std::vector<u64> perRankOffset;
 	        ...
 	        for (u32 i = 1; i < rankSize_; i++) {
                ...
                HCCL_DEBUG("[%s] myRank[%u] perRankOffset[%u]", __func__, myRank_, perRankOffset[i]);
 	        }
            ...
            std::vector<u64> perInnnerLoopOffset;
 	        ...
 	        for (u32 i = 1; i < rankSize_; i++) {
 	            perInnnerLoopOffset[i] = perInnnerLoopOffset[i - 1]  + multiLoopAllRankSplitData[loop][i -1]; // + allRankSplitData[i - 1]
                HCCL_DEBUG("[%s] myRank[%u] perInnnerLoopOffset[%u]", __func__, myRank_, perInnnerLoopOffset[i]);
 	        }
            ...
            // multiLoopAllRankSplitData[loop][myRank_] * myRank_ * dataTypeSize_;
...
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_BROADCAST, CcuBroadcastOmniPipe2D,
                                ...
                                CcuTempScatterOmniPipeMesh1DMem2Mem, CcuTempScatterOmniPipeMesh1DMem2Mem,  // CcuTempScatterOmniPipeNHR1DMem2Mem
