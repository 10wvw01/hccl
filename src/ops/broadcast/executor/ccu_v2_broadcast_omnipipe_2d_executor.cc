    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        for (auto i = 0; i < level0StepCountSC; ++i) {
            ...
            if (i == level0StepCountSC - 1) {
				...
				// 同快轴(y轴)的非root节点：沿x轴方向发送转发数据（使用mesh算法,走templateX）；数据流向：hcclbuff --> hcclbuff
			...
			// 中间步骤：逐步转发阶段。- root节点：快轴发对角数据，慢轴发同轴数据  - 同x轴非root节点：不发送任何数据（等待接收）同y轴非root节点：往x轴方向发送部分转发数据
				...
				HCCL_DEBUG("[%s] myRank[%u] StepNum[%u]", __func__, myRank_, i);
				// 同快轴(y轴)的非root节点：往x轴方向发送部分转发数据（使用mesh算法,走templateX）；数据流向：hcclbuff --> hcclbuff
				if (isSameYAxisAsRoot && rankSizeLevel0_ > 1) {
					HCCL_DEBUG("[%s] set myRank[%u] as root", __func__, myRank_);
                    scatterAlgTempX.SetRoot(myRank_);
					CHK_RET(GenTempAlgParamsHCCLBuff2HCCLBuff(
...
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_BROADCAST, CcuBroadcastOmniPipe2D,
                                ...
                                CcuTempAllGatherOmniPipeMesh1DMem2Mem, CcuTempAllGatherOmniPipeMesh1DMem2Mem); // CcuTempAllGatherOmniPipeNHR1DMem2Mem
} // namespace ops_hccl