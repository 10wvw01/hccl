    ...
    const OpParam &param, const TopoInfoWithNetLayerDetails *topoInfo, const AlgHierarchyInfoForAllLevel &algHierarchyInfo)
{
    ...
    HCCL_INFO("[InsV2BroadcastConcurrentExecutor][InitCommInfo] myRank [%u], rankSize [%u], devType [%u], "
    ...
    AlgResourceRequest &resourceRequest)
{
    ...
    // 两个temp模板并行，资源累加，temp0的主thread负责和temp1主thread同步
    resourceRequest.slaveThreadNum = temp0ResReq.slaveThreadNum + temp1ResReq.slaveThreadNum + 1; // 将mesh作为主流，其他都是从流
    resourceRequest.notifyNumOnMainThread = temp0ResReq.notifyNumOnMainThread + 1; // 1: 用于两个template间同步
    resourceRequest.notifyNumPerThread.insert(resourceRequest.notifyNumPerThread.end(),
                                              temp0ResReq.notifyNumPerThread.begin(),
                                              temp0ResReq.notifyNumPerThread.end());
    resourceRequest.notifyNumPerThread.emplace_back(temp1ResReq.notifyNumOnMainThread + 1); // 1: 用于两个template间同步
    resourceRequest.notifyNumPerThread.insert(resourceRequest.notifyNumPerThread.end(),
    ...
    if (param.engine == CommEngine::COMM_ENGINE_CCU) {
        ...
        CHK_PRT_RET(temp0Channels.size() != temp1Channels.size(),
            HCCL_ERROR("[InsV2BroadcastConcurrentExecutor][CalcRes] temp0Channels.size()[%zu] is not equal to temp1Channels.size()[%zu]",
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
void InsV2BroadcastConcurrentExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::GenTemplateAlgParams(
    ...
    const u64 dataCountPerLoop, const u64 scratchOffset, TemplateDataParams &tempAlgParams) const
{
    ...
    HCCL_DEBUG(
        "[InsV2BroadcastConcurrentExecutor][GenTemplateAlgParams] rank[%d] inBuffBaseOff[%llu] "
...
{
    const u32 portNum0 = rankSize_ - 1;
    ...
    HCCL_INFO("[InsV2BroadcastConcurrentExecutor][GetParallelDataSplit] portNum0[%u], portNum1[%u], splitData[%.4f], ",
            portNum0, portNum1, splitData);
    return;
}
 
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
HcclResult InsV2BroadcastConcurrentExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::PrepareResForTemplate(
    InsAlgTemplate0 &algTemplate0, InsAlgTemplate1 &algTemplate1)
{
    AlgResourceRequest temp0Request;
    AlgResourceRequest temp1Request;
    algTemplate0.GetRes(temp0Request);
    algTemplate1.GetRes(temp1Request);
    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    if (param.engine == CommEngine::COMM_ENGINE_CCU) {
        ...
        tmp1CcuKernels_.assign(resCtx.ccuKernels.begin() + resCtx.ccuKernelNum[0],
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR("[InsV2BroadcastConcurrentExecutor][Orchestrate]errNo[0x%016llx] Broadcast executor kernel run failed",
                   HCCL_ERROR_CODE(ret)),
        ret);
    return HCCL_SUCCESS;
}
    ...
    InsAlgTemplate1 &algTemplate1)
{
    ...
    HCCL_INFO("[InsV2BroadcastConcurrentExecutor]maxTmpMemSize_ [%u]", maxTmpMemSize_);
    ...
    CHK_PRT_RET(maxCountPerLoopforTemp0 == 0,
                HCCL_ERROR("[InsV2BroadcastConcurrentExecutor][OrchestrateLoop] maxDataCountPerLoop0 is 0"),
                HCCL_E_INTERNAL);
    CHK_PRT_RET(maxCountPerLoopforTemp1 == 0,
                HCCL_ERROR("[InsV2BroadcastConcurrentExecutor][OrchestrateLoop] maxDataCountPerLoop1 is 0"),
                HCCL_E_INTERNAL);
    ...
    HCCL_INFO("[InsV2BroadcastConcurrentExecutor][OrchestrateLoop] maxCountPerLoopforTemp0[%llu], maxCountPerLoopforTemp1[%llu], "
    ...
    for (u32 loopIndex = 0; loopIndex < loopTimesforTemp0 || loopIndex < loopTimesforTemp1; loopIndex++) {
        if (loopIndex < loopTimesforTemp0) {
            ...
            GenTemplateAlgParams(param, resCtx, dataOffsetforTemp0, currCountforTemp0, scratchOffsetforTemp0,
                                 AlgParamsforTemp0);
            HCCL_INFO("[InsV2BroadcastConcurrentExecutor][OrchestrateLoop] loopIndex[%u], currCountforTemp0[%u], dataOffsetforTemp0[%u]", loopIndex, currCountforTemp0, dataOffsetforTemp0);
        ...
        if (loopIndex < loopTimesforTemp1) {
            ...
            GenTemplateAlgParams(param, resCtx, dataOffsetforTemp1, currCountforTemp1, scratchOffsetforTemp1,
                                 AlgParamsforTemp1);
            HCCL_INFO("[InsV2BroadcastConcurrentExecutor][OrchestrateLoop] loopIndex[%u], currCountforTemp1[%u], dataOffsetforTemp1[%u]", loopIndex, currCountforTemp1, dataOffsetforTemp1);
...
#ifndef AICPU_COMPILE
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
HcclResult InsV2BroadcastConcurrentExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::FastLaunchSaveCtx(
...
{
    HCCL_INFO("[InsV2BroadcastConcurrentExecutor] loopTimes==1, save fast launch ctx.");
    u32 threadNum = threads_.size();
    u32 ccuKernelNum = templateAlgRes0.submitInfos.size() + templateAlgRes1.submitInfos.size();
    if (ccuKernelNum < 1) {
        HCCL_INFO("[InsV2BroadcastConcurrentExecutor] ccu kernel num is 0, no need to save.");
        return HCCL_SUCCESS;
    }
    HCCL_INFO("[InsV2BroadcastConcurrentExecutor][FastLaunchSaveCtx] threadNum[%llu], ccuKernelNum[%llu]", threadNum, ccuKernelNum);
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
HcclResult InsV2BroadcastConcurrentExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::FastLaunch(
        const OpParam &param, const CcuFastLaunchCtx *ctx)
{
    HCCL_INFO("[InsV2BroadcastConcurrentExecutor][FastLaunch] Start");
    ...
    HCCL_INFO("[InsV2BroadcastConcurrentExecutor][FastLaunch] temp0 ccuKernelNum[%llu]", ctx->ccuKernelNum[0]);
    ...
    HCCL_INFO("[InsV2BroadcastConcurrentExecutor][FastLaunch] temp1 ccuKernelNum[%llu]", ctx->ccuKernelNum[1]);
    ...
    HCCL_INFO("[InsV2BroadcastConcurrentExecutor][FastLaunch] End.");
    return HCCL_SUCCESS;
}
#endif
...
#ifndef AICPU_COMPILE
#if !defined(HCCL_CANN_COMPAT_850)
REGISTER_EXECUTOR_BY_TWO_TEMPS(HcclCMDType::HCCL_CMD_BROADCAST, CcuBroadcastConcurrentMesh1DNHRMem, InsV2BroadcastConcurrentExecutor,
    TopoMatchConcurrent, CcuTempBroadcastMesh1DMem2Mem, CcuTempBroadcastNHR1DMem2Mem);
 
REGISTER_EXECUTOR_BY_TWO_TEMPS(HcclCMDType::HCCL_CMD_BROADCAST, CcuBroadcastConcurrentMesh1DNHR,
                               InsV2BroadcastConcurrentExecutor, TopoMatchConcurrent, CcuTempBroadcastMesh1D,
