    ...
    InsAlgTemplate2 &tempAlgIntra1, InsAlgTemplate3 &tempAlgInter1)
{
    ...
    const bool isAicpuTs = param.engine == CommEngine::COMM_ENGINE_AICPU_TS;
    u64 processedCount = 0;
    u32 loopIndex = 0;
    while ((isAicpuTs && processedCount < dataCount_) || (!isAicpuTs && loopIndex < loopTimes)) {
        u64 currCountPart0 = 0;
        u64 currCountPart1 = 0;
        u64 dataOffset0 = 0;
        if (isAicpuTs) {
            u64 remainingCount = dataCount_ - processedCount;
            u64 currCount = remainingCount <= sliceCount ? remainingCount : sliceCount;
            currCountPart0 = static_cast<u64>(float(currCount) * dataSplitSize.at(0));
            currCountPart1 = currCount - currCountPart0;
            if (remainingCount > sliceCount) {
                const u64 alignSize = AICPU_ALIGN_SIZE;
                u64 alignedCountPart0 = currCountPart0 * dataTypeSize_ / alignSize * alignSize / dataTypeSize_;
                u64 alignedCountPart1 = currCountPart1 * dataTypeSize_ / alignSize * alignSize / dataTypeSize_;
            ...
            CHK_PRT_RET(currCountPart0 + currCountPart1 == 0,
                        ...
                        HcclResult::HCCL_E_INTERNAL);
            dataOffset0 = processedCount * dataTypeSize_;
        } else {
            currCountPart0 = (loopIndex == loopTimes - 1) ? finalSliceCountPart0 : sliceCountPart0;
            currCountPart1 = (loopIndex == loopTimes - 1) ? finalSliceCountPart1 : sliceCountPart1;
            dataOffset0 = loopIndex * sliceCount * dataTypeSize_;
        }
