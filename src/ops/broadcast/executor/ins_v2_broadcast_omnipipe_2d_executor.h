...
protected:
    ...
    HcclResult CalcEndpointBandwidth(std::vector<double>& endpointAttrBwAvgSC,
    ...
    HcclResult PrepareSliceInfoForLoop(u64 loop, u64 root,
                const std::vector<u64>& allRankSplitData,
                const std::vector<std::vector<u64>>& multiLoopAllRankSplitData,
                const std::vector<double>& endpointAttrBwAvgSC,
                const std::vector<double>& endpointAttrBwAvgAG,
                OmniPipeSliceParam& sliceParam,
                OmniPipeSliceInfo& omniPipeSliceInfoSC,
                OmniPipeSliceInfo& omniPipeSliceInfoAG);

    // 主从线程同步上下文（Scatter/AGLocalCopy/AG三阶段共用）
    struct ThreadSyncCtx {
        ThreadHandle mainThread;
        std::vector<ThreadHandle> syncThreads;
        std::vector<u32> notifyIdxesMainToSub;
        std::vector<u32> notifyIdxesSubToMain;
    };

    // Scatter阶段上下文（聚合scatterAlgTemp + tempAlgParams + templateResource + tempAlgParamsCommon）
    struct ScatterPhaseCtx {
        CcuScatterAlgTemplateX& scatterAlgTempX;
        CcuScatterAlgTemplateY& scatterAlgTempY;
        TemplateDataParams& tempScatterAlgParamsX;
        TemplateDataParams& tempScatterAlgParamsY;
        const TemplateDataParams& tempAlgParamsCommon;
        const TemplateResource& templateResourceScatterX;
        const TemplateResource& templateResourceScatterY;
    };

    // Loop进度信息（loop索引 + 当前loop数据量 + 已处理数据量）
    struct LoopProgress {
        u64 loop;
        u64 currDataCount;
    ...
    // 执行Scatter阶段: 内层2d循环, 按root/同x轴/同y轴分支转发数据
    HcclResult RunScatterPhase(const OpParam& param, const AlgResourceCtxSerializable& resCtx,
                ScatterPhaseCtx& scatterCtx, const OmniPipeSliceInfo& omniPipeSliceInfoSC,
                u64 processedDataCount, const ThreadSyncCtx& syncCtx);

    // 执行AG本地拷贝阶段: 非root节点从HCCL buff拷贝到output, 末尾重置scatterAlgTemp状态
    HcclResult RunAGLocalCopyPhase(const OpParam& param, const AlgResourceCtxSerializable& resCtx,
                ScatterPhaseCtx& scatterCtx, const LoopSplitData& loopSplitData,
                const LoopProgress& loopProgress, const ThreadSyncCtx& syncCtx);

    // 执行AG阶段: 内层2d循环, X/Y维度KernelRun + 主从线程同步
    HcclResult RunAGPhase(const OpParam& param,
                ...
                const OmniPipeSliceInfo& omniPipeSliceInfoAG, u64 processedDataCount,
                const ThreadSyncCtx& syncCtx);
