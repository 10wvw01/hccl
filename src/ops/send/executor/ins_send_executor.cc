...
namespace ops_hccl {
    ...
    HcclResult InsSendExecutor::OrchestrateP2p(const OpParam &param, const AlgResourceCtxSerializable &resCtx,
        ...
        if (resCtx.channels.at(0).size() < 2) {
            HCCL_ERROR(
                "[InsSendExecutor][OrchestrateP2p] resCtx channels size [%u] invalid.", resCtx.channels.at(0).size());
            return HcclResult::HCCL_E_PARA;
        }
        // 给channels_赋值
        const ChannelInfo &channel = myRank_ <= remoteRank_ ? resCtx.channels.at(0).at(0) : resCtx.channels.at(0).at(1);
