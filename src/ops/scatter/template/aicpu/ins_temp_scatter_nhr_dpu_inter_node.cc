...
HcclResult InsTempScatterNHRDPUInterNode::GetStepInfo(u32 step, u32 nSteps, AicpuNHRStepInfo &stepInfo)
{
    ...
    if (deltaRoot < nRanks) {  // 需要发
        ...
        HCCL_INFO("[InsTempScatterNHRDPUInterNode][GetStepInfo] rankSize[%u], myAlgRank[%u], sendTo Idx[%u]", subCommRanks_[0].size(), myAlgRank, sendTo);
        ...
        HCCL_INFO("[InsTempScatterNHRDPUInterNode][GetStepInfo] rankSize[%u], myAlgRank[%u], recvFrom Idx[%u]", subCommRanks_[0].size(), myAlgRank, recvFrom);
