...
namespace ops_hccl {
InsTempScatterMesh1DIntraNode::InsTempScatterMesh1DIntraNode(const OpParam& param, const u32 rankId, // 传通信域的rankId，userRank
                                const std::vector<std::vector<u32>> &subCommRanks)
                                : InsAlgTemplateBase(param, rankId, subCommRanks)
{}

InsTempScatterMesh1DIntraNode::~InsTempScatterMesh1DIntraNode()
{}

void InsTempScatterMesh1DIntraNode::SetRoot(u32 root)
{
    HCCL_INFO("[InsTempScatterMesh1DIntraNode][SetRoot] myRank_ [%u], set root_ [%u] ", myRank_, root);
    root_ = root;
}

u64 InsTempScatterMesh1DIntraNode::GetThreadNum() const
{
    u64 threadNum = templateRankSize_ > 1 ? templateRankSize_ - 1 : 1;
    return threadNum;
}

void InsTempScatterMesh1DIntraNode::GetNotifyIdxMainToSub(std::vector<u32> &notifyIdxMainToSub)
{
...
void InsTempScatterMesh1DIntraNode::GetNotifyIdxSubToMain(std::vector<u32> &notifyIdxSubToMain)
{
...
HcclResult InsTempScatterMesh1DIntraNode::CalcRes(HcclComm comm, const OpParam& param, const TopoInfoWithNetLayerDetails* topoInfo,
...
HcclResult InsTempScatterMesh1DIntraNode::GetRes(AlgResourceRequest &resourceRequest) const
{
    u32 threadNum = templateRankSize_ > 1 ? templateRankSize_ - 1 : 1;
    resourceRequest.slaveThreadNum = threadNum - 1;
    for (u32 index = 0; index < threadNum - 1; index++) {
...
u64 InsTempScatterMesh1DIntraNode::CalcScratchMultiple(BufferType inBuffType, BufferType outBuffType)
{
    return 1;
}

HcclResult InsTempScatterMesh1DIntraNode::KernelRun(const OpParam& param, const TemplateDataParams &tempAlgParams,
                     TemplateResource& templateResource)
{
    ...
    CHK_PRT_RET(templateResource.threads.empty(), 
                HCCL_ERROR("[InsTempScatterMesh1DIntraNode][KernelRun] threads is empty"), 
                HCCL_E_INTERNAL);
    u32 myAlgRank = 0;
    ...
    HCCL_INFO("[InsTempScatterMesh1DIntraNode] Run Start");
    ...
    HCCL_INFO("[InsTempScatterMesh1DIntraNode] Run End");
    return HcclResult::HCCL_SUCCESS;
}

HcclResult InsTempScatterMesh1DIntraNode::PreCopy(
    const TemplateDataParams &tempAlgParams, const std::vector<ThreadHandle> &threads) const
{
    ...
    (tempAlgParams.buffInfo.inBuffType == BufferType::HCCL_BUFFER && tempAlgParams.buffInfo.outBuffType == BufferType::HCCL_BUFFER)) {
        HCCL_INFO("[InsTempScatterMesh1DIntraNode][PreCopy] skip precopy, myRank = %u, root = %u", myRank_, root_);
        return HCCL_SUCCESS;
    }

    // inplace场景跳过前拷贝
    if (tempAlgParams.buffInfo.inputPtr == tempAlgParams.buffInfo.outputPtr) {
        HCCL_INFO("[InsTempScatterMesh1DIntraNode][PreCopy] skip precopy due to inplace, myRank = %u, root = %u", myRank_, root_);
        return HCCL_SUCCESS;
    }
...
HcclResult InsTempScatterMesh1DIntraNode::PostCopy(
    const TemplateDataParams &tempAlgParams, const std::vector<ThreadHandle> &threads) const
{
    ...
    if (tempAlgParams.buffInfo.outBuffType == BufferType::HCCL_BUFFER || enableRemoteMemAccess_) {
        HCCL_INFO("[InsTempScatterMesh1DIntraNode][PostCopy] skip postcopy, myRank = %u, root = %u", myRank_, root_);
        return HCCL_SUCCESS;
    }

    //root rank数据在非inplace场景不需要进行搬运
    if (u32(myRank_) == root_ && tempAlgParams.buffInfo.inputPtr != tempAlgParams.buffInfo.outputPtr) {
        HCCL_INFO("[InsTempScatterMesh1DIntraNode][PostCopy] root rank skip postcopy, myRank = %u, root = %u", myRank_, root_);
        return HCCL_SUCCESS;
    }
    ...
    if (isInplaceRootRank){
        HCCL_INFO("[InsTempScatterMesh1DIntraNode][PostCopy], is inplace root rank, copy from userIn to userOut");
    }
    else {// 如果是单算子模式, 并且是最后一步算子，需要将数据从 cclBuffer 拷贝到 userOut
        HCCL_INFO("[InsTempScatterMesh1DIntraNode][PostCopy], copy from cclBuffer to userOut");
    }
...
HcclResult InsTempScatterMesh1DIntraNode::RunMesh(const std::map<u32, std::vector<ChannelInfo>> &channels, const std::vector<ThreadHandle> &threads, 
                    const TemplateDataParams &tempAlgParams)
{
    ...
    const u32 dataTypeSize = DATATYPE_SIZE_TABLE[dataType_];
    GetAlgRank(myRank_, subCommRanks_[0], myAlgRank);
    HCCL_DEBUG("[InsTempScatterMesh1DIntraNode][RunMesh] myRank[%d], myAlgRank[%d], channels size[%d]", myRank_, myAlgRank, channels.size());
    for (u32 r = 0; r < tempAlgParams.repeatNum; r++) {
        if (root_ == u32(myRank_)) {
            ...
            for (u32 algRank = 0; algRank < subCommRanks_[0].size(); algRank++) {
                ...
                HCCL_DEBUG("[InsTempScatterMesh1DIntraNode][RunMesh] algRank[%d]", algRank);
                u32 remoteRank = subCommRanks_[0][algRank];
                HCCL_INFO("[InsTempScatterMesh1DIntraNode][RunMesh] myRank[%d], toRank[%d]", myRank_, remoteRank);
                CHK_PRT_RET(channels.find(remoteRank) == channels.end() || channels.at(remoteRank).empty(), 
 	                        HCCL_ERROR("[InsTempScatterMesh1DIntraNode][RunMesh] remoteRank[%d] not found in channels", remoteRank), 
 	                        HCCL_E_INTERNAL);
                ...
                HCCL_DEBUG("[InsTempScatterMesh1DIntraNode][RunMesh] srcOffset[%d], tempAlgParams.buffInfo.inputPtr[%d]", srcOffset, tempAlgParams.buffInfo.inputPtr);
                HCCL_DEBUG("[InsTempScatterMesh1DIntraNode][RunMesh] dstOffset[%d], txDstPtr[%d]", dstOffset, txDstPtr);
                DataSlice srcSlice = DataSlice(tempAlgParams.buffInfo.inputPtr, srcOffset, curSliceSize, curCount);
                HCCL_DEBUG("[InsTempScatterMesh1DIntraNode][RunMesh] got srcSlice");
                DataSlice dstSlice = DataSlice(txDstPtr, dstOffset, curSliceSize, curCount);
                HCCL_DEBUG("[InsTempScatterMesh1DIntraNode][RunMesh] got dstSlice");
                SlicesList txSlicesList({srcSlice}, {dstSlice});

                HCCL_DEBUG("[InsTempScatterMesh1DIntraNode][RunMesh] tempAlgParam.buffInfo.hcclBuff.addr[%d], tempAlgParams.buffInfo.inputPtr[%d], tempAlgParams.buffInfo.outputPtr[%d], ", tempAlgParams.buffInfo.hcclBuff.addr, tempAlgParams.buffInfo.inputPtr, tempAlgParams.buffInfo.outputPtr);
                HCCL_DEBUG("[InsTempScatterMesh1DIntraNode][RunMesh] txDstPtr[%d]", txDstPtr);
                HCCL_DEBUG("[InsTempScatterMesh1DIntraNode][RunMesh] tempAlgParam.buffInfo.inBuffBaseOff[%d], tempAlgParam.buffInfo.outBuffBaseOff[%d], tempAlgParam.buffInfo.hcclBuffBaseOff[%d]", tempAlgParams.buffInfo.inBuffBaseOff, tempAlgParams.buffInfo.outBuffBaseOff, tempAlgParams.buffInfo.hcclBuffBaseOff);

                DataInfo sendData(linkSend, txSlicesList);
                HCCL_DEBUG("[InsTempScatterMesh1DIntraNode][RunMesh] start SendWrite");
                CHK_PRT_RET(count >= threads.size(), 
                            HCCL_ERROR("[InsTempScatterMesh1DIntraNode][RunMesh] count[%d] >= threads.size()[%d]", count, threads.size()), 
                            HCCL_E_INTERNAL);
                CHK_PRT_RET(static_cast<HcclResult>(SendWrite(sendData, threads.at(count))),
                    HCCL_ERROR("[InsTempScatterMesh1DIntraNode] RunMesh Send failed"),
                    HcclResult::HCCL_E_INTERNAL);
                HCCL_DEBUG("[InsTempScatterMesh1DIntraNode][RunMesh] end SendWrite");
                count++;
            }
        } else {
            ...
            CHK_PRT_RET(channels.find(root_) == channels.end() || channels.at(root_).empty(), 
                        HCCL_ERROR("[InsTempScatterMesh1DIntraNode][RunMesh] root[%d] not found in channels", root_), 
                        HCCL_E_INTERNAL);
            ...
            CHK_PRT_RET(static_cast<HcclResult>(RecvWrite(recvData, threads.at(0))),
                HCCL_ERROR("[InsTempScatterMesh1DIntraNode] RunMesh Recv failed"),
                HcclResult::HCCL_E_INTERNAL);
        }
    }
    return HcclResult::HCCL_SUCCESS;
