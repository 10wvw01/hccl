    ...
    const OpParam &param, const TemplateDataParams &templateDataParams, TemplateResource &templateResource)
{
    ...
    if (localCopyFlag == 0) {
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            if (ifDoTask_) {
                sliceSize = stepSliceInfo.stepSliceSize[myRank_ % xRankSize_][rpt]; // 结合Scatter Omni构造数据特点，取的对应下标
                ...
                for (uint32_t ridx = 0; ridx < templateRankSize_; ridx++) {
                    if (ridx == subCommRootId_) {
                        ...
                        inputOmniSliceSizeVec.push_back(stepSliceInfo.stepSliceSize[myRank_%xRankSize_][sliceStrideiIndex]);
                        HCCL_DEBUG(
