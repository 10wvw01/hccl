            ...
            if (ifDoTask_) {
                ...
                for (uint32_t ridx = 0; ridx < templateRankSize_; ridx++) {
                    if (ridx == subCommRootId_) {
                        ...
                        HCCL_DEBUG("[checkSliceSize] myrank:%d,mySubCommRank_:%d, ridx:%d, stepSliceSize:%d", myRank_, mySubCommRank_, ridx, stepSliceInfo.stepSliceSize[ridx][sliceStrideiIndex]);
                        HCCL_DEBUG(
