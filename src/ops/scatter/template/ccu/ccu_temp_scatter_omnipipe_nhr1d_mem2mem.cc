    ...
    if (localCopyFlag == 0) {
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            if (ifRealRoot_ || ifNewRoot) {
                ...
                for (uint32_t ridx = 0; ridx < templateRankSize_; ridx++) {
                    if (ridx == subCommRootId_) {
                        ...
                        HCCL_INFO("[checkSliceStride] myrank:%d,mySubCommRank_:%d, "
                                  "ridx:%d,sliceStrideiIndex:%d,outputOmniSliceStrideTmp:%d",
            ...
            for (uint32_t i = 0; i< outputOmniSliceStrideVec.size(); i++) {
                HCCL_INFO("outputOmniSliceStrideVec[%d] = %llu", i, outputOmniSliceStrideVec[i]);
            }
