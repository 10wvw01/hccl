    ...
    : CcuAlgTemplateBase(param, rankId, subCommRanks)
{
    ...
    // 和root节点同x轴的rank也需要发送数据，所以也需要设置一下subCommRootId_
    // if (isSameXAxisAsRoot) {
    //     itRoot = std::find(ranks.begin(), ranks.end(), rankId);
    //     if (itRoot != ranks.end()) {
    //         subCommRootId_ = std::distance(ranks.begin(), itRoot);
    //     }
    // }
    ...
    if (localCopyFlag == 0) {
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            if (ifRealRoot_ || ifNewRoot) {
                ...
                for (uint32_t ridx = 0; ridx < templateRankSize_; ridx++) {
                    if (ridx == subCommRootId_) {
                        ...
                        HCCL_INFO("[checkSliceStride] myrank:%d,mySubCommRank_:%d, ridx:%d, tag0", myRank_, mySubCommRank_, ridx);
                        uint32_t skipIndex = ridx > 1 ? (ridx - 1) : 0;
                        HCCL_INFO("[checkSliceStride] myrank:%d,mySubCommRank_:%d, ridx:%d, tag1 skipIndex:%d", myRank_, mySubCommRank_, ridx, skipIndex);
                        ...
                        HCCL_INFO("[checkSliceStride] myrank:%d,mySubCommRank_:%d, ridx:%d, tag2 sliceStrideiIndex:%d", myRank_, mySubCommRank_, ridx, sliceStrideiIndex);
            ...
            for (uint32_t i = 0; i< outputOmniSliceStrideVec.size(); i++) {
                HCCL_INFO("myRank_[%u] outputOmniSliceStrideVec[%d] = %llu", myRank_, i, outputOmniSliceStrideVec[i]);
            }
            for (uint32_t i = 0; i< inputOmniSliceStrideVec.size(); i++) {
                HCCL_INFO("myRank_[%u] inputOmniSliceStrideVec[%d] = %llu", myRank_, i, inputOmniSliceStrideVec[i]);
            }
            ...
            HCCL_INFO("[%s] myRank[%u] mySubCommRank[%u] rpt[%u] argSize[%llu]", __func__, myRank_, mySubCommRank_, rpt, argSize);
