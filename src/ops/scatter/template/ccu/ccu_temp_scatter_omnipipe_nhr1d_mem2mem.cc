    ...
    const OpParam &param, const TemplateDataParams &templateDataParams, TemplateResource &templateResource)
{
    ...
    if (localCopyFlag == 0) {
        ...
        CHK_PRT_RET(totalPieceNum % peerNum != 0,
            HCCL_ERROR("inputOmniPipeSliceStride size=%llu not divisible by peerNum=%llu, repeatNum truncated", totalPieceNum,
            peerNum), HCCL_E_INTERNAL);
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            BuildSliceStrideVec(
                stepSliceInfo, rpt, repeatNum, ifDoTask_, sliceSize, inputOmniSliceStrideVec, outputOmniSliceStrideVec);
            // if (ifDoTask_) {
            //     sliceSize = stepSliceInfo.stepSliceSize[myRank_ % xRankSize_][rpt]; //  TODO 1、rpt
            //     对应的大小，需要计算下，当前默认发的都是一样大小 2、xRankSize_传过来 inputOmniPipeSliceStride =
            //     stepSliceInfo.inputOmniPipeSliceStride[myRank_% xRankSize_][rpt]; outputOmniPipeSliceStride=
            //     stepSliceInfo.outputOmniPipeSliceStride[myRank_% xRankSize_][rpt]; uint64_t originIndex = 0; for
            //     (uint32_t ridx = 0; ridx < templateRankSize_; ridx++) {
            //         if (ridx == subCommRootId_) {
            //             outputOmniSliceStrideVec.push_back(0);
            //             inputOmniSliceStrideVec.push_back(0);
            //         } else {
            //             HCCL_DEBUG("[checkSliceStride] myrank:%d,mySubCommRank_:%d, ridx:%d, tag0", myRank_,
            //             mySubCommRank_, ridx); uint64_t sliceStrideiIndex = repeatNum * originIndex  + rpt;
            //             originIndex = originIndex + 1;
            //             HCCL_DEBUG("[checkSliceStride] myrank:%d,mySubCommRank_:%d, ridx:%d, tag2
            //             sliceStrideiIndex:%d", myRank_, mySubCommRank_, ridx, sliceStrideiIndex); uint64_t
            ...
            //                 "ridx:%d,sliceStrideiIndex:%d,outputOmniSliceStrideTmp:%llu,
            //                 inputOmniSliceStrideTmp:%llu", myRank_, mySubCommRank_, ridx, sliceStrideiIndex,
            //                 outputOmniSliceStrideTmp, inputOmniSliceStrideTmp);
            //         }
            //     }
            // } else {
...
void CcuTempScatterOmniPipeNHR1DMem2Mem::BuildSliceStrideVec(const StepSliceInfo &stepSliceInfo, uint32_t rpt,
    uint64_t repeatNum, bool ifDoTask_, uint64_t &sliceSize, std::vector<uint64_t> &inputOmniSliceStrideVec,
    std::vector<uint64_t> &outputOmniSliceStrideVec)
{
    if (!ifDoTask_) {
        for (uint32_t ridx = 0; ridx < templateRankSize_; ridx++) {
    ...
    for (uint32_t ridx = 0; ridx < templateRankSize_; ridx++) {
        if (ridx == subCommRootId_) {
            ...
            originIndex = originIndex + 1;
            HCCL_DEBUG("[checkSliceStride] myrank:%d,mySubCommRank_:%d, ridx:%d, tag2 sliceStrideiIndex:%d", myRank_,
                mySubCommRank_, ridx, sliceStrideiIndex);
            ...
            uint64_t inputOmniSliceStrideTmp
                = stepSliceInfo.inputOmniPipeSliceStride[myRank_ % xRankSize_][sliceStrideiIndex];
            inputOmniSliceStrideVec.push_back(inputOmniSliceStrideTmp);
            HCCL_DEBUG("[checkSliceStride] myrank:%d,mySubCommRank_:%d, "
                       "ridx:%d,sliceStrideiIndex:%d,outputOmniSliceStrideTmp:%llu, inputOmniSliceStrideTmp:%llu",
                myRank_, mySubCommRank_, ridx, sliceStrideiIndex, outputOmniSliceStrideTmp, inputOmniSliceStrideTmp);
        }
    }
}
