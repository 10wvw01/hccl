...
HcclResult CcuTempScatterOmniPipeNHR1DMem2Mem::GetStepInfo(u32 step, u32 nSteps, NHRStepInfo &stepInfo) const
{
    ...
    if (deltaRoot < nRanks) {
        ...
        for (u64 i = 0; i < nSlices; i++) {
            ...
            HCCL_DEBUG("GetStepInfo [%s] step[%u] myRank[%u] mySubCommRank[%u] txSliceIdx[%u] sendTo[%u] slice-i[%llu]",
        ...
        for (u64 i = 0; i < nSlices; i++) {
            ...
            HCCL_DEBUG("GetStepInfo [%s] step[%u] myRank[%u] mySubCommRank[%u] rxSliceIdx[%u] recvFrom[%u] slice-i[%llu]",
