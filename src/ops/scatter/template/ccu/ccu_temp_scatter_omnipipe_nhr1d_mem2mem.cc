...
void CcuTempScatterOmniPipeNHR1DMem2Mem::BuildSliceStrideVec(const StepSliceInfo &stepSliceInfo, uint32_t rpt,
    uint64_t repeatNum, bool ifDoTask, uint64_t &sliceSize, std::vector<uint64_t> &inputOmniSliceStrideVec,
    std::vector<uint64_t> &outputOmniSliceStrideVec)
{
    if (!ifDoTask) {
...
HcclResult CcuTempScatterOmniPipeNHR1DMem2Mem::FastLaunch(
...
HcclResult CcuTempScatterOmniPipeNHR1DMem2Mem::GetStepInfo(u32 step, u32 nSteps, NHRStepInfo &stepInfo) const
{
    ...
    HCCL_DEBUG("[%s] myRank[%u] StepInfo step[%u] nSteps[%u] nSlices[%u] fromRank[%u] toRank[%u] ", __func__, myRank_,
        step, nSteps, stepInfo.nSlices, stepInfo.fromRank, stepInfo.toRank);
    return HcclResult::HCCL_SUCCESS;
}
