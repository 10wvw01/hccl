    ...
    const TopoInfoWithNetLayerDetails *topoInfo, AlgResourceRequest &resourceRequest)
{
    ...
    CHK_SAFETY_FUNC_RET(strcpy_s(kernelInfo.kernelFuncName, sizeof(kernelInfo.kernelFuncName), "CcuScatterOmniPipeMesh1DMem2MemKernel"));
    ...
    const OpParam &param, const TemplateDataParams &templateDataParams, TemplateResource &templateResource)
{
    ...
    if (localCopyFlag == 0) {
        ...
        uint64_t repeatNum = stepSliceInfo.inputOmniPipeSliceStride[0].size() / (templateRankSize_-1);

        // if (ifNewRoot) {
        //     targetPeerIdxVec_.clear();
        //     uint64_t peerNum = (subCommRootId_ < templateRankSize_) ? (templateRankSize_ - 1) : templateRankSize_;
        //     uint64_t repeatPerPeer = (peerNum > 0) ? (repeatNum / peerNum) : 0;
        //     for (uint64_t oneDid = 0; oneDid < templateRankSize_; oneDid++) {
        //         if (oneDid != subCommRootId_) {
        //             for (uint64_t r = 0; r < repeatPerPeer; r++) {
        //                 targetPeerIdxVec_.push_back(oneDid);
        //             }
        //         }
        //     }
        // }

        HCCL_DEBUG("[%s] myRank[%u] mySubCommRank_[%u] subCommRootId_[%u] ifNewRoot[%d] isStepone[%d] isLastStep[%d] repeatNum[%llu]", __func__, myRank_,
        mySubCommRank_, subCommRootId_, ifNewRoot, isStepOne_, isLastStep_, repeatNum);
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            if (ifNewRoot) {
                ...
                for (uint32_t ridx = 0; ridx < templateRankSize_; ridx++) {
                    if (ridx == subCommRootId_) {
                        ...
                        uint64_t outputOmniSliceStrideTmp = stepSliceInfo.outputOmniPipeSliceStride[myRank_/templateRankSize_][sliceStrideiIndex];
                        outputOmniSliceStrideVec.push_back(outputOmniSliceStrideTmp);
                        uint64_t inputOmniSliceStrideTmp = stepSliceInfo.inputOmniPipeSliceStride[mymyRank_/templateRankSize_][sliceStrideiIndex];
                        inputOmniSliceStrideVec.push_back(inputOmniSliceStrideTmp);
                        HCCL_INFO("myrank:%d,mySubCommRank_:%d, ridx:%d, sliceStrideiIndex:%d, "
                                  "outputOmniSliceStrideTmp:%llu, inputOmniSliceStrideTmp:%llu",
            ...
            for (uint32_t i = 0; i < outputOmniSliceStrideVec.size(); i++) {
                HCCL_INFO("myRank_[%u] subCommRootId_[%u] rpt[%u] sliceSize[%llu] outputOmniSliceStrideVec[%d] = %llu "
            ...
            for (uint32_t i = 0; i < inputOmniSliceStrideVec.size(); i++) {
                HCCL_INFO("myRank_[%u] subCommRootId_[%u] rpt[%u] sliceSize[%llu] inputOmniSliceStrideVec[%d] = %llu "
                          "isStepone[%d] isLastStep[%d] isFirstPiece[%d] isLastPiece[%d]",
