    ...
    const OpParam &param, const TemplateDataParams &templateDataParams, TemplateResource &templateResource)
{
    ...
    if (localCopyFlag == 0) {
        ...
        uint64_t repeatNum = stepSliceInfo.inputOmniPipeSliceStride[0].size();
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            uint64_t targetPeerIdx = templateRankSize_;
            ...
            if (ifNewRoot) {
                ...
                targetPeerIdx = targetPeerIdxVec_[rpt];
            }
            ...
            HCCL_INFO("[%s] myRank[%u] mySubCommRank[%u] subCommRootId_[%u] rpt[%u] sliceSize[%llu] "
                      ...
                      "outputOmniPipeSliceStride[%llu] isStepone[%d] isLastStep[%d] targetPeerIdx[%llu] "
                      "isFirstPiece[%d] isLastPiece[%d]",
                __func__, myRank_, mySubCommRank_, subCommRootId_, rpt, sliceSize, inputOmniPipeSliceStride, inputAddr,
                outputAddr, outputOmniPipeSliceStride, isStepOne_, isLastStep_, targetPeerIdx, isFirstPiece,
                isLastPiece);

            std::vector<uint64_t> taskArgs
                = {inputAddr, outputAddr, sliceSize, token, inputOmniPipeSliceStride, outputOmniPipeSliceStride,
                    isStepOne_, isLastStep_, ifNewRoot, targetPeerIdx, isFirstPiece, isLastPiece};
