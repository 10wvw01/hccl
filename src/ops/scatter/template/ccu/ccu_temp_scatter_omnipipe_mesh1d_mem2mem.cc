    ...
    if (localCopyFlag == 0) {
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            HCCL_INFO("[%s] myRank[%u] mySubCommRank[%u] subCommRootId_[%u] rpt[%u] sliceSize[%llu] "
                ...
                outputAddr, outputOmniPipeSliceStride, isStepOne_, isLastStep_, targetPeerIdx);

            std::vector<uint64_t> taskArgs = {inputAddr, outputAddr, sliceSize, token, inputOmniPipeSliceStride,
                outputOmniPipeSliceStride, isStepOne_, isLastStep_, ifNewRoot, targetPeerIdx};
