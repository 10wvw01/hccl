...
static CcuResult LoadArgs(ScatterOmniPipeNHR1DMem2MemContext &ctx)
{
    ...
    for (uint64_t i = 0; i < ctx.rankSize; i++) {
        ...
        HCCL_DEBUG("argId[%u] outputOmniSliceStrideVec[%u] = %u", argId - 1, i, ctx.outputOmniSliceStrideVec[i]);
    }
    for (uint64_t i = 0; i < ctx.rankSize; i++) {
        CCU_CHK_RET(ccu::LoadArg(ctx.inputOmniSliceStrideVec[i], argId++));
        HCCL_DEBUG("argId[%u] inputOmniSliceStrideVec[%u] = %u", argId - 1, i, ctx.inputOmniSliceStrideVec[i]);
    }
    return CCU_SUCCESS;
}
    ...
    const NHRStepInfo &nhrStepInfo)
{
    ...
    if (!sendSliceIdxList.empty()) {
        ...
        for (u32 i = 0; i < sendSliceIdxList.size(); i++) {
            ...
            {
                ccu::Write(sendChannel, dst, src, ctx.sliceSize, ctx.event, 1);
            }
            CCU_IF(ctx.sliceSize == 0)
            {
