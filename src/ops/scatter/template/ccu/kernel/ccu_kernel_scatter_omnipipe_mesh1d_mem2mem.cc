...
static CcuResult ParseKernelArg(ScatterOmniPipeMesh1DMem2MemContext &ctx, CcuKernelArgScatterOmniPipeMesh1DMem2Mem *kernelArg)
{
    ctx.arg = kernelArg;
...
static CcuResult PostSync(ScatterOmniPipeMesh1DMem2MemContext &ctx)
{
    for (uint32_t i = 0; i < ctx.arg->channelCount; i++) {
        HCCL_INFO("---5----PostSync realRank[%u] channel[%u]", ctx.myrealrank, i);
        CCU_CHK_RET(ccu::NotifyRecord(ctx.arg->channels[i], CKE_IDX_0, 1 << POST_SYNC_ID));
        HCCL_INFO("---6----PostSync realRank[%u] channel[%u]", ctx.myrealrank, i);
    }
    for (uint32_t i = 0; i < ctx.arg->channelCount; i++) {
        HCCL_INFO("---7----PostSync realRank[%u] channel[%u]", ctx.myrealrank, i);
        CCU_CHK_RET(ccu::NotifyWait(ctx.arg->channels[i], CKE_IDX_0, 1 << POST_SYNC_ID));
        HCCL_INFO("---8----PostSync realRank[%u] channel[%u]", ctx.myrealrank, i);
    }
    return CCU_SUCCESS;
}
...
{
    HCCL_INFO("[CcuScatterOmniPipeMesh1DMem2MemKernel] PreSync realRank[%u] ctx.arg->channelCount[%u]", ctx.myrealrank, ctx.arg->channelCount);
    
    for (uint32_t i = 0; i < ctx.arg->channelCount; i++) {
        HCCL_INFO("---1----PreSync realRank[%u] channel[%u]", ctx.myrealrank, i);
        CCU_CHK_RET(ccu::WriteVariableWithNotify(ctx.arg->channels[i], ctx.output[ctx.arg->rankId], OUTPUT_XN_ID, CKE_IDX_0, 1 << OUTPUT_XN_ID));
        CCU_CHK_RET(ccu::WriteVariableWithNotify(ctx.arg->channels[i], ctx.token[ctx.arg->rankId], TOKEN_XN_ID, CKE_IDX_0, 1 << TOKEN_XN_ID));
        HCCL_INFO("---2----PreSync realRank[%u] channel[%u]", ctx.myrealrank, i);
    }
    uint32_t allBit = (1 << OUTPUT_XN_ID) | (1 << TOKEN_XN_ID);
    HCCL_INFO("[CcuScatterOmniPipeMesh1DMem2MemKernel] PreSync realRank[%u] allBit[%u]", ctx.myrealrank, allBit);
    for (uint32_t i = 0; i < ctx.arg->channelCount; i++) {
        HCCL_INFO("---3----PreSync realRank[%u] channel[%u]", ctx.myrealrank, i);
        CCU_CHK_RET(ccu::NotifyWait(ctx.arg->channels[i], CKE_IDX_0, allBit));
        HCCL_INFO("---4----PreSync realRank[%u] channel[%u]", ctx.myrealrank, i);
    }
    return CCU_SUCCESS;
}
        ...
        CCU_IF(ctx.sliceSize != 0)
        {
            if (rankIdx != ctx.rankId) {
                ccu::Write(ctx.arg->channels[channelId], ctx.outputMem[rankIdx],
                           ctx.inputMem[rankIdx], ctx.sliceSize, ctx.event, mask);
                channelId++;
            } else {
