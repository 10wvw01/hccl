...
{
    // TODO 加点日志
    HCCL_INFO("[PreSync] start tag0! ctx.rankId is [%u]", ctx.rankId);
    uint32_t allBit = (1 << OUTPUT_XN_ID) | (1 << TOKEN_XN_ID);
    for (uint32_t i = 0; i < ctx.arg->channelCount; i++) {
        HCCL_INFO("[PreSync] start tag1! ctx.rankId is [%u] i is [%u]", ctx.rankId, i);
    ...
    for (uint32_t i = 0; i < ctx.arg->channelCount; i++) {
        HCCL_INFO("[PreSync] start tag2! ctx.rankId is [%u] i is [%u]", ctx.rankId, i);
        // ccu::NotifyWait(ctx.arg->channels[i], CKE_IDX_0, allBit);
    }
    HCCL_INFO("[PreSync] end tag!");
    return CCU_SUCCESS;
}
...
CcuResult CcuScatterOmniPipeMesh1DMem2MemKernel(CcuKernelArg arg)
{
    ...
    // CCU_CHK_RET(PostSync(ctx)); // 这个放开这里也卡死
    return CCU_SUCCESS;
}
