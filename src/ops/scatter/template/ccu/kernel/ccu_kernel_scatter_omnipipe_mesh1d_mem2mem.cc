...
static CcuResult PostSync(ScatterOmniPipeMesh1DMem2MemContext &ctx)
{
    for (uint32_t i = 0; i < ctx.arg->channelCount; i++) {
        HCCL_INFO("---1----PostSync realRank[%u] channel[%u]", ctx.myrealrank, i);
        CCU_CHK_RET(ccu::NotifyRecord(ctx.arg->channels[i], CKE_IDX_0, 1 << POST_SYNC_ID));
        HCCL_INFO("---2----PostSync realRank[%u] channel[%u]", ctx.myrealrank, i);
    }
    for (uint32_t i = 0; i < ctx.arg->channelCount; i++) {
        HCCL_INFO("---3----PostSync realRank[%u] channel[%u]", ctx.myrealrank, i);
        CCU_CHK_RET(ccu::NotifyWait(ctx.arg->channels[i], CKE_IDX_0, 1 << POST_SYNC_ID));
        HCCL_INFO("---4----PostSync realRank[%u] channel[%u]", ctx.myrealrank, i);
    }
    return CCU_SUCCESS;
}
