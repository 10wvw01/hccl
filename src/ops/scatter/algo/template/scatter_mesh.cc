...
HcclResult ScatterMesh::RunSendScatter(const u32 dstRank, const Slice &slice, std::vector<ChannelInfo> &channels)
{
    ...
    if (dstRank == interRank_) {
        ...
        CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(thread_, channels[dstRank].handle, NOTIFY_IDX_ACK, ExecTimeoutManager::Instance().GetExecTimeout())));
...
HcclResult ScatterMesh::RunRecvScatter(const u32 srcRank, const Slice &slice, std::vector<ChannelInfo> &channels)
{
    ...
    CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(thread_, channels[srcRank].handle, NOTIFY_IDX_DATA_SIGNAL, ExecTimeoutManager::Instance().GetExecTimeout())));
