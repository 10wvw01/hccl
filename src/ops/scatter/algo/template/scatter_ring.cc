...
HcclResult ScatterRing::RunScatterOnRootRank()
{
    ...
    for (u32 i = 1; i < interRankSize_; i++) {
        ...
        // 等待后一节点同步信号，进行下一轮操作
        CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(thread_, channelRight_.handle, NOTIFY_IDX_ACK, ExecTimeoutManager::Instance().GetExecTimeout())));
...
HcclResult ScatterRing::RunScatterOnEndRank()
{
    ...
    CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(thread_, channelLeft_.handle, NOTIFY_IDX_DATA_SIGNAL, ExecTimeoutManager::Instance().GetExecTimeout())));
