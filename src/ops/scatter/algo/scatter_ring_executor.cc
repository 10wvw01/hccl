    ...
    const u64 baseOffset)
{
    ...
    for (u32 ringIndex = 0; ringIndex < ringNum; ringIndex++) {
        ...
        if (ringIndex != (ringNum - 1)) {  // 0~ringNum-2的环
            // 从环等主环通知开始
            CHK_RET(static_cast<HcclResult>(HcommThreadNotifyWaitOnThread(slaveThreads_[ringIndex], LOCAL_NOTIFY_IDX_ZERO, ExecTimeoutManager::Instance().GetExecTimeout())));
            ...
            for (u32 ring = 0; ring < (ringNum - 1); ring++) {
                // 主环等所有从环结束
                CHK_RET(static_cast<HcclResult>(HcommThreadNotifyWaitOnThread(thread_, ring, ExecTimeoutManager::Instance().GetExecTimeout())));
            }
        }
    }
