...
#include "ins_v2_scatter_sequence_executor_3level.h"
...
InsV2ScatterSequenceExecutor3Level<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::
    InsV2ScatterSequenceExecutor3Level()
{
}

template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult InsV2ScatterSequenceExecutor3Level<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::InitCommInfo(
    ...
    const AlgHierarchyInfoForAllLevel& algHierarchyInfo)
{
    ...
    HCCL_INFO("[InsV2ScatterSequenceExecutor3Level][InitCommInfo] myRank [%u], rankSize [%u], "
        "dataType [%u] dataTypeSize [%u]", myRank_, rankSize_, dataType_, dataTypeSize_);
    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult InsV2ScatterSequenceExecutor3Level<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::CalcAlgHierarchyInfo(
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult InsV2ScatterSequenceExecutor3Level<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::CalcRes(
    HcclComm comm, const OpParam& param, const TopoInfoWithNetLayerDetails* topoInfo,
    const AlgHierarchyInfoForAllLevel& algHierarchyInfo, AlgResourceRequest& resourceRequest)
{
    HCCL_DEBUG("[InsV2ScatterSequenceExecutor3Level][CalcRes] myRank[%u] start", myRank_);
    InitCommInfo(param, topoInfo, algHierarchyInfo);

    skipLevel1_ = (algHierarchyInfo.infos[1][0].size() == 1);
    skipLevel2_ = std::string(param.algName) == "InsScatterSequenceMesh1DNHR";
    ...
    if (skipLevel1_) {
        HCCL_INFO("[InsV2ScatterSequenceExecutor3Level][CalcRes] myRank[%u] level1 rankSize is 1, skip level1 CalcRes",
            myRank_);
    } else {
        CHK_RET(tempAlgLevel1->CalcRes(comm, param, topoInfo, resReq1));
    }
    if (skipLevel2_) {
        HCCL_INFO("[InsV2ScatterSequenceExecutor3Level][CalcRes] myRank[%u] level2 rankSize is 1, skip level2 CalcRes",
            myRank_);
    } else {
    ...
    HCCL_INFO("[InsV2ScatterSequenceExecutor3Level] myRank[%u] notifyNumOnMainThread is %u", myRank_, resourceRequest.notifyNumOnMainThread);
    resourceRequest.channels.resize(SEQUENCE_EXECUTOR_LEVEL_NUM);
    if (resReq0.channels.empty()) {
        HCCL_ERROR("[InsV2ScatterSequenceExecutor3Level] myRank[%u] channels empty, level0[%u] level2[%u]",
            myRank_, resReq0.channels.size(), resReq2.channels.size());
        return HCCL_E_INTERNAL;
    }
    if (!skipLevel1_ && resReq1.channels.empty()) {
        HCCL_ERROR("[InsV2ScatterSequenceExecutor3Level] myRank[%u] channels empty, level1[%u]",
            myRank_, resReq1.channels.size());
        return HCCL_E_INTERNAL;
    }
    if (!skipLevel2_ && resReq2.channels.empty()) {
        HCCL_ERROR("[InsV2ScatterSequenceExecutor3Level] myRank[%u] channels empty, level2[%u]",
    ...
    HCCL_INFO("[InsV2ScatterSequenceExecutor3Level] myRank[%u] slaveThreadNum is [%u], notifyNumOnMainThread is [%u], "
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult InsV2ScatterSequenceExecutor3Level<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::Orchestrate(
    const OpParam &param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    HCCL_INFO("[InsV2ScatterSequenceExecutor3Level] [Orchestrate] myRank_[%u] rankIdxLevel0_[%u] "
    ...
    skipLevel2_ = std::string(param.algName) == "InsScatterSequenceMesh1DNHR";
    if (skipLevel1_) {
        HCCL_INFO("[InsV2ScatterSequenceExecutor3Level] [Orchestrate] myRank[%u] level1 rankSize is 1, skip level1",
            myRank_);
    }
    if (skipLevel2_) {
        HCCL_INFO("[InsV2ScatterSequenceExecutor3Level] [Orchestrate] myRank[%u] level2 rankSize is 1, skip level2",
            myRank_);
    }
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[InsV2ScatterSequenceExecutor3Level][Orchestrate] myRank[%u] errNo[0x%016llx] "
            "Reduce scatter excutor kernel run failed", myRank_, HCCL_ERROR_CODE(ret)), ret);
    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
void InsV2ScatterSequenceExecutor3Level<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::GenIntraTemplateParams(
    TemplateDataParams &tempAlgParamsIntra, const u64 processedDataCount, const u64 currDataCount, const u64 loop) const
{
    ...
    HCCL_INFO("[InsV2ScatterSequenceExecutor3Level] myRank[%u] loop[%llu] Intra inputSliceStride[%llu] "
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
void InsV2ScatterSequenceExecutor3Level<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::GenInterTemplateParams1(
    TemplateDataParams &tempAlgParamsInter, const u64 processedDataCount, const u64 currDataCount, const u64 loop) const
{
    ...
    HCCL_INFO("[InsV2ScatterSequenceExecutor3Level] myRank[%u] loop[%llu] Inter1 inputSliceStride[%llu] "
        "outputSliceStride[%llu] sliceSize[%llu] inBuffBaseOff[%llu] outBuffBaseOff[%llu] "
        "repeatNum[%llu] inputRepeatStride[%llu] outputRepeatStride[%llu]",
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
void InsV2ScatterSequenceExecutor3Level<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::GenInterTemplateParams2(
    TemplateDataParams &tempAlgParamsInter, const u64 processedDataCount, const u64 currDataCount, const u64 loop) const
{
    tempAlgParamsInter.count = currDataCount;
    tempAlgParamsInter.buffInfo.inBuffBaseOff = 0; // 没有precopy，用不到
    tempAlgParamsInter.buffInfo.outBuffBaseOff = processedDataCount * dataTypeSize_; // 最后拷贝回output的起始点，只与已经处理的数据有关
    tempAlgParamsInter.buffInfo.hcclBuffBaseOff = 0; // level2的输入数据是连续的，所以hcclBuffBaseOff为0
    
    tempAlgParamsInter.sliceSize = currDataCount * dataTypeSize_;
    tempAlgParamsInter.tailSize = tempAlgParamsInter.sliceSize;
 	 
 	tempAlgParamsInter.inputSliceStride = 0; // 没有用到，不需要precopy
    tempAlgParamsInter.outputSliceStride = 0; // 最后拷贝回output时，只有自己的数据，stride为0
    tempAlgParamsInter.repeatNum = 1; // 最后拷贝回output时，只需要拷贝一次。
    tempAlgParamsInter.inputRepeatStride = 0; // 没有用到，不需要precopy

    HCCL_INFO("[InsV2ScatterSequenceExecutor3Level] myRank[%u] loop[%llu] Inter2 inputSliceStride[%llu] "
        "outputSliceStride[%llu] sliceSize[%llu] inBuffBaseOff[%llu] outBuffBaseOff[%llu] "
        "repeatNum[%llu] inputRepeatStride[%llu] outputRepeatStride[%llu]",
        myRank_, loop, tempAlgParamsInter.inputSliceStride, tempAlgParamsInter.outputSliceStride,
        tempAlgParamsInter.sliceSize, tempAlgParamsInter.buffInfo.inBuffBaseOff,
...
HcclResult InsV2ScatterSequenceExecutor3Level<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::GenTempResource
    ...
    const std::shared_ptr<InsAlgTemplate> &algTemplate, TemplateResource &tempResource) const
{
    ...
    if (channelLevelIdx >= remoteRankToChannelInfo_.size()) {
        HCCL_ERROR("[InsV2ScatterSequenceExecutor3Level][GenTempResource] myRank[%u] channelLevelIdx[%u] should be lower"
...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1, typename InsAlgTemplate2>
HcclResult InsV2ScatterSequenceExecutor3Level<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2>::OrchestrateLoop(
    const OpParam &param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    if (maxCountPerLoop == 0) {
        HCCL_ERROR("[InsV2ScatterSequenceExecutor3Level] myRank[%u] maxCountPerLoop is 0, "
...
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_SCATTER,
    InsScatterSequenceMesh1DNHRNHR,
    InsV2ScatterSequenceExecutor3Level,
    TopoMatchMultilevel,
