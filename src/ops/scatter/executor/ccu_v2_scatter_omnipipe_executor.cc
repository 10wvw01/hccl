    ...
    const AlgHierarchyInfoForAllLevel &algHierarchyInfo, AlgResourceRequest &resourceRequest)
{
    ...
    if (rankSizeLevel1_ > 1) {
        InsAlgTempLevel1 algTempLevel1(param, myRank_, subCommRanks1, isSameXAxisAsRoot);
        // 初始化时候，设置subRootId
            
        HCCL_DEBUG("[%s] checkSetRoot=%u param.root=%u rankSizeLevel0_=%u rankIdxLevel0_=%u", __func__,
            param.root / rankSizeLevel0_ * rankSizeLevel0_ + rankIdxLevel0_, param.root, rankSizeLevel0_, rankIdxLevel0_);
...
{
    // TODOv 临时写法，待改进，两层边三层 [AICPU 使用这块]
    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    // void *bt[32];
    // int btSize = backtrace(bt, 32);
    // char **btSymbols = backtrace_symbols(bt, btSize);
    // HCCL_INFO("[%s] Call stack:", __func__);
    // for (int i = 0; i < btSize; i++) {
    //     HCCL_INFO("  #%d %s", i, btSymbols ? btSymbols[i] : "???");
    // }
    // free(btSymbols);
    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    if (rankSizeLevel1_ > 1) {
        algTempY = InsAlgTempLevel1(param, myRank_, subCommRanks1, isSameXAxisAsRoot);
        // TODO: 循环里面初始化下
    ...
    if (dataCount_ > maxCountPerLoop && dataCount_ % maxCountPerLoop != 0) {
        ...
        HCCL_INFO("[%s] myRank[%u] tailCount[%u]", __func__, myRank_, tailCount);
    ...
    for(int i=0;i<alignSliceInfo.dataSliceLevel0.size();i++){
        for(int j=0;j<alignSliceInfo.dataSliceLevel0[i].inputOmniPipeSliceStride.size();j++){
            for(int k=0;k<alignSliceInfo.dataSliceLevel0[i].inputOmniPipeSliceStride[j].size();k++){
                HCCL_DEBUG("[jjy][dataSliceLevel0]myRank[%d][inputOmniPipeSliceStride][%d][%d][%d]:%d",myRank_,i,j,k,alignSliceInfo.dataSliceLevel0[i].inputOmniPipeSliceStride[j][k]);
            }
        } 
    }
    for(int i=0;i<alignSliceInfo.dataSliceLevel1.size();i++){
        for(int j=0;j<alignSliceInfo.dataSliceLevel1[i].inputOmniPipeSliceStride.size();j++){
            for(int k=0;k<alignSliceInfo.dataSliceLevel1[i].inputOmniPipeSliceStride[j].size();k++){
                HCCL_DEBUG("[jjy][dataSliceLevel1]myRank[%d][inputOmniPipeSliceStride][%d][%d][%d]:%d",myRank_,i,j,k,alignSliceInfo.dataSliceLevel1[i].inputOmniPipeSliceStride[j][k]);
            }
        } 
    }
    ...
    HCCL_INFO("[%s] myRank_[%u] End", __func__, myRank_);
    return HcclResult::HCCL_SUCCESS;
}
