    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    HCCL_DEBUG("[%s]localThreads_ size[%u] dataSize_[%u] dataCount_[%u]", __func__, localThreads_.size(), dataSize_, dataCount_); // 3 main+x+y
    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
...
#else
    endpointAttrBwAvg = {141.0, 175.0, 1.0};
    const char* bwRatioXEnv = std::getenv("OMNIPIPE_BW_X");
    const char* bwRatioYEnv = std::getenv("OMNIPIPE_BW_Y");
    if (bwRatioXEnv != nullptr && bwRatioXEnv[0] != '\0') {
        try { endpointAttrBwAvg[0] = std::stod(std::string(bwRatioXEnv)); }
        catch (...) { HCCL_WARNING("[%s] invalid OMNIPIPE_BW_X[%s]", __func__, bwRatioXEnv); }
    }
    if (bwRatioYEnv != nullptr && bwRatioYEnv[0] != '\0' && rankSizeLevel1_ > 1) {
        try { endpointAttrBwAvg[1] = std::stod(std::string(bwRatioYEnv)) / (rankSizeLevel1_ - 1); }
        catch (...) { HCCL_WARNING("[%s] invalid OMNIPIPE_BW_Y[%s]", __func__, bwRatioYEnv); }
    }

    HCCL_INFO("[%s] myRank[%u] endpointAttrBwAvg[%.3f, %.3f, %.3f]", __func__, myRank_,
        endpointAttrBwAvg[0], endpointAttrBwAvg[1], endpointAttrBwAvg[2]);
#endif
    ...
    HCCL_DEBUG("[%s] myRank[%u] maxTmpMemSize[%u] transportBoundDataSize[%u]", __func__,
        myRank_, maxTmpMemSize, transportBoundDataSize);
    u64 maxCountPerLoop = std::max(transportBoundDataSize, maxTmpMemSize) / HCCL_MIN_SLICE_ALIGN
    ...
    scratchParam.engine = CommEngine::COMM_ENGINE_CCU;
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        for (auto i = 0; i < innerServerStepNum; ++i) {
            ...
            // - 同y轴非root节点：往x轴方向发送部分转发数据
                ...
                if (endpointAttrBwAvg[0] <= endpointAttrBwAvg[1]) {
                    ...
                    if (isSameXAxisAsRoot && rankSizeLevel1_ > 1) {
                        CHK_RET(GenTempAlgParamsHCCLBuff2HCCLBuff(
                            tempAlgParamsX, currentSliceInfo.dataSliceLevel1[i], processedDataCount, resCtx, param));
                    }
                }   
            }
            // 执行X维度通信
        ...
        HCCL_INFO("[%s] myRank[%u] localCopy inBuffBaseOff[%lu] outBuffBaseOff[%lu] sliceSize[%lu] outputSize[%u] "
                  "inputSize[%u]",
