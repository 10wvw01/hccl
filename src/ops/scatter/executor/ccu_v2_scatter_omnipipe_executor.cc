    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    HCCL_INFO("[%s] myRank[%u] maxTmpMemSize_[%u] transportBoundDataSize[%u]", __func__,
        myRank_, maxTmpMemSize_, transportBoundDataSize);
    u64 maxCountPerLoop = std::min(transportBoundDataSize, resCtx.cclMem.size) / HCCL_MIN_SLICE_ALIGN
