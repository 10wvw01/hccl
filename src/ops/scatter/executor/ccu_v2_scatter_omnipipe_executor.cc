    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    // u64 maxCountPerLoop = std::min(transportBoundDataSize, scratchBoundDataSize) / dataTypeSize_ / 2; // 256M
    // maxCountPerLoop = dataSize_ / dataTypeSize_ / 2;  //设一个小一点的maxCountPerLoop，方便测试多loop
    ...
    tempAlgParamsLocalCopy.buffInfo.inBuffBaseOff = myRank_ * perLoopSize; // TODO 切分下rankSize
