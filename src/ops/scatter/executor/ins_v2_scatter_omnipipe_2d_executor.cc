    ...
    for (auto i = 0; i < algHierarchyInfo.infos.size(); ++i) {
        for (auto j = 0; j < algHierarchyInfo.infos[i].size(); ++j) {
            for (auto k = 0; k < algHierarchyInfo.infos[i][j].size(); ++k) {
                HCCL_DEBUG("[%s] myRank[%u] (%d, %d, %d) %u", __func__, userrank, i, j, k,
    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    u64 maxCountPerLoop = std::max(maxTmpMemSize, transportBoundDataSize) / HCCL_MIN_SLICE_ALIGN * HCCL_MIN_SLICE_ALIGN
                          / dataTypeSize_ / rankSize_;
