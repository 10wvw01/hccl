    ...
    for (auto& signalInfo : ccuSendRecvInfo_) {
        ...
        if (signalInfo.optype == OP_SEND_RECV_WRITE) { // WriteNb
            for (uint32_t i = 0; i < signalInfo.srcSliceInfo.size(); i++) {
                ...
                remoteDst.token = token_[signalInfo.remoteRank];
                ...
                WriteNb(rankId2Channel_[signalInfo.remoteRank], remoteDst, src, sliceSize_, event_);
                WaitEvent(event_);
            }
        }
        ...
        if (signalInfo.optype == OP_SEND_RECV_WRITE_REDUCE) { // WriteReduceNb
            for (uint32_t i = 0; i < signalInfo.srcSliceInfo.size(); i++) {
                ...
                event_.SetMask(1 << rankId_);
                WriteReduceNb(rankId2Channel_[signalInfo.remoteRank], remoteDst, src, sliceSize_, signalInfo.inputDataType, signalInfo.reduceType, event_);
                WaitEvent(event_);
            }
        }
        ...
        if (signalInfo.optype == OP_SEND_RECV_READ) { // ReadNb
            for (uint32_t i = 0; i < signalInfo.srcSliceInfo.size(); i++) {
                ...
                event_.SetMask(1 << rankId_);
                ReadNb(rankId2Channel_[signalInfo.remoteRank], src, remoteDst, sliceSize_, event_);
                WaitEvent(event_);
            }
        }
        ...
        if (signalInfo.optype == OP_SEND_RECV_READ_REDUCE) { // ReadReduceNb
            for (uint32_t i = 0; i < signalInfo.srcSliceInfo.size(); i++) {
                ...
                event_.SetMask(1 << rankId_);
                ReadReduceNb(rankId2Channel_[signalInfo.remoteRank], src, remoteDst, sliceSize_, signalInfo.inputDataType, signalInfo.reduceType, event_);
                WaitEvent(event_);
            }
        }
...
std::vector<uint64_t> CcuKernelOmni::GeneArgs(const CcuTaskArg &arg)
{
    ...
    HCCL_INFO("[CcuKernelOmni] RankId[%u], inputAddr[%llu], outputAddr[%llu], sliceSize[%llu], "
              "repeatNum[%llu], inputRepeatStride[%llu], outputRepeatStride[%llu].", rankId_, 
              inputAddr, outputAddr, sliceSize, repeatNum, inputRepeatStride, outputRepeatStride);
