                                                      ...
                                                      AlgResourceRequest& resourceRequest, const XmlInfo& xmlInfo)
{
    ...
    for (auto& sendRecvInfo : xmlInfo.vecSendRecvInfo) {
        if (std::find(rankGroup_[0].begin(), rankGroup_[0].end(), sendRecvInfo.remoteRank) != rankGroup_[0].end()) {
            tmpSendRecvInfo[0].push_back(sendRecvInfo);
        } else if (std::find(rankGroup_[1].begin(), rankGroup_[1].end(), sendRecvInfo.remoteRank) != rankGroup_[1].end()){
            tmpSendRecvInfo[1].push_back(sendRecvInfo);
        } else {
            HCCL_ERROR("[CalcRes] sendRecvInfo.remoteRank is %u not in rankGroup", sendRecvInfo.remoteRank);
        }
    }
