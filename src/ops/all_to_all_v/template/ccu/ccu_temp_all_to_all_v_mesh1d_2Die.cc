                                                         ...
                                                         std::map<u32, std::vector<HcclChannelDesc>>& rankIdToChannelDesc)
{
    ...
    if (is2Plus6_) {
        ...
        if (dropClos2) {
            ...
            for (auto& pair : multiChByDie) {
                if (pair.first != fullmeshDieId_) {
                    fillKernel(KERNEL_CLOS_MAJOR, pair.second);
                }
            }
        } else {
            kernelCount_ = 3;
    ...
    HCCL_INFO("[CcuTempAlltoAllVMesh1D2Die][PartitionChannels] Rank[%d], is2Plus6[%d], dropClos2[%d], kernelCount[%u], "
        "fullmeshRankGroup[%zu], closMajorRankGroup[%zu], closMinorRankGroup[%zu].",
        myRank_, is2Plus6_, dropClos2, kernelCount_, kernelRankGroup_[KERNEL_FULLMESH].size(),
