    ...
    const TopoInfoWithNetLayerDetails* topoInfo, AlgResourceRequest& resourceRequest)
{
    ...
    bool needSwap = (closBwCoeff_[0] < closBwCoeff_[1]);
    CHK_RET(ReverseChannelPerDieIfNeed(comm, myRank_, channels_));
    if (needSwap) {
        std::swap(rankGroup_[0], rankGroup_[1]);
        std::swap(closBwCoeff_[0], closBwCoeff_[1]);
    }
    closMinorDieId_ = (closBwCoeff_[0] <= closBwCoeff_[1]) ? 0 : 1;
    closMajorDieId_ = 1 - closMinorDieId_;
