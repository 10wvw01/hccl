...
private:
    ...
    HcclResult LocalCopyForMyRank(const TemplateDataParams &tempAlgParams, const ThreadHandle &thread,
        const u32 myAlgRank) const;
    HcclResult PreCopyForRemoteRank(const TemplateDataParams &tempAlgParams, const ThreadHandle &thread,
        const u32 remoteAlgRank, const u64 sendCount) const;
    ...
    HcclResult RunSendRecv(const SendRecvInfo &sendRecvInfo, const DataInfo &sendInfo, const DataInfo &recvInfo,
        const ThreadHandle& thread, const u64 sendCount, const u64 recvCount) const;
    ...
    u64 CalcCclBuffOffset(const u32 algRank, const TemplateDataParams &tempAlgParams) const;
