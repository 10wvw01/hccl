    ...
    const std::map<u32, std::vector<ChannelInfo>> &channels)
{
    ...
    u64 actualChunkSize = (tempAlgParams_.sliceSize + rankSize_ - 1) / rankSize_;
    ...
    u64 outputOffset = isAlltoAllV ? tempAlgParams_.rdispls[myRank_] * dataTypeSize :
    ...
    if (localByteSize > 0) {
        ...
        DataSlice dstSlice(tempAlgParams_.buffInfo.outputPtr, outputOffset, localByteSize, localCount);
    ...
    u32 connectedRank = peerGroup * meshSize_ + connectedLocalRank;
        ...
        if (dmaResult != HCCL_SUCCESS) {
            HCCL_ERROR("[ALLTOALL_V2_DEBUG][MeshClos] send/recv FAILED: linkIdx=%u connectedRank=%u err=0x%x "
                       "myRank=%d templateRank=%u actualChunkSize=%llu txByteSize=%llu",
                       linkIdx, connectedRank, dmaResult, myRank_, templateRankSize_, actualChunkSize, txByteSize);
            return dmaResult;
        }
