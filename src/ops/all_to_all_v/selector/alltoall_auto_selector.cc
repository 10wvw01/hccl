                                                   ...
                                                   std::string &selectAlgName) const
{
    ...
    u64* sendCounts = reinterpret_cast<u64*>(opParam.all2AllVDataDes.sendCounts);
    u64 perRankDataSize = sendCounts[0] * dataTypeSize;
    u64 totalSize = perRankDataSize * topoInfo->userRankSize;
    if (totalSize > AIV_MAX_PER_RANK_DATA_SIZE * topoInfo->userRankSize) {
        HCCL_AIV_NOT_MATCH_LOG(opParam, HCCL_DEBUG, "[AlltoAllAutoSelector][%s] totalSize[%llu] larger than AIV_MAX_PER_RANK_DATA_SIZE[%llu] * rankSize[%u]",
