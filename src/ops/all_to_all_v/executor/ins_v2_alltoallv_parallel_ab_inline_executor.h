...
#ifndef HCCLV2_INS_V2_ALLTOALLV_PARALLEL_AB_INLINE_EXECUTOR_H
#define HCCLV2_INS_V2_ALLTOALLV_PARALLEL_AB_INLINE_EXECUTOR_H
...
class InsV2AlltoAllVParallelABInlineExecutor : public InsCollAlgBase {
public:
    explicit InsV2AlltoAllVParallelABInlineExecutor() = default;
    ~InsV2AlltoAllVParallelABInlineExecutor() override = default;
...
private:
    ...
    HcclResult SplitParams(const TemplateDataParams &baseParams, double ratio, TemplateDataParams &meshParams,
    ...
    HcclResult RunInlineTemplates(const OpParam &param, const AlgResourceCtxSerializable &resCtx,
                                  const TemplateDataParams &meshParams,
                                  const TemplateDataParams &aClosParams,
                                  const TemplateDataParams &bClosParams);
    ...
    bool IsIntraPeer(u32 peerRank) const;
    ...
    std::map<u32, std::vector<ChannelInfo>> aClosLinkMap_;
    std::map<u32, std::vector<ChannelInfo>> bClosLinkMap_;
    ...
    std::vector<ThreadHandle> meshThreads_;
    std::vector<ThreadHandle> aClosThreads_;
    std::vector<ThreadHandle> bClosThreads_;

    TemplateResMeta meshMeta_;
    TemplateResMeta aClosMeta_;
    TemplateResMeta bClosMeta_;
};

} // namespace ops_hccl

#endif // HCCLV2_INS_V2_ALLTOALLV_PARALLEL_AB_INLINE_EXECUTOR_H
