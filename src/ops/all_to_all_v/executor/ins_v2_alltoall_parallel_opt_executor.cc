    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    if (IsAlltoAllNoMemcpyAlg(param)) {
        CHK_PRT_RET(param.opType != HcclCMDType::HCCL_CMD_ALLTOALL &&
                        param.opType != HcclCMDType::HCCL_CMD_ALLTOALLV,
                    HCCL_ERROR("[ALLTOALL_NO_MEMCPY][Orchestrate] only alltoall/alltoallv is supported. opType=%d",
                               param.opType),
    ...
    InsAlgTemplate1 &tempAlgInter, bool hasInterComm)
{
...

    {
        ...
        if (param.opType == HcclCMDType::HCCL_CMD_ALLTOALLV) {
            const u64 rankSize = resCtx.topoInfo.userRankSize;
            const u64 *sendCounts = reinterpret_cast<const u64 *>(param.all2AllVDataDes.sendCounts);
            ...
            tempAlgParams.sendCounts.resize(rankSize, 0);
...
REGISTER_EXECUTOR_BY_TWO_TEMPS(
    ...
    InsAlltoAllVParallelMesh2DClosV3NoMemcpy,
    InsV2AlltoAllParallelOptExecutor,
    TopoMatchUBX_V2,
    InsTempAlltoAllMesh2DV3NoMemcpy,
    InsTempAlltoAllMeshClosV3NoMemcpy);
...
REGISTER_EXECUTOR_BY_TWO_TEMPS(
    HcclCMDType::HCCL_CMD_ALLTOALLV,
    InsAlltoAllVParallelMesh2DClosV3NoMemcpyPodUbxV2,
    InsV2AlltoAllParallelOptExecutor,
    TopoMatchUBX_V2,
    InsTempAlltoAllMesh2DV3NoMemcpy,
    InsTempAlltoAllMeshClosV3NoMemcpy);
...
REGISTER_EXECUTOR_BY_TWO_TEMPS(
    HcclCMDType::HCCL_CMD_ALLTOALLV,
    InsAlltoAllVParallelMesh2DClosV3NoMemcpyPodDirect,
    InsV2AlltoAllParallelOptExecutor,
    TopoMatchAlltoAllPodDirect,
    InsTempAlltoAllMesh2DV3NoMemcpy,
    InsTempAlltoAllMeshClosV3NoMemcpy);
