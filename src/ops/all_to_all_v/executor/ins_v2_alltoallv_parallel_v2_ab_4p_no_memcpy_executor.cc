...
#include "ins_v2_alltoallv_parallel_v2_ab_4p_no_memcpy_executor.h"
#include <algorithm>
#include <cstring>
#include "ins_temp_all_to_all_v_v2_ab_4p_no_memcpy.h"
...
constexpr double DEFAULT_AB_RATIO = 0.75;
constexpr double DEFAULT_PATH_SPLIT_RATIO = 0.5;
constexpr double MIN_SPLIT_RATIO = 0.0;
constexpr double MAX_SPLIT_RATIO = 1.0;

bool IsA2AVV2AB4PNoMemcpyAlg(const OpParam &param)
{
    return std::strcmp(param.algName, "InsAlltoAllVParallelMesh2DClosV2AB4PlaneSprayNoMemcpy") == 0 ||
...
bool IsA2AVV2AB4PColorAlg(const OpParam &param)
{
    return std::strcmp(param.algName, "InsAlltoAllVParallelMesh2DClosV2AB4PlaneColorNoMemcpy") == 0 ||
           std::strcmp(param.algName, "InsAlltoAllVParallelMesh2DClosV2AB4PlaneColorNoMemcpyPodUbxV2") == 0 ||
           std::strcmp(param.algName, "InsAlltoAllVParallelMesh2DClosV2AB4PlaneColorNoMemcpyPodDirect") == 0;
}
...
void SplitABPairCountByRatio(u64 count, double ratio, u64 &aCount, u64 &bCount)
{
    long double scaled = static_cast<long double>(count) * static_cast<long double>(ratio);
...
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::CalcAlgHierarchyInfo(
...
template <typename AlgTopoMatch>
u64 InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::GetRankSize(
...
template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::BuildHierarchyInfo(
    const TopoInfoWithNetLayerDetails *topoInfo, const AlgHierarchyInfoForAllLevel &algHierarchyInfo)
{
    ...
    CHK_PRT_RET(algHierarchyInfo.infos.empty() || algHierarchyInfo.infos[0].empty(),
                HCCL_ERROR("[A2AV_V2_AB_4P][BuildHierarchyInfo] invalid hierarchy."),
    ...
    if (IsSingleLayerUbx4x2(topoInfo)) {
        ...
        CHK_PRT_RET(algHierarchyInfo.infos[0].size() < 2 || algHierarchyInfo.infos[0][0].empty(),
                    HCCL_ERROR("[A2AV_V2_AB_4P][BuildHierarchyInfo] invalid MESH_1D_CLOS hierarchy."),
                    HcclResult::HCCL_E_INTERNAL);
        ...
        CHK_PRT_RET(intraHierarchyInfo_.empty() || algHierarchyInfo.infos.size() < 2 ||
                    ...
                    HCCL_ERROR("[A2AV_V2_AB_4P][BuildHierarchyInfo] invalid direct hierarchy."),
                    HcclResult::HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(meshSize_ == 0 || rankSize_ % meshSize_ != 0,
                HCCL_ERROR("[A2AV_V2_AB_4P][BuildHierarchyInfo] invalid mesh/rank. rankSize=%llu mesh=%llu",
                           rankSize_, meshSize_),
                HcclResult::HCCL_E_INTERNAL);
    groupNum_ = rankSize_ / meshSize_;
    HCCL_WARNING("[A2AV_V2_AB_4P][BuildHierarchyInfo] rank=%u rankSize=%llu mesh=%llu group=%llu",
                 topoInfo->userRank, rankSize_, meshSize_, groupNum_);
    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::CalcRes(
    HcclComm comm, const OpParam &param, const TopoInfoWithNetLayerDetails *topoInfo,
    const AlgHierarchyInfoForAllLevel &algHierarchyInfo, AlgResourceRequest &resourceRequest)
{
    resourceRequest = AlgResourceRequest{};
    CHK_PRT_RET(param.opType != HcclCMDType::HCCL_CMD_ALLTOALLV || !IsA2AVV2AB4PNoMemcpyAlg(param),
                HCCL_ERROR("[A2AV_V2_AB_4P][CalcRes] unsupported op/alg. opType=%d alg=%s",
    ...
    CHK_PRT_RET(meshChannels.empty() || closChannels.empty(),
                HCCL_ERROR("[A2AV_V2_AB_4P][CalcRes] empty channel request. mesh=%zu clos=%zu",
    ...
    bMode_ = IsA2AVV2AB4PColorAlg(param) ? A2AVV2AB4PBMode::COLOR : A2AVV2AB4PBMode::SPRAY;
    if (bMode_ == A2AVV2AB4PBMode::COLOR) {
        bMeta_.slaveThreadNum = 1;
    } else if (bMode_ == A2AVV2AB4PBMode::SPRAY) {
        bMeta_.slaveThreadNum = static_cast<u32>(rankSize_ - 1);
    }
    bMeta_.notifyNumOnMainThread = bMeta_.slaveThreadNum;
    bMeta_.notifyNumPerThread.assign(bMeta_.slaveThreadNum, 1);

    resourceRequest.notifyNumOnMainThread = TEMPLATE_NUM;
    resourceRequest.slaveThreadNum = stage0Meta_.slaveThreadNum + stage1Meta_.slaveThreadNum +
        bMeta_.slaveThreadNum + TEMPLATE_NUM;
    ...
    resourceRequest.notifyNumPerThread.emplace_back(bMeta_.notifyNumOnMainThread + 1);
    resourceRequest.notifyNumPerThread.insert(resourceRequest.notifyNumPerThread.end(),
                                              bMeta_.notifyNumPerThread.begin(),
                                              bMeta_.notifyNumPerThread.end());
    resourceRequest.channels.emplace_back(meshChannels);
    resourceRequest.channels.emplace_back(closChannels);
    HCCL_WARNING("[A2AV_V2_AB_4P][CalcRes] rank=%u stage0Slave=%u stage1Slave=%u bSlave=%u totalThreads=%u "
                 "meshChannels=%zu closChannels=%zu maxPeer=%u bMode=%d",
                 myRank_, stage0Meta_.slaveThreadNum, stage1Meta_.slaveThreadNum, bMeta_.slaveThreadNum,
                 resourceRequest.slaveThreadNum, meshChannels.size(), closChannels.size(), maxPeerNum,
                 static_cast<int>(bMode_));
    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::RestoreChannelMaps(
    const AlgResourceCtxSerializable &resCtx)
{
    ...
    CHK_PRT_RET(remoteRankToChannelInfo_.size() < 2,
                HCCL_ERROR("[A2AV_V2_AB_4P][RestoreChannelMaps] expected 2 channel levels, got %zu.",
    ...
    for (const auto &item : allLinkMap_) {
        ...
        CHK_PRT_RET(!channel.hasRemoteAlltoAllVInfo,
                    HCCL_ERROR("[A2AV_V2_AB_4P][RestoreChannelMaps] missing remote a2av info. rank=%u peer=%u",
                               myRank_, item.first),
                    HcclResult::HCCL_E_INTERNAL);
        ...
        HCCL_WARNING("[A2AV_V2_AB_4P][RestoreChannelMaps] rank=%u peer=%u links=%zu remoteMax=%llu",
...
template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::BuildRuntimeMetas()
{
    ...
    CHK_PRT_RET(stage0PeerNum > maxPeerNum || stage1PeerNum > maxPeerNum,
                HCCL_ERROR("[A2AV_V2_AB_4P][BuildRuntimeMetas] active peers exceed resource upper bound. "
    ...
    if (bMode_ == A2AVV2AB4PBMode::COLOR) {
        ...
        bMeta_.slaveThreadNum = static_cast<u32>(bLinkMap_.size());
    }
    bMeta_.notifyNumOnMainThread = bMeta_.slaveThreadNum;
    bMeta_.notifyNumPerThread.assign(bMeta_.slaveThreadNum, 1);
    HCCL_WARNING("[A2AV_V2_AB_4P][BuildRuntimeMetas] rank=%u stage0Peers=%u stage1Peers=%u bPeers=%zu "
                 "bSlave=%u bMode=%d",
                 myRank_, stage0PeerNum, stage1PeerNum, bLinkMap_.size(), bMeta_.slaveThreadNum,
                 static_cast<int>(bMode_));
    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch>
bool InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::IsSameGroup(u32 rankA, u32 rankB) const
{
    return meshSize_ != 0 && rankA / meshSize_ == rankB / meshSize_;
}

template <typename AlgTopoMatch>
double InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::GetABRatio(const OpParam &param) const
{
    double ratio = param.opConfig.multipleDimensionSplitRatio;
    if (ratio < MIN_SPLIT_RATIO || ratio > MAX_SPLIT_RATIO) {
        HCCL_WARNING("[A2AV_V2_AB_4P][ABRatio] invalid ratio[%f], use default[%f].",
                     ratio, DEFAULT_AB_RATIO);
        return DEFAULT_AB_RATIO;
    }
    HCCL_WARNING("[A2AV_V2_AB_4P][ABRatio] use ratio[%f] from opConfig.", ratio);
    return ratio;
}

template <typename AlgTopoMatch>
double InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::GetPathSplitRatio(
    const OpParam &param) const
{
    (void)param;
    return DEFAULT_PATH_SPLIT_RATIO;
}

template <typename AlgTopoMatch>
void InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::SplitABPairCount(
    u64 count, u64 &aCount, u64 &bCount) const
{
    SplitABPairCountByRatio(count, abRatio_, aCount, bCount);
}

template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::AddStageLink(
    u32 peerRank, std::map<u32, std::vector<ChannelInfo>> &stageLinkMap) const
{
    ...
    HCCL_ERROR("[A2AV_V2_AB_4P][AddStageLink] missing channel. rank=%u peer=%u", myRank_, peerRank);
    return HcclResult::HCCL_E_INTERNAL;
}

template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::BuildStageLinkMaps(
    const TemplateDataParams &params)
{
    ...
    auto addStage0Relay = [this](u32 srcRank, u32 dstRank, u64 count) -> HcclResult {
        ...
        (void)bCount;
        SplitV2PairCount(aCount, pathSplitRatio_, part0, part1);
    ...
    HCCL_WARNING("[A2AV_V2_AB_4P][BuildStageLinkMaps] rank=%u stage0Peers=%zu stage1Peers=%zu",
                 myRank_, stage0LinkMap_.size(), stage1LinkMap_.size());
    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::SelectFourthClosChannel(
    u32 peerRank, std::vector<ChannelInfo> &channels) const
{
    auto it = interLinkMap_.find(peerRank);
    CHK_PRT_RET(it == interLinkMap_.end() || it->second.empty(),
                HCCL_ERROR("[A2AV_V2_AB_4P][SelectFourthClosChannel] missing fourth-plane peer. rank=%u peer=%u",
                           myRank_, peerRank),
                HcclResult::HCCL_E_INTERNAL);
    channels.clear();
    if (it->second.size() >= 4) {
        channels.push_back(it->second[3]);
    } else {
        channels.push_back(it->second.back());
    }
    HCCL_WARNING("[A2AV_V2_AB_4P][SelectFourthClosChannel] rank=%u peer=%u inputLinks=%zu selectedIdx=%zu",
                 myRank_, peerRank, it->second.size(), it->second.size() >= 4 ? 3UL : it->second.size() - 1);
    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::BuildBLinkMap()
{
    bLinkMap_.clear();
    for (u32 peerRank = 0; peerRank < rankSize_; ++peerRank) {
        if (peerRank == myRank_) {
            continue;
        }
        CHK_RET(SelectFourthClosChannel(peerRank, bLinkMap_[peerRank]));
    }
    HCCL_WARNING("[A2AV_V2_AB_4P][BuildBLinkMap] rank=%u peers=%zu bMode=%d",
                 myRank_, bLinkMap_.size(), static_cast<int>(bMode_));
    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::PrepareTemplateResources(
    const AlgResourceCtxSerializable &resCtx)
{
    ...
    u32 required = 1 + stage0ThreadNum + stage1ThreadNum + bThreadNum;
    CHK_PRT_RET(threads_.size() < required,
                HCCL_ERROR("[A2AV_V2_AB_4P][PrepareTemplateResources] threads[%zu] < required[%u].",
    ...
    HCCL_WARNING("[A2AV_V2_AB_4P][PrepareTemplateResources] rank=%u totalThreads=%zu required=%u "
                 "stage0=%u stage1=%u b=%u",
                 myRank_, threads_.size(), required, stage0ThreadNum, stage1ThreadNum, bThreadNum);
    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::BuildBaseParams(
    const OpParam &param, const AlgResourceCtxSerializable &resCtx, TemplateDataParams &params)
{
    ...
    for (u64 i = 0; i < rankSize_; ++i) {
        ...
        CHK_PRT_RET(i >= remoteCountInfoValid_.size() || !remoteCountInfoValid_[i],
                    HCCL_ERROR("[A2AV_V2_AB_4P][BaseParam] missing remote count info. rank=%u peer=%llu",
                               myRank_, i),
                    HcclResult::HCCL_E_INTERNAL);
    }
    return HCCL_SUCCESS;
...
template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::GetPairCountFromRecvMatrix(
    const TemplateDataParams &params, u32 srcRank, u32 dstRank, u64 &count) const
{
    ...
    if (dstRank == myRank_) {
        CHK_PRT_RET(srcRank >= params.recvCounts.size(),
                    HCCL_ERROR("[A2AV_V2_AB_4P][PairCount] invalid local src. rank=%u src=%u",
                               myRank_, srcRank),
                    HcclResult::HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(it == allLinkMap_.end() || it->second.empty() ||
                ...
                HCCL_ERROR("[A2AV_V2_AB_4P][PairCount] missing remote recv count. "
...
template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::GetGlobalMaxSendCount(
...
template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::BuildABExactSlotOffsets(
    TemplateDataParams &params)
{
    ...
    HCCL_WARNING("[A2AV_V2_AB_4P][ExactSlot] rank=%u compactBytes=%llu slotNum=%llu rankSize=%llu "
...
template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::CheckScratch(
    const AlgResourceCtxSerializable &resCtx) const
{
    CHK_PRT_RET(exactSlotBytes_ > resCtx.cclMem.size,
                HCCL_ERROR("[A2AV_V2_AB_4P][Scratch] insufficient ccl buffer. rank=%u required=%llu "
...
template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::RunAAndBParallel(
...
{
    InsTempAlltoAllVV2AB4PNoMemcpy stage0Temp(param, resCtx.topoInfo.userRank, intraHierarchyInfo_);
    stage0Temp.SetV2AB4PNoMemcpyInfo(A2AVV2AB4PPhase::STAGE0_TO_RELAY,
                                     static_cast<u32>(rankSize_), static_cast<u32>(meshSize_), slotStride_,
                                     abRatio_, pathSplitRatio_);
    InsTempAlltoAllVV2AB4PNoMemcpy stage1Temp(param, resCtx.topoInfo.userRank, interHierarchyInfo_);
    stage1Temp.SetV2AB4PNoMemcpyInfo(A2AVV2AB4PPhase::STAGE1_TO_OUTPUT,
                                     static_cast<u32>(rankSize_), static_cast<u32>(meshSize_), slotStride_,
                                     abRatio_, pathSplitRatio_);
    InsTempAlltoAllVV2AB4PNoMemcpy bTemp(param, resCtx.topoInfo.userRank, interHierarchyInfo_);
    A2AVV2AB4PPhase bPhase = bMode_ == A2AVV2AB4PBMode::COLOR ?
        A2AVV2AB4PPhase::B_COLOR_TO_OUTPUT : A2AVV2AB4PPhase::B_SPRAY_TO_OUTPUT;
    bTemp.SetV2AB4PNoMemcpyInfo(bPhase, static_cast<u32>(rankSize_), static_cast<u32>(meshSize_), slotStride_,
                                abRatio_, pathSplitRatio_);

    TemplateResource stage0Res;
    stage0Res.channels = stage0LinkMap_;
    stage0Res.threads = stage0Threads_;
    stage0Res.aivCommInfoPtr = resCtx.aivCommInfoPtr;
    TemplateResource stage1Res;
    stage1Res.channels = stage1LinkMap_;
    stage1Res.threads = stage1Threads_;
    stage1Res.aivCommInfoPtr = resCtx.aivCommInfoPtr;
    TemplateResource bRes;
    bRes.channels = bLinkMap_;
    bRes.threads = bThreads_;
    bRes.aivCommInfoPtr = resCtx.aivCommInfoPtr;

    HCCL_WARNING("[A2AV_V2_AB_4P][RunAAndBParallel] rank=%u fullCountNoSelfA=%llu fullCountNoSelfB=%llu "
                 "bMode=%d submitOrder=B,A0,A1",
                 myRank_, aParams.count, bParams.count, static_cast<int>(bMode_));
    CHK_RET(PreSyncInterThreads(mainThread_, {stage0Threads_[0], stage1Threads_[0], bThreads_[0]},
                                {stage0Meta_.notifyNumOnMainThread,
                                 stage1Meta_.notifyNumOnMainThread,
                                 bMeta_.notifyNumOnMainThread}));
    HcclResult bRet = bTemp.KernelRun(param, bParams, bRes);
    HcclResult stage0Ret = stage0Temp.KernelRun(param, aParams, stage0Res);
    HcclResult stage1Ret = HCCL_SUCCESS;
    if (stage0Ret == HCCL_SUCCESS) {
        stage1Ret = stage1Temp.KernelRun(param, aParams, stage1Res);
    }
    HcclResult postRet = PostSyncInterThreads(mainThread_, {stage0Threads_[0], stage1Threads_[0], bThreads_[0]},
                                              {0, 1, 2});
    CHK_RET(stage0Ret);
    CHK_RET(stage1Ret);
    CHK_RET(bRet);
...
template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::CopySelfToOutputAfterCommunication(
    const TemplateDataParams &params)
{
    if (myRank_ >= params.sendCounts.size() || params.sendCounts[myRank_] == 0) {
        return HCCL_SUCCESS;
    }
    ...
    CHK_PRT_RET(srcOffset + byteSize > params.buffInfo.inputSize ||
                    dstOffset + byteSize > params.buffInfo.outputSize,
                HCCL_ERROR("[A2AV_V2_AB_4P][SelfCopyFinal] range overflow. rank=%u count=%llu "
                           "srcOff=%llu dstOff=%llu bytes=%llu inputSize=%llu outputSize=%llu",
                           myRank_, count, srcOffset, dstOffset, byteSize, params.buffInfo.inputSize,
                           params.buffInfo.outputSize),
                HcclResult::HCCL_E_INTERNAL);
    ...
    HCCL_WARNING("[A2AV_V2_AB_4P][SelfCopyFinal] rank=%u count=%llu srcOff=%llu dstOff=%llu",
                 myRank_, count, srcOffset, dstOffset);
    CHK_RET(static_cast<HcclResult>(LocalCopy(mainThread_, srcSlice, dstSlice)));
    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch>
HcclResult InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor<AlgTopoMatch>::Orchestrate(
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    HCCL_WARNING("[A2AV_V2_AB_4P][Orchestrate] start alg=%s", param.algName);
    CHK_PRT_RET(param.opType != HcclCMDType::HCCL_CMD_ALLTOALLV || !IsA2AVV2AB4PNoMemcpyAlg(param),
                HCCL_ERROR("[A2AV_V2_AB_4P][Orchestrate] unsupported op/alg. opType=%d alg=%s",
                           param.opType, param.algName),
                HcclResult::HCCL_E_NOT_SUPPORT);
    ...
    CHK_PRT_RET(dataType_ >= HCCL_DATA_TYPE_RESERVED || dataTypeSize_ == 0,
                HCCL_ERROR("[A2AV_V2_AB_4P][Orchestrate] invalid dataType=%d", static_cast<int>(dataType_)),
                HcclResult::HCCL_E_INTERNAL);
    abRatio_ = GetABRatio(param);
    pathSplitRatio_ = GetPathSplitRatio(param);
    bMode_ = IsA2AVV2AB4PColorAlg(param) ? A2AVV2AB4PBMode::COLOR : A2AVV2AB4PBMode::SPRAY;
    ...
    u64 globalMaxA = 0;
    u64 globalMaxB = 0;
    SplitABPairCount(globalMaxSend, globalMaxA, globalMaxB);
    u64 maxPart0 = 0;
    u64 maxPart1 = 0;
    SplitV2PairCount(globalMaxA, pathSplitRatio_, maxPart0, maxPart1);
    ...
    CHK_RET(BuildABExactSlotOffsets(params));
    CHK_RET(CheckScratch(resCtx));
    TemplateDataParams aParams = params;
    TemplateDataParams bParams = params;
    if (myRank_ < aParams.sendCounts.size()) {
        aParams.count = aParams.count >= aParams.sendCounts[myRank_] ? aParams.count - aParams.sendCounts[myRank_] : 0;
        aParams.sendCounts[myRank_] = 0;
        aParams.recvCounts[myRank_] = 0;
        aParams.sliceSize = aParams.count * dataTypeSize_;
    }
    if (myRank_ < bParams.sendCounts.size()) {
        bParams.count = bParams.count >= bParams.sendCounts[myRank_] ? bParams.count - bParams.sendCounts[myRank_] : 0;
        bParams.sendCounts[myRank_] = 0;
        bParams.recvCounts[myRank_] = 0;
        bParams.sliceSize = bParams.count * dataTypeSize_;
    }
    HCCL_WARNING("[A2AV_V2_AB_4P][Orchestrate] rank=%u total=%llu globalMax=%llu globalMaxA=%llu "
                 "globalMaxB=%llu abRatio=%f pathRatio=%f fallbackSlotStride=%llu exactSlotBytes=%llu bMode=%d",
                 myRank_, dataCount_, globalMaxSend, globalMaxA, globalMaxB, abRatio_, pathSplitRatio_,
                 slotStride_, exactSlotBytes_, static_cast<int>(bMode_));
    CHK_RET(RunAAndBParallel(param, resCtx, aParams, bParams));
    CHK_RET(CopySelfToOutputAfterCommunication(params));
    HCCL_WARNING("[A2AV_V2_AB_4P][Orchestrate] end.");
    return HCCL_SUCCESS;
}

REGISTER_EXECUTOR_BY_TOPO(HcclCMDType::HCCL_CMD_ALLTOALLV,
                          InsAlltoAllVParallelMesh2DClosV2AB4PlaneSprayNoMemcpy,
                          InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor,
                          TopoMatchUBX_V2);
...
REGISTER_EXECUTOR_BY_TOPO(HcclCMDType::HCCL_CMD_ALLTOALLV,
                          InsAlltoAllVParallelMesh2DClosV2AB4PlaneColorNoMemcpy,
                          InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor,
                          TopoMatchUBX_V2);

REGISTER_EXECUTOR_BY_TOPO(HcclCMDType::HCCL_CMD_ALLTOALLV,
                          InsAlltoAllVParallelMesh2DClosV2AB4PlaneColorNoMemcpyPodUbxV2,
                          InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor,
                          TopoMatchUBX_V2);

REGISTER_EXECUTOR_BY_TOPO(HcclCMDType::HCCL_CMD_ALLTOALLV,
                          InsAlltoAllVParallelMesh2DClosV2AB4PlaneColorNoMemcpyPodDirect,
                          InsV2AlltoAllVParallelV2AB4PNoMemcpyExecutor,
                          TopoMatchAlltoAllPodDirect);
