...
constexpr u64 CCU_MS_2P_DATA_SIZE = 8 * 1024 * 1024;
                                                    ...
                                                    std::string &selectAlgName) const
{
    ...
    if (IsTwoLevelNetLayer(topoInfo) && topoInfo->netLayerDetails.localNetInsSizeOfLayer[0] == 2 && dataSize > (CCU_MS_2P_DATA_SIZE)) {
        HCCL_DEBUG("[AllReduceAutoSelector] 2P scenario with data size[%llu] > [%llu], "
            "not supported for ccu_ms, fallback to AICPU.", dataSize, CCU_MS_2P_DATA_SIZE);
        return SelectorStatus::NOT_MATCH;
    }
