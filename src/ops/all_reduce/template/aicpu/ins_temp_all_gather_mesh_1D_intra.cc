                                                    ...
                                                    const std::map<u32, std::vector<ChannelInfo>> &channels)
{
    ...
    HCCL_INFO("[InsTempAllGatherMesh1dIntra] RunAllGatherMesh RankIDs[%d], myAlgRank[%u].", myRank_, myAlgRank);
    ...
    for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
        ...
        for (u32 threadIdx = 0; threadIdx < subCommRanks_[0].size() - 1; threadIdx++) {
            ...
            HCCL_INFO("[InsTempAllGatherMesh1dIntra] RunAllGatherMesh RankIDs[%d], connectedRank[%u], connectedAlgRank[%u].",
...
HcclResult InsTempAllGatherMesh1dIntra::PostLocalCopy(const std::vector<ThreadHandle> &threads)
{
    ...
    for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
        ...
        for (auto rank : subCommRanks_[0]) {
            ...
            HCCL_DEBUG("[InsTempAllGatherMesh1D][PostLocalCopy] LocalDataCopy RankID [%d] dataRank [%d] dataAlgRank[%u] "
