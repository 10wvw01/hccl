...
static CcuResult ParseKernelArg(AllGatherMesh1DMem2MemClosV2Context &ctx, CcuKernelArgAllGatherMesh1DMem2MemClosV2 *kernelArg)
{
    ctx.arg = kernelArg;
    return CCU_SUCCESS;
}

static CcuResult InitResource(AllGatherMesh1DMem2MemClosV2Context &ctx)
{
    const auto *arg = ctx.arg;
    uint32_t channelIdx = 0;

    if (arg->channelCount == 0) {
        HCCL_ERROR("[CcuKernelAllGatherMesh1DMem2MemClosV2] channels is empty!");
        return CcuResult::CCU_E_INTERNAL;
    }
    HCCL_INFO("[CcuKernelAllGatherMesh1DMem2MemClosV2] channels.size: [%u]", arg->channelCount);
...
static CcuResult LoadArgs(AllGatherMesh1DMem2MemClosV2Context &ctx)
{
    const auto *arg = ctx.arg;
    uint32_t argId = 0;
...
static CcuResult PreSync(AllGatherMesh1DMem2MemClosV2Context &ctx)
{
    const auto *arg = ctx.arg;
...
static CcuResult PostSync(AllGatherMesh1DMem2MemClosV2Context &ctx)
{
    const auto *arg = ctx.arg;
...
static CcuResult DoAllGather(AllGatherMesh1DMem2MemClosV2Context &ctx, const ccu::LocalAddr &src, const std::vector<ccu::RemoteAddr> &dst, const ccu::Variable &sliceSize)
{
    const auto *arg = ctx.arg;
...
static CcuResult DoRepeatAllGather(AllGatherMesh1DMem2MemClosV2Context &ctx)
{
    const auto *arg = ctx.arg;
...
{
    auto *kernelArg = static_cast<CcuKernelArgAllGatherMesh1DMem2MemClosV2 *>(arg);

    AllGatherMesh1DMem2MemClosV2Context ctx;
    ctx.resourceAllocated = false;
    ...
    HCCL_INFO("[CcuKernelAllGatherMesh1DMem2MemClosV2] AllGatherMesh1DMem2MemClosV2 run");
    ...
    HCCL_INFO("[CcuKernelAllGatherMesh1DMem2MemClosV2] AllGatherMesh1DMem2MemClosV2 end");
