...
using NHRStepInfoAG = struct NHRStepInfoDefAG {
    uint32_t step = 0;
    uint32_t myRank = 0;
    uint32_t nSlices;
    uint32_t toRank = 0;
    ...
    NHRStepInfoDefAG() : nSlices(0)
    {
    }
};

class CcuKernelArgAllGatherOmniPipeNHR1DMem2Mem : public hcomm::CcuKernelArg {
public:
    explicit CcuKernelArgAllGatherOmniPipeNHR1DMem2Mem(uint64_t rankSize_, uint32_t rankId,
                                                const OpParam& opParam,
                                                const std::vector<NHRStepInfoAG> stepInfoVector,
    ...
    uint32_t                                rankId_;
    OpParam                                 opParam_;
    std::vector<NHRStepInfoAG>                stepInfoVector_;
    std::map<uint32_t, uint32_t>            rank2ChannelIdx_;
    std::vector<std::vector<uint32_t>>      subCommRanks_;
    // std::map<uint32_t, uint32_t>            subRankIdx2RankIdx_;
};

class CcuTaskArgAllGatherOmniPipeNHR1DMem2Mem : public hcomm::CcuTaskArg {
public:
...
private:
    ...
    HcclResult DoRepeatAllGatherNHRSingleStep(const NHRStepInfoAG &nhrStepInfo);
