...
public:
    ...
    // Z轴绕路场景下，cclBuff由框间NHR按一维[rpt]布局写入，本rank无需再LocalDataCopy里做input->cclBuff的
    // 自拷贝；且RunAllGatherMesh从remote cclBuff读取时，应按inputSliceStride(框间写入的步进)索引，
    // 而非基类standalone用的sliceSize(基类ccl-copy建立的[rpt][algRank]二维布局步进)。
