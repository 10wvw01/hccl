...
{
    u32 channelsPerRank = std::max(channelsPerRank_, 1U);
    return templateRankSize_ > 1 ? (templateRankSize_ - 1) * channelsPerRank : 1;
}
                                                    ...
                                                    const std::map<u32, std::vector<ChannelInfo>> &channels)
{
    ...
    const bool dmaRead = (tempAlgParams_.buffInfo.inBuffType == BufferType::HCCL_BUFFER &&
