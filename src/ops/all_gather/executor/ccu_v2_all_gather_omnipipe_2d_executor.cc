...
CcuV2AllGatherOmniPipe2DExecutor<AlgTopoMatch, CcuAlgTempLevel0, CcuAlgTempLevel1>::CcuV2AllGatherOmniPipe2DExecutor()
{
}

template <typename AlgTopoMatch, typename CcuAlgTempLevel0, typename CcuAlgTempLevel1>
HcclResult CcuV2AllGatherOmniPipe2DExecutor<AlgTopoMatch, CcuAlgTempLevel0, CcuAlgTempLevel1>::CalcAlgHierarchyInfo(
    HcclComm comm,
    ...
    AlgHierarchyInfoForAllLevel& algHierarchyInfo)
{
    ...
    // topoInfo->rs_x_bw = GetExternalInputBandWidthRSX();
    // topoInfo->rs_y_bw = GetExternalInputBandWidthRSY();
    // topoInfo->ag_x_bw = GetExternalInputBandWidthAGX();
    // topoInfo->ag_y_bw = GetExternalInputBandWidthAGY();
    // HCCL_DEBUG("[%s] BandWidth: rs_x[%f] rs_y[%f] ag_x[%f] ag_y[%f]", __func__, topoInfo->rs_x_bw, topoInfo->rs_y_bw,
    //     topoInfo->ag_x_bw, topoInfo->ag_y_bw);

    return HCCL_SUCCESS;
}

template <typename AlgTopoMatch, typename CcuAlgTempLevel0, typename InsAlgTempLevel1>
HcclResult CcuV2AllGatherOmniPipe2DExecutor<AlgTopoMatch, CcuAlgTempLevel0, InsAlgTempLevel1>::InitCommInfo(
...
template <typename AlgTopoMatch, typename CcuAlgTempLevel0, typename CcuAlgTempLevel1>
HcclResult CcuV2AllGatherOmniPipe2DExecutor<AlgTopoMatch, CcuAlgTempLevel0, CcuAlgTempLevel1>::CalcResLevel(
...
template <typename AlgTopoMatch, typename CcuAlgTempLevel0, typename CcuAlgTempLevel1>
HcclResult CcuV2AllGatherOmniPipe2DExecutor<AlgTopoMatch, CcuAlgTempLevel0, CcuAlgTempLevel1>::CalcRes(
...
HcclResult CcuV2AllGatherOmniPipe2DExecutor<M, X, Y>::GenTemplateAlgParamsByDimData(
...
// template <typename AlgTopoMatch, typename CcuAlgTempLevel0, typename CcuAlgTempLevel1>
// HcclResult CcuV2AllGatherOmniPipe2DExecutor<AlgTopoMatch, CcuAlgTempLevel0, CcuAlgTempLevel1>::RestoreChannelMap(
...
template <typename AlgTopoMatch, typename CcuAlgTempLevel0, typename CcuAlgTempLevel1>
HcclResult CcuV2AllGatherOmniPipe2DExecutor<AlgTopoMatch, CcuAlgTempLevel0, CcuAlgTempLevel1>::Orchestrate(
...
template <typename AlgTopoMatch, typename CcuAlgTempLevel0, typename CcuAlgTempLevel1>
HcclResult CcuV2AllGatherOmniPipe2DExecutor<AlgTopoMatch, CcuAlgTempLevel0, CcuAlgTempLevel1>::PrepareResForTemplate(
...
template <typename AlgTopoMatch, typename CcuAlgTempLevel0, typename CcuAlgTempLevel1>
HcclResult CcuV2AllGatherOmniPipe2DExecutor<AlgTopoMatch, CcuAlgTempLevel0, CcuAlgTempLevel1>::OrchestrateLoop(
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    HCCL_DEBUG("[%s] Start", __func__);
    ...
    // 1. 计算带宽
    // double eqBwLevel1 = resCtx.topoInfo.ag_y_bw;
    // eqBwLevel1 = rankSizeLevel1_ > 1 ? eqBwLevel1 / (rankSizeLevel1_ - 1) : eqBwLevel1;
    // std::vector<double> endpointAttrBw = {resCtx.topoInfo.ag_x_bw, eqBwLevel1, 1.0};

    std::vector<double> endpointAttrBw = {3.0, 4.0, 1.0};

    // 2. 计算loop  ccu不用cclbuff, 根据UB_MAX_DATA_SIZE来计算
#if T_DESC("手动切Loop测试", false)
...
#else
    u64 maxCountPerLoop = static_cast<u64>(256 * 1024 * 1024) / dataTypeSize_;//UB_MAX_DATA_SIZE
#endif
    u32 loopTimes = dataCount_ / maxCountPerLoop + ((dataCount_ % maxCountPerLoop == 0) ? 0 : 1);
    u64 perLoopSize = maxCountPerLoop * dataTypeSize_;
    perLoopSize = dataSize_ > perLoopSize ? perLoopSize : dataSize_;
    HCCL_DEBUG("[%s] myRank[%u], loopTimes[%llu], perLoopSize[%llu]", __func__, myRank_, loopTimes, perLoopSize);
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        // 5.3 for内层2d
        for (auto i = 0; i < innerServerStepNum; ++i) {//innerServerStepNum
            //第一步开始前同步
...
REGISTER_EXECUTOR_BY_TWO_TEMPS(HcclCMDType::HCCL_CMD_ALLGATHER, CcuAllGatherOmniPipe2D,
    CcuV2AllGatherOmniPipe2DExecutor, TopoMatchUBX,
