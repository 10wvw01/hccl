...
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1>
HcclResult InsV2AllGatherConcurrentExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1>::CalcRes(HcclComm comm,
    const OpParam &param, const TopoInfoWithNetLayerDetails *topoInfo,
    const AlgHierarchyInfoForAllLevel &algHierarchyInfo, AlgResourceRequest &resourceRequest)
{
    ...
    resourceRequest.notifyNumPerThread.insert(resourceRequest.notifyNumPerThread.end(),
        temp1ResReq.notifyNumPerThread.begin(), temp1ResReq.notifyNumPerThread.end());
    ...
    if (param.engine == CommEngine::COMM_ENGINE_CCU) {
        ...
        CHK_PRT_RET(temp0Channels.size() != temp1Channels.size(),
            HCCL_ERROR("[InsV2AllGatherConcurrentExecutor][CalcRes] temp0Channels.size()[%zu] is not equal to "
    ...
    InsAlgTemplate1 &algTemplate1)
{
    ...
    if (scratchMultiplierforTemp0 > 0) {
        maxCountPerLoopforTemp0
            = std::min(maxCountUBLimit, scratchSizeforTemp0 / scratchMultiplierforTemp0 / HCCL_MIN_SLICE_ALIGN
                                            * HCCL_MIN_SLICE_ALIGN / dataTypeSize_);
    }
    if (scratchMultiplierforTemp1 > 0) {
        maxCountPerLoopforTemp1
            = std::min(maxCountUBLimit, scratchSizeforTemp1 / scratchMultiplierforTemp1 / HCCL_MIN_SLICE_ALIGN
                                            * HCCL_MIN_SLICE_ALIGN / dataTypeSize_);
    }
    ...
    for (u32 loopIndex = 0; loopIndex < loopTimesforTemp0 || loopIndex < loopTimesforTemp1; loopIndex++) {
        if (loopIndex < loopTimesforTemp0) {
            ...
            HCCL_INFO("[InsV2AllGatherConcurrentExecutor][OrchestrateLoop] loopIndex[%u], currCountforTemp0[%u], "
        ...
        if (loopIndex < loopTimesforTemp1) {
            ...
            HCCL_INFO("[InsV2AllGatherConcurrentExecutor][OrchestrateLoop] loopIndex[%u], currCountforTemp1[%u], "
    ...
    u32 notifyNumOnMainThread)
{
    ...
    HCCL_INFO("[InsV2AllGatherConcurrentExecutor][FastLaunchSaveCtx] threadNum[%llu], ccuKernelNum[%llu]", threadNum,
        ccuKernelNum);
...
#else
REGISTER_EXECUTOR_BY_TWO_TEMPS(HcclCMDType::HCCL_CMD_ALLGATHER, InsAllGatherConcurrentMesh1DNHR,
    InsV2AllGatherConcurrentExecutor, TopoMatchUBX, InsTempAllGatherMesh1D, InsTempAllGatherNHR);
#endif /* HCCL_AICPU_CODEGEN */

#ifndef AICPU_COMPILE
#if CANN_VERSION_NUM >= CANN_VERSION(9, 0, 0)
REGISTER_EXECUTOR_BY_TWO_TEMPS(HcclCMDType::HCCL_CMD_ALLGATHER, CcuAllGatherConcurrentMesh1DNHRMem,
    InsV2AllGatherConcurrentExecutor, TopoMatchUBX, CcuTempAllGatherMesh1DMem2Mem,
...
REGISTER_EXECUTOR_BY_TWO_TEMPS(HcclCMDType::HCCL_CMD_ALLGATHER, CcuAllGatherConcurrentMesh1DNHR,
    InsV2AllGatherConcurrentExecutor, TopoMatchUBX, CcuTempAllGatherMesh1D, CcuTempAllGatherNHR1DMultiJettyMem2Mem);
#endif /* CANN_VERSION_NUM >= CANN_VERSION(9, 0, 0) */
#endif
