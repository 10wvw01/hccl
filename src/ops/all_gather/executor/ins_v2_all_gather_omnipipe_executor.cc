...
HcclResult InsV2AllGatherOmniPipeExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1,
    InsAlgTemplate2>::PrepareResForTemplateLevel(u32 level, std::shared_ptr<InsAlgTemplateBase> &tempBase)
{
    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    if (!resCtx.channels.empty()) {
        ...
        auto tryLevel = [&](u32 i, const std::vector<std::vector<u32>> &subComms, u32 remoteRank,
        ...
        for (const auto &channel : resCtx.channels[0]) {
            ...
            if (tryLevel(OMNIPIPE_LEVEL0, subCommRanks0, rr, channel)) {
                continue;
            }
            if (tryLevel(OMNIPIPE_LEVEL1, subCommRanks1, rr, channel)) {
                continue;
            }
            if (tryLevel(OMNIPIPE_LEVEL2, subCommRanks2, rr, channel)) {
                continue;
            }
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        if (!param.supportSymmetricMemory) {
            if (omniUbxLastStepRead_) {
                CHK_RET(UbxLocalCopy(param, omniPipeSliceInfo, omniPipeSliceLocalcopyInfo, tempAlgParamMap,
                    processedDataCount, level0StepCount));
            } else {
                for (u32 rank = 0; rank < rankSize_; rank++) {
                    ...
                    DataSlice src(resCtx.cclMem.addr, rank * currDataCount * dataTypeSize_,
                        currDataCount * dataTypeSize_, currDataCount);
                    CHK_RET(LocalCopy(controlThread_, src, dst));
                }
            }
    ...
    const u64 processedDataCount, int step) const
{
    ...
    for (int rpt = 0; rpt < omniPipeSliceLocalcopyInfo.dataSliceLevel0[step - 1].inputOmniPipeSliceStride[k].size();
        ...
        txBaseOffDst0 = txOffsetDst0 + processedDataCount * dataTypeSize_;
        // src用ccl的
        DataSlice txSrcSlice0 =
            DataSlice(txSrcPtr0, txOffset0, omniPipeSliceLocalcopyInfo.dataSliceLevel0[step - 1].stepSliceSize[k][rpt],
                omniPipeSliceLocalcopyInfo.dataSliceLevel0[step - 1].stepCount[k][rpt]);
        // dst用localcopy的
        DataSlice txDstSlice0 = DataSlice(txDstPtr0, txBaseOffDst0,
            omniPipeSliceLocalcopyInfo.dataSliceLevel0[step - 1].stepSliceSize[k][rpt],
    ...
    for (int rpt = 0; rpt < omniPipeSliceLocalcopyInfo.dataSliceLevel1[step - 1].inputOmniPipeSliceStride[k].size();
        ...
        txBaseOffDst1 = txOffsetDst1 + processedDataCount * dataTypeSize_;
        // src用ccl的
        DataSlice txSrcSlice1 =
            DataSlice(txSrcPtr1, txOffset1, omniPipeSliceLocalcopyInfo.dataSliceLevel1[step - 1].stepSliceSize[k][rpt],
                omniPipeSliceLocalcopyInfo.dataSliceLevel1[step - 1].stepCount[k][rpt]);
        // dst用localcopy的
        DataSlice txDstSlice1 = DataSlice(txDstPtr1, txBaseOffDst1,
            omniPipeSliceLocalcopyInfo.dataSliceLevel1[step - 1].stepSliceSize[k][rpt],
...
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_ALLGATHER, InsV2AllGatherOmniPipeMultilevel,
    InsV2AllGatherOmniPipeExecutor, TopoMatchMultilevel, InsTempAllGatherOmniPipeMesh1D, InsTempAllGatherOmniPipeNHR,
    InsTempAllGatherOmniPipeNHRDPU);
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_ALLGATHER, InsV2AllGatherOmniPipePcie, InsV2AllGatherOmniPipeExecutor,
    TopoMatchPcieMix, InsTempAllGatherOmniPipeMesh1D, InsTempAllGatherOmniPipeNHR, InsTempAllGatherOmniPipeNHRDPU);
REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_ALLGATHER, InsV2AllGatherOmniPipe, InsV2AllGatherOmniPipeExecutor,
    TopoMatchUBX, InsTempAllGatherOmniPipeMesh1D, InsTempAllGatherOmniPipeNHR, InsTempAllGatherOmniPipeNHRDPU);

REGISTER_EXEC_V2_MULTI(HcclCMDType::HCCL_CMD_ALLGATHER, InsV2AllGatherOmniPipeUboe, InsV2AllGatherOmniPipeExecutor,
    TopoMatch3Level, InsTempAllGatherOmniPipeMesh1D, InsTempAllGatherOmniPipeNHR, InsTempAllGatherOmniPipeNHR);
} // namespace ops_hccl
