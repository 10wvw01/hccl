    ...
    const std::map<HcclCMDType, std::vector<HcclAlgoType>> &configAlgMap, std::string &selectAlgName) const
{
    ...
    constexpr u64 CCU_SCHEDULE_2LEVEL_MAX_LESS_32P_PER_RANK_DATA_SIZE = 64ULL * 1024 * 1024;
    constexpr u64 CCU_SCHEDULE_2LEVEL_MAX_MORE_32P_PER_RANK_DATA_SIZE = 1ULL * 1024 * 1024;
    u64 ccuSize = 32;
    u64 perDataSize = DATATYPE_SIZE_TABLE[opParam.DataDes.dataType];
    u64 dataSize = opParam.DataDes.count * perDataSize;
    ...
    if (topoInfo->topoLevelNums > 1) {
        if (topoInfo->level0Topo == Level0Shape::MESH_1D) {
            if (topoInfo->userRankSize == 0 ||
                (dataSize / topoInfo->userRankSize > CCU_SCHEDULE_2LEVEL_MAX_MORE_32P_PER_RANK_DATA_SIZE && topoInfo->userRankSize > ccuSize) ||
                (dataSize > CCU_SCHEDULE_2LEVEL_MAX_LESS_32P_PER_RANK_DATA_SIZE && topoInfo->userRankSize <= ccuSize)) {
