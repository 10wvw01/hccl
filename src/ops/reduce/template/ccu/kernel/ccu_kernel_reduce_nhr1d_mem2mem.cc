...
static CcuResult LocalCopySlices(ReduceNHR1DMem2MemContext &ctx)
{
    ...
    CCU_IF(ctx.isInputOutputEqual == 0)
    {
        ...
        for (u32 i = 0; i < nonTxSliceIdxList.size(); i++) {
            ...
            // signalIndex必须mod RANK_NUM_PER_CKE：Event mask是uint16_t，
            // 1<<signalIndex在signalIndex>=16时会渢断为0，导致EventRecord不设置任何bit，
            // 后续EventWait永久等待。64p时nonTxSliceIdxList.size()=32会触发。
