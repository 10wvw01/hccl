...
static CcuResult DoGather(GatherOmniPipeMesh1DMem2MemContext &ctx)
{
    ...
    for (uint64_t rankIdx = 0; rankIdx < ctx.rankSize; rankIdx++) {
        ...
        CCU_IF(ctx.sliceSizeOmniSliceStrideVec[rankIdx] != 0) {
            if (ctx.rankIdx != rankIdx) {
                ccu::Read(ctx.arg->channels[channelId], ctx.outputMem[rankIdx], ctx.inputMem[rankIdx], ctx.sliceSizeOmniSliceStrideVec[rankIdx], ctx.event, rankMask);
                HCCL_INFO("[CcuGatherOmniPipeMesh1DMem2Mem] channelId[%u] rankIdx[%u] inputMem[%llu] sliceSize[%u]", channelId, rankIdx,ctx.outputMem[rankIdx], ctx.inputMem[rankIdx], ctx.sliceSizeOmniSliceStrideVec[rankIdx]);
            } else {
                ccu::EventRecord(ctx.event, rankMask);
            }
        }
