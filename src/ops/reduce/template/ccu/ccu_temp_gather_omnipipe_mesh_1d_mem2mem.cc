                                                          ...
                                                          TemplateResource& templateResource)
{
    ...
    if (localCopyFlag == 0) {
        ...
        for (uint32_t rpt = 0; rpt < rptTime; ++rpt) { // 子通信域的第peerId个卡，要发的第几个数据片
            sliceSize = stepSliceInfo.stepSliceSize[rpt];
            inputOmniPipeSliceStride = stepSliceInfo.inputOmniPipeSliceStride[rpt]; // 远端的输入offset
            outputOmniPipeSliceStride = stepSliceInfo.outputOmniPipeSliceStride[rpt]; // 远端的输出offset
