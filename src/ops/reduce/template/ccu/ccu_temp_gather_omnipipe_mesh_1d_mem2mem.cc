                                                           ...
                                                           TemplateResource& templateResource)
{
    ...
    if (localCopyFlag == 0) {
        CHK_RET(RunGatherMeshSteps(templateDataParams, inputAddr, outputAddr, token, localCopyFlag, templateResource));
...
HcclResult CcuTempGatherOmniPipeMesh1DMem2Mem::RunGatherMeshSteps(const TemplateDataParams& templateDataParams,
    ...
    TemplateResource& templateResource)
{
    ...
    for (uint32_t rpt = 0; rpt < inputOmniPipeSliceStrides[0].size(); ++rpt) {
        ...
        std::vector<uint64_t> taskArgs = { inputAddr, outputAddr, token, localCopyFlag, sliceSize,
            isStepOne_, isLastStep_, ifNewRoot, isFirstPiece, isLastPiece };
        ...
        HCCL_DEBUG("[RunGatherMeshSteps] rpt=%u inputAddr=%llu outputAddr=%llu sliceSize=%llu ifNewRoot=%llu myRank[%u]",
            rpt, inputAddr, outputAddr, sliceSize, ifNewRoot, myRank_);
