...
HcclResult CcuTempGatherOmniPipeNHR1DMem2Mem::GetStepInfo(u32 step, u32 nSteps, NHRStepInfo &stepInfo) const
{
    ...
    // 数据份数和数据编号增量
    u32 nSlices = (templateRankSize_ - 1 + deltaRankPair) / (1 << (nSteps - step));
    u32 deltaSliceIndex = 1 << (nSteps - step);

    // 对称的发送/接收配对：每个 rank 在每一步都既发送又接收
    // sendTo = (virtRankIdx + deltaRankPair) % templateRankSize_
    // recvFrom = (virtRankIdx + templateRankSize_ - deltaRankPair) % templateRankSize_
    // 这样 rank A 的 sendTo = rank B 时，rank B 的 recvFrom = rank A，天然配对
    // 修复非2的幂 rankSize 时发送组/接收组划分不对称导致 NotifyWait 等不到 NotifyRecord 的死锁问题
    ...
    HCCL_DEBUG("[%s] myRank[%u] StepInfo step[%u] nSteps[%u] nSlices[%u] fromRank[%u] toRank[%u] subRoot[%u] subCommRootId_[%u]",
        __func__, myRank_, step, nSteps, stepInfo.nSlices, stepInfo.fromRank, stepInfo.toRank, subCommRootId_, subCommRootId_);
    return HcclResult::HCCL_SUCCESS;
}
} // namespace ops_hccl