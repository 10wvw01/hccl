            ...
            const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
        ...
        if (myRank_ == param.root) { // loop偏移 + 外部卡偏移
            ...
            for (u32 i = 0; i < rankSize_; i++) {
                ...
                if (i == param.root) {
                    ...
                    if (loop == 0) {
                        tempAlgParamLocalCopy.count = allRankSplitData[i];
                        tempAlgParamLocalCopy.sliceSize = allRankSplitData[i] * dataTypeSize_;
                    } else {
                        rankOffset += allRankSplitData[i] * dataTypeSize_; // 卡偏移
                        rankLoopOffset += multiLoopAllRankSplitData[loop][i] * dataTypeSize_;// 0 11 
                       continue; 
                    }
                } else {
                ...
                if (i % 2 == 0) {
                    ...
                    CHK_RET(gAlgTempY.KernelRun(param, tempAlgParamLocalCopy, templateResourceGY));
                }
    ...
    HCCL_INFO("[%s][OrchestrateLoop] Endzq.", __func__);
    return HCCL_SUCCESS;
}
