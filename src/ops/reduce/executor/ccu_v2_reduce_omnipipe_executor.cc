            ...
            const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    allRankSplitData[rankSize_ - 1] = allRankSplitData[rankSize_ - 2] - 5;
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {//loopTimes
        ...
        if (myRank_ == param.root) { // loop偏移 + 外部卡偏移
            ...
            for (u32 i = 0; i < rankSize_; i++) {
                ...
                if (i == rankSize_ - 1 && loop == 1) {
                    processedDataCount = processedDataCount - (multiLoopAllRankSplitData[loop][i] - multiLoopAllRankSplitData[loop][i-1]);
                }
