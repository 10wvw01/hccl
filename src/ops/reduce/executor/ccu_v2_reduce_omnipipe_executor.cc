    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {//loopTimes
        ...
        if (myRank_ == param.root) { // loop偏移 + 外部卡偏移
            ...
            for (u32 i = 0; i < rankSize_; i++) {
                ...
                HCCL_DEBUG("START PRINT");
                // HCCL_DEBUG("[%s] myRank[%u]  inBuffBaseOff[%lu] outBuffBaseOff[%lu] sliceSize[%lu] processedDataCount[%lu] rankOffset[%lu] rankLoopOffset[%lu]", __func__, myRank_, tempAlgParamLocalCopy.buffInfo.inBuffBaseOff, tempAlgParamLocalCopy.buffInfo.outBuffBaseOff, tempAlgParamLocalCopy.sliceSize, processedDataCountTmp, rankOffset, rankLoopOffset);
