...
#ifndef HCCLV2_INS_V2_REDUCE_SCATTER_SEQUENCE_EXECUTOR_4LEVEL_OCS_H
#define HCCLV2_INS_V2_REDUCE_SCATTER_SEQUENCE_EXECUTOR_4LEVEL_OCS_H
...
namespace ops_hccl {
// 4层(含 net_layer_3, SuperNode 级 OCS) 序列编排 executor:
//   layer0(intra, INPUT->HCCL) -> layer1(inter, HCCL->HCCL)
//   -> layer2(inter, HCCL->HCCL) -> layer3(inter, HCCL->OUTPUT)
template <typename AlgTopoMatch, typename InsAlgTemplate0, typename InsAlgTemplate1,
    typename InsAlgTemplate2, typename InsAlgTemplate3>
class InsV2ReduceScatterSequenceExecutor4LevelOCS : public InsCollAlgBase {
public:
    explicit InsV2ReduceScatterSequenceExecutor4LevelOCS();
    ~InsV2ReduceScatterSequenceExecutor4LevelOCS() override = default;
...
protected:
    ...
    void GenInterTemplateParams2(TemplateDataParams &tempAlgParamsInter, const u64 processedDataCount,
        const u64 currDataCount, const u64 loop) const;
    void GenInterTemplateParams3(TemplateDataParams &tempAlgParamsInter, const u64 processedDataCount,
        const u64 currDataCount, const u64 loop) const;
