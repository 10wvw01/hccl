    ...
    const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    if (rankSizeLevel0_ > 1) {
        CHK_RET(tempMap[OMNIPIPE_LEVEL0]->SetchannelsPerRank(remoteRankToChannelInfo_[0]));
    }
    if (rankSizeLevel1_ > 1) {
        CHK_RET(tempMap[OMNIPIPE_LEVEL1]->SetchannelsPerRank(remoteRankToChannelInfo_[1]));
    }
    if (rankSizeLevel2_ > 1) {
        CHK_RET(tempMap[OMNIPIPE_LEVEL2]->SetchannelsPerRank(remoteRankToChannelInfo_[2]));
    }
