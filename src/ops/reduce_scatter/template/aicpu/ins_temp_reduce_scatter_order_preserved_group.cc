...
// 内部静态函数：发送单个远端分片
static HcclResult SendSingleRankSlice(
    u32 dstRank,
    ...
    const std::vector<ThreadHandle> &threads, const TemplateDataParams &tempAlgParams)
{
    ...
    u32 totalRanks = templateRankSize_ - 1;
    ...
    // 按环距离对称遍历收发对端，移除冗余visited数组
    for (u32 distance = 1; distance <= N / 2; distance++) {
        u32 rightRank = (myAlgRank + distance) % N;
        u32 leftRank  = (myAlgRank + N - distance) % N;

        // 发送右侧distance peer
        CHK_RET(SendSingleRankSlice(rightRank, channels, memBlockInfo, myAlgRank, tempAlgParams, threads[queIdx]));
        queIdx++;
        if (queIdx >= threadNum_) {
            queIdx = 1;
        }

        // 左右rank不重合才发送左侧peer（N为偶数时distance=N/2左右rank相等，避免重复发送）
        if (leftRank != rightRank) {
            CHK_RET(SendSingleRankSlice(leftRank, channels, memBlockInfo, myAlgRank, tempAlgParams, threads[queIdx]));
            queIdx++;
            if (queIdx >= threadNum_) {
                queIdx = 1;
            }
