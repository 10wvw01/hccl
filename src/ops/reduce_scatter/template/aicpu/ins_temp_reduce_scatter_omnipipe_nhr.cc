...
HcclResult InsTempReduceScatterOmniPipeNHR::GetNHRRemoteAddrs(
    u32 sendToRank, u32 recvFromRank, const ChannelInfo &linkSend, const ChannelInfo &linkRecv,
    void *&sendRemoteAddr, void *&recvRemoteAddr) const
{
    if (!supportSymmetricMemory_) {
        // 普通路径使用建链阶段获取的对端 ccl scratch 地址。
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS || sendRemoteAddr == nullptr,
                HCCL_ERROR("[InsTempReduceScatterOmniPipeNHR][GetNHRRemoteAddrs] failed to get peer input "
                           "pointer for send target, sendToRank[%u], ret[%d], ptr[%p].",
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS || recvRemoteAddr == nullptr,
                HCCL_ERROR("[InsTempReduceScatterOmniPipeNHR][GetNHRRemoteAddrs] failed to get peer input "
                           "pointer for receive source, recvFromRank[%u], ret[%d], ptr[%p].",
...
HcclResult InsTempReduceScatterOmniPipeNHR::RunNHRStep(
    const AicpuNHRStepInfo &stepInfo, const std::vector<ThreadHandle> &threads, u32 channelIdx,
    u32 dataTypeSize, u64 repeatNum, bool isPcieProtocol)
{
    CHK_PRT_RET(stepInfo.fromRank >= subCommRanks_[0].size() || stepInfo.toRank >= subCommRanks_[0].size(),
        HCCL_ERROR("[InsTempReduceScatterOmniPipeNHR][RunNHRStep] algorithm rank is out of range, "
                   "fromAlgRank[%u], toAlgRank[%u], step[%u].",
                   stepInfo.fromRank, stepInfo.toRank, stepInfo.step), HcclResult::HCCL_E_INTERNAL);
    const u32 recvFromRank = subCommRanks_[0][stepInfo.fromRank];
    const u32 sendToRank = subCommRanks_[0][stepInfo.toRank];
    CHK_PRT_RET(channels_.count(recvFromRank) == 0 || channels_.count(sendToRank) == 0 ||
        ...
        HCCL_ERROR("[InsTempReduceScatterOmniPipeNHR][RunNHRStep] required communication channel is missing, "
                   "recvFromRank[%u], sendToRank[%u], channelIdx[%u], step[%u].",
                   recvFromRank, sendToRank, channelIdx, stepInfo.step), HcclResult::HCCL_E_INTERNAL);
    ...
    HCCL_DEBUG("[InsTempReduceScatterOmniPipeNHR][RunNHRStep] selected channels for current step, "
               "step[%u], channelIdx[%u], recvFromRank[%u], sendToRank[%u], recvChannelRank[%u], "
               "sendChannelRank[%u].", stepInfo.step, channelIdx, recvFromRank, sendToRank,
               linkRecv.remoteRank, linkSend.remoteRank);
    void *sendRemoteAddr = nullptr;
    void *recvRemoteAddr = nullptr;
    CHK_RET(GetNHRRemoteAddrs(sendToRank, recvFromRank, linkSend, linkRecv, sendRemoteAddr, recvRemoteAddr));

    std::vector<DataSlice> txSrcSlices, txDstSlices, rxSrcSlices, rxDstSlices;
    CHK_RET(GetNHRDataSize(stepInfo, channelIdx, sendRemoteAddr, recvRemoteAddr, dataTypeSize, repeatNum,
    ...
    if (isPcieProtocol) {
        CHK_PRT_RET(SendRecvReadReduce(info, threads[channelIdx]),
            HCCL_ERROR("[InsTempReduceScatterOmniPipeNHR][RunNHRStep] read-reduce communication failed, "
                       "step[%u], channelIdx[%u], recvFromRank[%u], sendToRank[%u].",
                       stepInfo.step, channelIdx, recvFromRank, sendToRank), HcclResult::HCCL_E_INTERNAL);
    } else {
        CHK_PRT_RET(SendRecvBatchWriteReduce(info, threads[channelIdx]),
            HCCL_ERROR("[InsTempReduceScatterOmniPipeNHR][RunNHRStep] batch write-reduce communication failed, "
                       "step[%u], channelIdx[%u], recvFromRank[%u], sendToRank[%u].",
                       stepInfo.step, channelIdx, recvFromRank, sendToRank), HcclResult::HCCL_E_INTERNAL);
    }
    return HcclResult::HCCL_SUCCESS;
...
HcclResult InsTempReduceScatterOmniPipeNHR::RunNHR(const std::vector<ThreadHandle> &threads, u32 channelIdx)
{
    CHK_PRT_RET(threads.size() <= channelIdx,
        HCCL_ERROR("[InsTempReduceScatterOmniPipeNHR][RunNHR] required thread is missing, "
                   "channelIdx[%u], threadNum[%zu].", channelIdx, threads.size()),
        HcclResult::HCCL_E_INTERNAL);
    if (templateRankSize_ <= 1) {
        return HcclResult::HCCL_SUCCESS;
    }
    ...
    // 重复片数由 inputOmniPipeSliceStride 决定，每一步复用同一份拆分结果。
    ...
    for (const auto &stepInfo : steps) {
        CHK_RET(RunNHRStep(stepInfo, threads, channelIdx, dataTypeSize, repeatNum, isPcieProtocol));
    }
    return HcclResult::HCCL_SUCCESS;
}
