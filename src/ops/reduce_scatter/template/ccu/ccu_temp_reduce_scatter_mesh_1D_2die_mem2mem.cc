                                                      ...
                                                      AlgResourceRequest& resourceRequest)
{
    ...
    // 参照 alltoall/alltoallv mesh1d 2Die 的做法：按 meshDieId 先、closDieId 后的顺序创建 kernel，
    // 保证所有 rank 的 ccuKernelInfos[0] 总是 mesh kernel、[1] 总是 clos kernel。
    // 必须这样统一顺序，否则当 2 张卡 die 连线方向相反时（卡A meshDieId=0、卡B meshDieId=1），
    // 按 dieId 顺序创建会导致卡A 的 mesh kernel 在 index 0、卡B 的 clos kernel 在 index 0，
    // CCU channel 跨 rank 同步时 thread/kernel index 不对齐，建链失败。
    ...
    for (uint32_t i = 0; i < DIE_NUM; i++) {
        ...
        // closDieId 上的链路为出框 clos（channels 较多），isReduceToOutput=false，reduce 结果写 scratch，
