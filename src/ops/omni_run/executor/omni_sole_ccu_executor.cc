...
template <typename AlgTopoMatch, typename InsAlgTemplate>
HcclResult OmniSoleExecutor<AlgTopoMatch, InsAlgTemplate>::ParseXmlInfo(const OpParam& param,
    const TopoInfoWithNetLayerDetails* topoInfo)
{
    if (param.xmlPath[0] == '\0') {
        HCCL_ERROR("[ParseXmlInfo] xmlPath is not set in OpParam");
        return HCCL_E_PARA;
    }
    std::string fileName(param.xmlPath);
    HCCL_INFO("[ParseXmlInfo] rank[%u] parsing from xmlPath: %s", myRank_, fileName.c_str());
    ...
    do {
        ...
        if (op == 0) { // resRequest
            ...
            Channel channel;
            std::map<u32, omni::OmniChannelInfo> mapChannelInfo;
            for (uint16_t i = 0; i < resRequest.chanCount; i++) {
                ...
                omni::OmniChannelInfo omniChannelInfo;
            ...
            omni::OmniNormalInstruction omniInstruction;
            omniInstruction.opType = static_cast<omni::OpType>(ctrlOp.opCode);
            omniInstruction.sendRecvInfo.inputDataType = static_cast<HcclDataType>(ctrlOp.inputDataType);
            omniInstruction.sendRecvInfo.outputDataType = static_cast<HcclDataType>(ctrlOp.outputDataType);
            omniInstruction.sendRecvInfo.reduceType = static_cast<HcclReduceOp>(ctrlOp.reduceType);
            omniInstruction.sendRecvInfo.sliceNum = ctrlOp.sliceNum;
            omniInstruction.sendRecvInfo.threadIdx = ctrlOp.threadIdx;
            omniInstruction.sendRecvInfo.netlayerId = ctrlOp.netlayerId;
            HCCL_INFO("rank [%u] op [%u] sliceNum [%u] netlayerId [%u] threadIdx [%u]",
                myRank_, omniInstruction.opType, omniInstruction.sendRecvInfo.sliceNum, omniInstruction.sendRecvInfo.netlayerId, omniInstruction.sendRecvInfo.threadIdx);



            for (uint16_t i = 0; i < ctrlOp.srcSliceNum; i++) {
                omni::OmniSliceInfo omniSliceInfo;
                file.seekg(offset); // 定位到正确的偏移位置
                ...
                omniSliceInfo.sliceType = static_cast<omni::BufferTypeTmp>(srcSlice.bufferType);
                omniSliceInfo.sliceIdx = srcSlice.sliceIdx;
                omniSliceInfo.remoteRank = srcSlice.rankId;
                omniInstruction.sendRecvInfo.srcSliceInfo.push_back(omniSliceInfo);
            ...
            for (uint16_t i = 0; i < ctrlOp.dstSliceNum; i++) {
                ...
                omniSliceInfo.sliceType = static_cast<omni::BufferTypeTmp>(dstSlice.bufferType);
                omniSliceInfo.sliceIdx = dstSlice.sliceIdx;
                omniSliceInfo.remoteRank = dstSlice.rankId;
                omniInstruction.sendRecvInfo.dstSliceInfo.push_back(omniSliceInfo);
            ...
            xmlInfo_.vecNormalInstruction.push_back(omniInstruction);
        }
    ...
    const OpParam &param, const AlgResourceCtxSerializable &resCtx)
{
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        tempAlgParams.sliceSize = currDataCount * dataTypeSize_ / xmlInfo_.vecNormalInstruction[0].sendRecvInfo.sliceNum * 4;
