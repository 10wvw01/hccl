...
#ifndef OPS_HCCL_OMNI_RUN_TEMPLATE_CCU_OMNI_TEMP_CCU_H
#define OPS_HCCL_OMNI_RUN_TEMPLATE_CCU_OMNI_TEMP_CCU_H

#include "utils.h"
#include "omni_parser.h"
#include "ccu_alg_template_base.h"
#include "kernel/omni_kernel_ccu.h"

namespace ops_hccl {

using RankGroup = std::vector<RankId>;

class OmniTempCcu : public CcuAlgTemplateBase {
public:
    explicit  OmniTempCcu(const OpParam& param, 
                                        const u32 rankId, // 传通信域的rankId，userRank
                                        const std::vector<std::vector<u32>> &subCommRanks);

    ~OmniTempCcu() override;

    std::string Describe() const override
    {
        return StringFormat("Template of omni_run CCU with tempRankSize [%u].", tempRankSize_);
    }
...
private:
    HcclResult CalcChannelRequestOmni(HcclComm comm, 
        ...
        const std::vector<std::map<u32, omni::OmniChannelInfo>>& mapchannelInfo, 
...
#endif// OPS_HCCL_OMNI_RUN_TEMPLATE_CCU_OMNI_TEMP_CCU_H