...
OmniTempCcu::OmniTempCcu(const OpParam& param, const u32 rankId,
...
OmniTempCcu::~OmniTempCcu()
{
}

HcclResult OmniTempCcu::CreateChannelFromLink(HcclComm comm, u32 myRank, u32 rank, uint32_t netLayer, u32 idx,
...
HcclResult OmniTempCcu::ProcessLinkForProtocol(HcclComm comm, const std::vector<CommProtocol>& expectedProtocols,
...
// HcclResult OmniTempCcu::CalcChannelRequestOmni(HcclComm comm, const OpParam& param, const TopoInfoWithNetLayerDetails* topoInfo,
//     const std::vector<std::vector<u32>>& subcommInfo, const std::vector<std::map<u32, omni::OmniChannelInfo>>& mapchannelInfo, 
...
HcclResult OmniTempCcu::PartitionChannels(HcclComm comm, const std::vector<HcclChannelDesc>& channelDescs)
{
    for (const auto &channel : channelDescs) {
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[OmniTempCcu][PartitionChannels] Rank[%d] channel to remoteRank[%d], Failed to "
                "get dieId. errNo[0x%016llx]", myRank_, remoteRank, HCCL_ERROR_CODE(ret)),
            ret);
        CHK_PRT_RET(dieId >= DIE_NUM,
            HCCL_ERROR("[OmniTempCcu][PartitionChannels] Rank[%d] channel to remoteRank[%d], dieId[%u] is "
                "invalid.", myRank_, remoteRank, dieId),
            HCCL_E_INTERNAL);
        HCCL_INFO("[OmniTempCcu][PartitionChannels] Rank[%d] channel to remoteRank[%d], insert to "
    ...
    //     HCCL_ERROR("[OmniTempCcu][PartitionChannels] Rank[%d], Unexpected channels size, "
    //         "die0 channels[%u], die1 channels[%u].", myRank_, channels_[0].size(), channels_[1].size()),
    //     HcclResult::HCCL_E_PARA);
    HCCL_DEBUG("[OmniTempCcu][PartitionChannels] Rank[%d], die0 channels[%u], die1 channels[%u].", myRank_,
...
HcclResult OmniTempCcu::CalcRes(HcclComm comm, const OpParam& param, const TopoInfoWithNetLayerDetails* topoInfo,
                                                      AlgResourceRequest& resourceRequest, const omni::XmlInfo& xmlInfo)
{
    ...
    std::vector<std::vector<omni::OmniNormalInstruction>> tmpsendRecvInstr;
    tmpsendRecvInstr.resize(DIE_NUM);
    for (auto& sendRecvInstr : xmlInfo.vecNormalInstruction) {
        if (std::find(rankGroup_[0].begin(), rankGroup_[0].end(), sendRecvInstr.sendRecvInfo.dstSliceInfo[0].remoteRank) != rankGroup_[0].end()) {
            tmpsendRecvInstr[0].push_back(sendRecvInstr);
        } else if (std::find(rankGroup_[1].begin(), rankGroup_[1].end(), sendRecvInstr.sendRecvInfo.dstSliceInfo[0].remoteRank) != rankGroup_[1].end()){
            tmpsendRecvInstr[1].push_back(sendRecvInstr);
        } else {
            HCCL_ERROR("[CalcRes] sendRecvInstr.remoteRank is %u not in rankGroup", sendRecvInstr.sendRecvInfo.dstSliceInfo[0].remoteRank);
        }
    }
    ...
    for (uint32_t dieId = 0; dieId < DIE_NUM; dieId++) {    // 2Die算法，需要执行两次
        ...
        kernelInfo.creator = [](const hcomm::CcuKernelArg &arg) {
            return std::make_unique<OmniKernelCcu>(arg);
        };

        auto kernelArg = std::make_shared<OmniKernelArgCcu>(myRank_, param, subCommRanks_, rankGroup_[dieId], handleSelfRank, tmpsendRecvInstr[dieId]);
        kernelInfo.kernelArg = kernelArg;
        kernelInfo.channels = channels_[dieId];
        resourceRequest.ccuKernelInfos.emplace_back(kernelInfo);
        HCCL_DEBUG("[OmniTempCcu][CalcRes] dieId=%u, channels=%llu, ccuKernelInfos=%llu",
            dieId, channels_[dieId].size(), resourceRequest.ccuKernelInfos.size());
    }

    resourceRequest.ccuKernelNum.push_back(dieNum);        // kernel数量

    HCCL_DEBUG("[OmniTempCcu::CalcRes] channelDescs.size()=%llu, dimsize=%llu, "
...
HcclResult OmniTempCcu::KernelRun(const OpParam& param, 
                                            const TemplateDataParams& templateDataParams,
                                            const TemplateResource& templateResource, const omni::XmlInfo& xmlInfo)
{
    HCCL_INFO("[OmniTempCcu] rankid [%u] KernelRun begin", mySubCommRank_);
    ...
    for(uint64_t dieIdx = 0; dieIdx < templateResource.ccuKernels.size(); dieIdx++) {
        std::unique_ptr<hcomm::CcuTaskArg> taskArg = std::make_unique<OmniTaskArgCcu>(
            inputAddr, outputAddr, scratchAddr, token, sliceSize, repeatNum, inputRepeatStride, outputRepeatStride);

        HCCL_INFO("[OmniTempCcu] [KernelRun] rankid [%u] input[%llu] outputAddr[%llu] scratchAddr[%llu] token[%llu] sliceSize[%llu] ,",
    ...
    HCCL_INFO("[OmniTempCcu] rankid [%u] KernelRun end", mySubCommRank_);
    return HcclResult::HCCL_SUCCESS;
}

u64 OmniTempCcu::CalcScratchMultiple(BufferType inBuffType, BufferType outBuffType)
{
