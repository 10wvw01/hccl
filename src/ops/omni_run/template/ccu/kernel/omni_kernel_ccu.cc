...
const std::string LOCAL_REDUCE_LOOP_BLOCK_TAG_OMNI_RUN{"OMNI"};

OmniKernelCcu::OmniKernelCcu(const hcomm::CcuKernelArg &arg)
    : CcuKernelAlgBase(arg)
{
    const OmniKernelArgCcu *kernelArg = dynamic_cast<const OmniKernelArgCcu *>(&arg);
    if (kernelArg == nullptr) {
        HCCL_ERROR("[OmniKernelCcu] kernelArg ptr is null.");
        return;
    }
    ...
    HCCL_INFO("rank[%u] OmniKernelCcu channels_ size %u", rankId_, channels_.size());
...
void OmniKernelCcu::ReduceLoopGroup(CcuRep::LocalAddr &outDstOrg, std::vector<CcuRep::LocalAddr> &srcOrg,
...
void OmniKernelCcu::CreateReduceLoop(uint32_t size, HcclDataType dataType, HcclDataType outputDataType,
                                                         HcclReduceOp opType, std::string loopName)
{
...
std::string OmniKernelCcu::GetLoopBlockTag(std::string loopType, int32_t index)
{
    return loopType + LOCAL_REDUCE_LOOP_BLOCK_TAG_OMNI_RUN + std::to_string(index);
}
 

HcclResult OmniKernelCcu::InitResources()
{
    // 创建Variable，用于交换地址及token
    if (channels_.size() == 0) {
        HCCL_ERROR("[OmniKernelCcu] RankId[%u] channels_ is empty", rankId_);
...
void OmniKernelCcu::DoRepeatOmni()
{
    for (auto& signalInfo : ccuSendRecvInfo_) {
        if (signalInfo.opType == omni::OP_LOCAL_COPY) { // groupCopy
            ...
            if (signalInfo.sendRecvInfo.srcSliceInfo.size() > 1 || signalInfo.sendRecvInfo.dstSliceInfo.size() > 1) {
                HCCL_WARNING("[DoRepeatOmni] groupCopy slicesize > 1");
            }

            for (auto& sliceInfo : signalInfo.sendRecvInfo.srcSliceInfo) {
            ...
            for (auto& sliceInfo : signalInfo.sendRecvInfo.dstSliceInfo) {
        ...
        if (signalInfo.opType == omni::OP_LOCAL_REDUCE) { // ReduceLoopGroup
            ...
            if (signalInfo.sendRecvInfo.dstSliceInfo.size() > 1) {
            ...
            ReduceLoopGroup(myOutput, myInput, groupOpSize_, signalInfo.sendRecvInfo.inputDataType, signalInfo.sendRecvInfo.outputDataType, signalInfo.sendRecvInfo.reduceType, "OMNI");            
        }

        if (signalInfo.opType == omni::OP_SEND_RECV_WRITE) { // WriteNb
            for (uint32_t i = 0; i < signalInfo.sendRecvInfo.srcSliceInfo.size(); i++) {
                ...
                src.token = token_[signalInfo.sendRecvInfo.srcSliceInfo[i].remoteRank];
                remoteDst.token = token_[signalInfo.sendRecvInfo.dstSliceInfo[i].remoteRank];

                if (signalInfo.sendRecvInfo.srcSliceInfo[i].sliceType == omni::BufferTypeTmp::INPUT) {
                    src.addr = input_[signalInfo.sendRecvInfo.srcSliceInfo[i].remoteRank];                
                }

                if (signalInfo.sendRecvInfo.srcSliceInfo[i].sliceType == omni::BufferTypeTmp::OUTPUT) {
                    src.addr = output_[signalInfo.sendRecvInfo.srcSliceInfo[i].remoteRank];
                }

                if (signalInfo.sendRecvInfo.srcSliceInfo[i].sliceType == omni::BufferTypeTmp::HCCL_BUFFER) {
                    src.addr = scratchAddr_;
                }

                for (uint32_t j = 0; j < signalInfo.sendRecvInfo.srcSliceInfo[i].sliceIdx; j++) {
                    src.addr += sliceSize_; // 一个sliceId 对应一份sliceSize
                }

                if (signalInfo.sendRecvInfo.dstSliceInfo[i].sliceType == omni::BufferTypeTmp::INPUT) {
                    remoteDst.addr = input_[signalInfo.sendRecvInfo.dstSliceInfo[i].remoteRank];                
                }

                if (signalInfo.sendRecvInfo.dstSliceInfo[i].sliceType == omni::BufferTypeTmp::OUTPUT) {
                    remoteDst.addr = output_[signalInfo.sendRecvInfo.dstSliceInfo[i].remoteRank];
                }

                if (signalInfo.sendRecvInfo.dstSliceInfo[i].sliceType == omni::BufferTypeTmp::HCCL_BUFFER) {
                    remoteDst.addr = scratchAddr_;
                }

                for (uint32_t j = 0; j < signalInfo.sendRecvInfo.dstSliceInfo[i].sliceIdx; j++) {
                    remoteDst.addr += sliceSize_; // 一个sliceId 对应一份sliceSize
                }

                event_.SetMask(1 << rankId_);
                WriteNb(rankId2Channel_[signalInfo.sendRecvInfo.dstSliceInfo[i].remoteRank], remoteDst, src, sliceSize_, event_);
                WaitEvent(event_);
            }
        }

        if (signalInfo.opType == omni::OP_SEND_WRITE) { // WriteNb
            for (uint32_t i = 0; i < signalInfo.sendRecvInfo.srcSliceInfo.size(); i++) {
                CcuRep::RemoteAddr remoteDst = CreateRemoteAddr();
                CcuRep::LocalAddr src  = CreateLocalAddr();
                src.token = token_[signalInfo.sendRecvInfo.srcSliceInfo[i].remoteRank];
                ...
                if (signalInfo.sendRecvInfo.srcSliceInfo[i].sliceType == omni::BufferTypeTmp::INPUT) { // input
                    src.addr = input_[signalInfo.sendRecvInfo.srcSliceInfo[i].remoteRank];                
                }
                ...
                if (signalInfo.sendRecvInfo.dstSliceInfo[i].sliceType == omni::BufferTypeTmp::INPUT) { // input
                    remoteDst.addr = input_[signalInfo.sendRecvInfo.dstSliceInfo[i].remoteRank];                
                }
        ...
        if (signalInfo.opType == omni::OP_SEND_RECV_WRITE_REDUCE) { // WriteReduceNb
            for (uint32_t i = 0; i < signalInfo.sendRecvInfo.srcSliceInfo.size(); i++) {
                CcuRep::RemoteAddr remoteDst = CreateRemoteAddr();
                CcuRep::LocalAddr src  = CreateLocalAddr();
                src.token = token_[signalInfo.sendRecvInfo.srcSliceInfo[i].remoteRank];
                ...
                event_.SetMask(1 << rankId_);
                WriteReduceNb(rankId2Channel_[signalInfo.sendRecvInfo.dstSliceInfo[i].remoteRank], remoteDst, src, sliceSize_, signalInfo.sendRecvInfo.inputDataType, signalInfo.sendRecvInfo.reduceType, event_);
                WaitEvent(event_);
            }
        }

        if (signalInfo.opType == omni::OP_SEND_WRITE_REDUCE) { // WriteReduceNb
            for (uint32_t i = 0; i < signalInfo.sendRecvInfo.srcSliceInfo.size(); i++) {
                CcuRep::RemoteAddr remoteDst = CreateRemoteAddr();
                CcuRep::LocalAddr src  = CreateLocalAddr();
                src.token = token_[signalInfo.sendRecvInfo.srcSliceInfo[i].remoteRank];
        ...
        if (signalInfo.opType == omni::OP_RECV_WRITE_REDUCE) { // omni::OP_RECV_WRITE_REDUCE
            // ccu 没有对应的函数
        }

        if (signalInfo.opType == omni::OP_SEND_RECV_READ) { // ReadNb
            for (uint32_t i = 0; i < signalInfo.sendRecvInfo.srcSliceInfo.size(); i++) {
                CcuRep::RemoteAddr remoteDst = CreateRemoteAddr();
                CcuRep::LocalAddr src  = CreateLocalAddr();
                src.token = token_[signalInfo.sendRecvInfo.srcSliceInfo[i].remoteRank];
                ...
                event_.SetMask(1 << rankId_);
                ReadNb(rankId2Channel_[signalInfo.sendRecvInfo.dstSliceInfo[i].remoteRank], src, remoteDst, sliceSize_, event_);
                WaitEvent(event_);
            }
        }

        if (signalInfo.opType == omni::OP_SEND_READ) { // omni::OP_SEND_READ
            // ccu 没有对应的函数
        }

        if (signalInfo.opType == omni::OP_RECV_READ) { // ReadNb
            for (uint32_t i = 0; i < signalInfo.sendRecvInfo.srcSliceInfo.size(); i++) {
                CcuRep::RemoteAddr remoteDst = CreateRemoteAddr();
                CcuRep::LocalAddr src  = CreateLocalAddr();
                src.token = token_[signalInfo.sendRecvInfo.srcSliceInfo[i].remoteRank];
        ...
        if (signalInfo.opType == omni::OP_SEND_RECV_READ_REDUCE) { // ReadReduceNb
            for (uint32_t i = 0; i < signalInfo.sendRecvInfo.srcSliceInfo.size(); i++) {
                CcuRep::RemoteAddr remoteDst = CreateRemoteAddr();
                CcuRep::LocalAddr src  = CreateLocalAddr();
                src.token = token_[signalInfo.sendRecvInfo.srcSliceInfo[i].remoteRank];
                ...
                event_.SetMask(1 << rankId_);
                ReadReduceNb(rankId2Channel_[signalInfo.sendRecvInfo.dstSliceInfo[i].remoteRank], src, remoteDst, sliceSize_, signalInfo.sendRecvInfo.inputDataType, signalInfo.sendRecvInfo.reduceType, event_);
                WaitEvent(event_);
            }
        }

        if (signalInfo.opType == omni::OP_SEND_READ_REDUCE) { // omni::OP_SEND_READ_REDUCE
            // ccu 没有对应的函数
        }

        if (signalInfo.opType == omni::OP_RECV_READ_REDUCE) { // ReadReduceNb
            for (uint32_t i = 0; i < signalInfo.sendRecvInfo.srcSliceInfo.size(); i++) {
                CcuRep::RemoteAddr remoteDst = CreateRemoteAddr();
                CcuRep::LocalAddr src  = CreateLocalAddr();
                src.token = token_[signalInfo.sendRecvInfo.srcSliceInfo[i].remoteRank];
        ...
        if (signalInfo.opType == omni::OP_GROUP_BROAD_CAST) { // GroupBroadcast
            if (signalInfo.sendRecvInfo.srcSliceInfo.size() > 1) {
                HCCL_WARNING("[DoRepeatOmni] GroupBroadcast srcSliceInfo > 1");
            }

            CcuRep::LocalAddr src = CreateLocalAddr();
            std::vector<CcuRep::RemoteAddr> dst;
            src.token = token_[signalInfo.sendRecvInfo.srcSliceInfo[0].remoteRank];
            ...
            if (signalInfo.sendRecvInfo.dstSliceInfo[0].sliceType == omni::BufferTypeTmp::INPUT) {
                tmpdst.addr = input_[rankId_];
            }

            if (signalInfo.sendRecvInfo.dstSliceInfo[0].sliceType == omni::BufferTypeTmp::OUTPUT) {
                tmpdst.addr = output_[rankId_];
            }

            if (signalInfo.sendRecvInfo.dstSliceInfo[0].sliceType == omni::BufferTypeTmp::HCCL_BUFFER) {
                tmpdst.addr = scratchAddr_;
            }

            for (uint64_t i = 0; i < signalInfo.sendRecvInfo.srcSliceInfo[0].sliceIdx; i++) {
                tmpdst.addr += sliceSize_; // 一个sliceId 对应一份sliceSize
            }
        ...
        if (signalInfo.opType == omni::OP_GROUP_REDUCE) { // GroupReduce
            ...
            if (handleSelfRank_) {
                ...
                for (uint64_t i = 0; i < signalInfo.sendRecvInfo.dstSliceInfo[0].sliceIdx; i++) {
            ...
            //self defined broadcast
            if (handleSelfRank_) {
                GroupReduce(channels, dst, src, groupOpSize_,signalInfo.sendRecvInfo.inputDataType, signalInfo.sendRecvInfo.outputDataType, signalInfo.sendRecvInfo.reduceType);
            } else {
                GroupReduceWithoutMyRank(channels, dst, src, groupOpSize_,signalInfo.sendRecvInfo.inputDataType, signalInfo.sendRecvInfo.outputDataType, signalInfo.sendRecvInfo.reduceType);
            }
        }
    }
}

HcclResult OmniKernelCcu::Algorithm()
{
    HCCL_INFO("[omniRun_ccu_kernel] RankId[%u] Omni run begin.", rankId_);
    ...
    HCCL_INFO("[omniRun_ccu_kernel] RankId[%u] Omni run end.", rankId_);
    return HcclResult::HCCL_SUCCESS;
}

std::vector<uint64_t> OmniKernelCcu::GeneArgs(const CcuTaskArg &arg)
{
    HCCL_INFO("GeneArgs begin RankId[%u]", rankId_);

    const OmniTaskArgCcu *taskArg = dynamic_cast<const OmniTaskArgCcu *>(&arg);
    if (taskArg == nullptr) {
        HCCL_ERROR("[OmniKernelCcu] taskArg ptr is null. RankId[%u]", rankId_);
    }
    ...
    HCCL_INFO("[OmniKernelCcu] RankId[%u], inputAddr[%llu], outputAddr[%llu], scratchAddr[%llu], tokenInfo[%llu], sliceSize[%llu], "
