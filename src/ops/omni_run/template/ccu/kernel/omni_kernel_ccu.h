...
#ifndef OPS_HCCL_OMNI_RUN_TEMPLATE_CCU_KERNEL_OMNI_KERNEL_CCU_H_
#define OPS_HCCL_OMNI_RUN_TEMPLATE_CCU_KERNEL_OMNI_KERNEL_CCU_H_
...
class OmniKernelArgCcu : public hcomm::CcuKernelArg{
public:
    OmniKernelArgCcu(uint32_t rankId, const OpParam &opParam,
        const std::vector<std::vector<RankId>> &subCommRanks, const std::vector<RankId> &rankGroup,
        bool handleSelfRank, const std::vector<omni::OmniNormalInstruction>& sendRecvInfo)
    ...
    ~OmniKernelArgCcu() override {}

    hcomm::CcuKernelSignature GetKernelSignature() const override
    {
        hcomm::CcuKernelSignature signature;
        GenerateCcuKernelSignature(signature, "OmniKernelArgCcu", opParam_, subCommRanks_);
        return signature;
    }
    ...
    std::vector<omni::OmniNormalInstruction> sendRecvInfo_;
};

class OmniTaskArgCcu : public hcomm::CcuTaskArg {
public:
    explicit OmniTaskArgCcu(uint64_t inputAddr, uint64_t outputAddr, uint64_t scratchAddr,
...
class OmniKernelCcu : public CcuKernelAlgBase {
public:
    OmniKernelCcu(const hcomm::CcuKernelArg &arg);
    ~OmniKernelCcu() override {}
...
private:
    ...
    std::vector<omni::OmniNormalInstruction> ccuSendRecvInfo_;
    bool handleSelfRank_;
...
#endif // OPS_HCCL_OMNI_RUN_TEMPLATE_CCU_KERNEL_OMNI_KERNEL_CCU_H_