...
HcclResult CalcInputOutputSize(const u64* sendCountsData, const u64* recvCountsData,
    const u64* sdisplsData, const u64* rdisplsData, const u32 userRankSize,
...
HcclResult ContructVarData(const u64* sendCountsData, const u64* recvCountsData,
    const u64* sdisplsData, const u64* rdisplsData, const u32 userRankSize,
    const u32 rankSize, OpParam &param)
{
...
HcclResult CheckBufNullptr(const u64* countsData, u32 rankSize, const void* buf,
    const std::string funcName, const std::string bufName)
{
    bool zeroFlag = true;
    ...
    RPT_INPUT_ERR(buf == nullptr, "EI0003",
...
HcclResult CheckOmniRunInputPara(const HcclComm comm, const void *sendBuf,
    const void *sendCounts, const void *sdispls, const HcclDataType sendType,
...
HcclResult OmniRunConstructOpParam(const void *sendBuf, const void *sendCounts,
    const void *sdispls, const void *recvBuf, const void *recvCounts,
    const void *rdispls, HcclDataType dataType, HcclComm comm, aclrtStream stream,
    const std::string &tag, HcclCMDType opType, u32 rankSize, OpMode opMode,
    u64 varMemSize, OpParam &param, const char* xmlPath)
{
    ...
    CHK_RET(ContructVarData(sendCountsData, recvCountsData, sdisplsData, rdisplsData,
        rankSize, rankSize, param));
    u64* data = reinterpret_cast<u64*>(param.varData);
    ...
    if (xmlPath != nullptr && xmlPath[0] != '\0') {
        int pathRet = sprintf_s(param.xmlPath, sizeof(param.xmlPath), "%s", xmlPath);
        if (pathRet <= 0) {
            HCCL_ERROR("failed to fill param.xmlPath");
            return HCCL_E_INTERNAL;
        }
    }
...
HcclResult OmniRunOutPlaceCommon(const void *sendBuf, const void *sendCounts,
    const void *sdispls, const void *recvBuf, const void *recvCounts,
    const void *rdispls, HcclDataType dataType, HcclComm comm, aclrtStream stream,
    const std::string &tag, HcclCMDType opType, u32 rankSize, OpMode opMode,
    const ResPackGraphMode &resPack, const char* xmlPath)
{
    ...
    CHK_RET(OmniRunConstructOpParam(sendBuf, sendCounts, sdispls, recvBuf, recvCounts, rdispls,
        dataType, comm, stream, tag, opType, rankSize, opMode, varMemSize, param, xmlPath));
...
// ---------------------------------------------------------------------------
// HcclOmniRun: C API entry point
// ---------------------------------------------------------------------------
HcclResult HcclOmniRun(const void *sendBuf, const void *recvBuf,
    ...
    HcclComm comm, aclrtStream stream)
{
    ...
    if (opParam == nullptr || opParamSize < OP_PARAM_BLOB_SIZE) {
        HCCL_ERROR("[HcclOmniRun] opParam is null or too small (size=%lu, required=%lu)",
                   opParamSize, OP_PARAM_BLOB_SIZE);
        return HCCL_E_PARA;
    }
    ...
    CHK_RET(CheckOmniRunInputPara(comm, sendBuf, sendCounts, sdisplsPtr, sendType,
                                  recvBuf, recvCounts, rdisplsPtr, recvType, stream));
    ...
    // Buffer null checks
    CHK_RET(CheckBufNullptr(reinterpret_cast<const u64*>(sendCounts), rankSize, sendBuf,
        string(__func__), "sendBuf"));
    CHK_RET(CheckBufNullptr(reinterpret_cast<const u64*>(recvCounts), rankSize, recvBuf,
        string(__func__), "recvBuf"));
    ...
    CHK_RET_AND_PRINT_IDE(
        ops_hccl::OmniRunOutPlaceCommon(sendBuf, sendCounts, sdisplsPtr, recvBuf, recvCounts,
            rdisplsPtr, recvType, comm, stream, tag, HcclCMDType::HCCL_CMD_ALLTOALLV, rankSize,
            OpMode::OPBASE, ResPackGraphMode(), xmlPath),
        tag.c_str());
