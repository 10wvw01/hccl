...
OmniSliceInfo BinaryParser::ParseSlice(uint32_t sliceValue) {
    OmniSliceInfo slice;
    slice.sliceType = static_cast<BufferTypeTmp>(
        ExtractBits32(sliceValue, field::SLICE_BUFFER_TYPE_WIDTH, field::SLICE_BUFFER_TYPE_OFFSET));
    slice.sliceIdx = ExtractBits32(sliceValue, field::SLICE_IDX_WIDTH, field::SLICE_IDX_OFFSET);
    slice.remoteRank = ExtractBits32(sliceValue, field::SLICE_RANK_ID_WIDTH, field::SLICE_RANK_ID_OFFSET);
    slice.remoteRecvRank = ExtractBits32(sliceValue, field::SLICE_RECV_RANK_ID_WIDTH, field::SLICE_RECV_RANK_ID_OFFSET);
...
void BinaryParser::ParseControlInstruction(XmlInfo& xmlInfo, const std::vector<uint8_t>& data,
    ...
    for (uint32_t i = 0; i < srcSliceNum; ++i) {
        ...
        uint32_t sliceValue = static_cast<uint32_t>(data[offset]) |
                             (static_cast<uint32_t>(data[offset+1]) << 8) |
                             (static_cast<uint32_t>(data[offset+2]) << 16) |
                             (static_cast<uint32_t>(data[offset+3]) << 24);
        offset += SLICE_ENTRY_SIZE;

        instr.sendRecvInfo.srcSliceInfo.push_back(ParseSlice(sliceValue));
    }
    ...
    for (uint32_t i = 0; i < dstSliceNum; ++i) {
        ...
        instr.sendRecvInfo.dstSliceInfo.push_back(ParseSlice(sliceValue));
    }
