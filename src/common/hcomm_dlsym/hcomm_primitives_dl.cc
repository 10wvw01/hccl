...
#ifdef HCOMM_TIMEOUT_FLOAT_TYPE
    HCCL_COMPAT_ERROR("[HcclSetNotifyWaitTimeOut] HCOMM_TIMEOUT_FLOAT_TYPE IS DEFINED. timeout[%u] -> float[%f]", timeout, static_cast<float>(timeout));
    return static_cast<HcclResult>(HcommSetNotifyWaitTimeOut(static_cast<float>(timeout)));
#else
    HCCL_COMPAT_ERROR("[HcclSetNotifyWaitTimeOut] HCOMM_TIMEOUT_FLOAT_TYPE NOT DEFINED. timeout[%u]", timeout);
