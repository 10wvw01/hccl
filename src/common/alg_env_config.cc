...
HcclResult ParseBandWidthRSX(int flag)
 	 ...
 	 HcclResult ParseBandWidthGX(int flag)
 	 {
 	     char* p = getenv("HCCL_BW_GX");
 	     if(p == nullptr){
 	         g_algEnvConfig.hccl_g_x_bw = HCCL_BW_DEFAULT;
 	         HCCL_WARNING("HCCL_BW_GX is nullptr, flag[%d]", flag);
 	         return HCCL_SUCCESS;
 	     }
 	     std::string bwEnv(p);
 	 
 	     if (!IsValidNumberFormat(bwEnv)) {
 	         HCCL_INFO("HCCL_BW_GX[%s] format is invalid, use default. flag[%d]",
 	             bwEnv.c_str(), flag);
 	         g_algEnvConfig.hccl_g_x_bw = HCCL_BW_DEFAULT;
 	         return HCCL_E_PARA;
 	     }
 	     ...
 	     double bw = 0;
 	     if (SalStrToDouble(bwEnv, bw) != HCCL_SUCCESS) {
 	         HCCL_INFO("HCCL_BW_GX[%s] parse failed, use default. flag[%d]",
 	             bwEnv.c_str(), flag);
 	         g_algEnvConfig.hccl_g_x_bw = HCCL_BW_DEFAULT;
 	         return HCCL_E_PARA;
 	     }
 	     g_algEnvConfig.hccl_g_x_bw = bw;
 	     HCCL_INFO("[%s] hccl_g_x_bw[%f], flag[%d]", __func__, g_algEnvConfig.hccl_g_x_bw, flag);
 	     return HCCL_SUCCESS;
 	 }
 	 
 	 HcclResult ParseBandWidthGY(int flag)
 	 {
 	     char* p = getenv("HCCL_BW_GY");
 	     if(p == nullptr){
 	         g_algEnvConfig.hccl_g_y_bw = HCCL_BW_DEFAULT;
 	         HCCL_WARNING("HCCL_BW_GY is nullptr, flag[%d]", flag);
 	         return HCCL_SUCCESS;
 	     }
 	     std::string bwEnv(p);
 	 
 	     if (!IsValidNumberFormat(bwEnv)) {
 	         HCCL_INFO("HCCL_BW_GY[%s] format is invalid, use default. flag[%d]",
 	             bwEnv.c_str(), flag);
 	         g_algEnvConfig.hccl_g_y_bw = HCCL_BW_DEFAULT;
 	         return HCCL_E_PARA;
 	     }
 	     ...
 	     double bw = 0;
 	     if (SalStrToDouble(bwEnv, bw) != HCCL_SUCCESS) {
 	         HCCL_INFO("HCCL_BW_GY[%s] parse failed, use default. flag[%d]",
 	             bwEnv.c_str(), flag);
 	         g_algEnvConfig.hccl_g_y_bw = HCCL_BW_DEFAULT;
 	         return HCCL_E_PARA;
 	     }
 	     g_algEnvConfig.hccl_g_y_bw = bw;
 	     HCCL_INFO("[%s] hccl_g_y_bw[%f], flag[%d]", __func__, g_algEnvConfig.hccl_g_y_bw, flag);
 	     return HCCL_SUCCESS;
 	 }
 	 ...
 	 double GetExternalInputBandWidthGX(){
 	     std::lock_guard<std::mutex> lock(g_algEnvConfigMutex);
 	     HCCL_INFO("[%s]: [%f]", __func__, g_algEnvConfig.hccl_g_x_bw);
 	     return g_algEnvConfig.hccl_g_x_bw;
 	 }
 	 
 	 double GetExternalInputBandWidthGY(){
 	     std::lock_guard<std::mutex> lock(g_algEnvConfigMutex);
 	     HCCL_INFO("[%s]: [%f]", __func__, g_algEnvConfig.hccl_g_y_bw);
 	     return g_algEnvConfig.hccl_g_y_bw;
 	 }
