...
该环境变量用于配置AllReduce、AllGather、ReduceScatter、Braodcast、Reduce算子在特定两维度并行通信算法下的数据切分比例。
...
- 该环境变量仅影响HCCL已经选择两维度并行通信算法时AllReduce、AllGather、ReduceScatter、Braodcast、Reduce算子的两片数据分配比例，不用于选择通信算法。
- AllReduce和ReduceScatter算子中，两片数据0、1的目标切分比例为“环境变量取值”和“1-环境变量取值”；AllGather算子中，算法内部使用的两片数据切分顺序与AllReduce、ReduceScatter、Braodcast、Reduce相反，即两片数0、1的目标切分比例为“1-环境变量取值”和“环境变量取值”。
...
- 该环境变量仅在AllReduce、AllGather、ReduceScatter、Braodcast、Reduce算子选择两维度并行通信算法时生效。若当前拓扑、数据量、数据类型、reduce类型或算子展开模式选择了其他通信算法，则该环境变量不生效。
