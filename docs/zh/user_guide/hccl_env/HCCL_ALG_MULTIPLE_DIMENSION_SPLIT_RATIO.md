...
该环境变量用于配置AllReduce、AllGather、ReduceScatter算子在特定跨维度并行通信算法下的数据切分比例。

在部分跨框、跨维度通信场景下，HCCL会将每轮待通信的数据切分为两片，并分别走两条可并行的通信路径：一片数据先执行Mesh通信、再执行NHR通信，另一片数据先执行NHR通信、再执行Mesh通信。该环境变量用于调整这两片数据的大小，使耗时较长的通信路径分配较少数据，耗时较短的通信路径分配较多数据，从而改善两条路径的负载均衡。

该环境变量需要配置为数字，取值范围：\[0,1\]，默认值：0.5。

需要注意：

- 该环境变量仅影响HCCL已经选择跨维度并行通信算法时AllReduce、AllGather、ReduceScatter算子的两片数据分配比例，不用于选择通信算法。
...
- 一般情况下，用户保持默认值即可。仅建议在确认当前业务处于跨框、跨维度通信场景，且需要针对特定组网、数据量进行性能调优时修改该环境变量。
...
- 跨维度并行通信算法会使用“数据片0”和“数据片1”对应两条通信路径，但两个算子的分片与通信路径的对应关系相反。ReduceScatter中，`R`对应数据片0，即“Mesh -> NHR”通信路径；AllGather中，`R`对应数据片1，即“NHR -> Mesh”通信路径。
...
- 该环境变量仅在AllReduce、AllGather、ReduceScatter算子选择跨维度并行通信算法时生效。若当前拓扑、数据量、数据类型、reduce类型或算子展开模式选择了其他通信算法，则该环境变量不生效。
