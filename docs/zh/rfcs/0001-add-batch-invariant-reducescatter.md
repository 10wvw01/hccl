...
Scratch Memory:
┌─────────────────────────────────────────────┐
│ IM[0]: offset = 0 * localStrideSize         │  ← Round 0 intermediate result
├─────────────────────────────────────────────┤
│ IM[1]: offset = 1 * localStrideSize         │  ← Round 1 intermediate result
├─────────────────────────────────────────────┤
│ ...                                         │
├─────────────────────────────────────────────┤
│ IM[N]: offset = N * localStrideSize         │  ← Round N intermediate result
└─────────────────────────────────────────────┘
```
...
The tree reduction guarantees deterministic addition ordering: for `rankSizeY = 4`, the reduction order is `(IM_0 + IM_1) + (IM_2 + IM_3)`, independent of rank id.
...
When conditions are not met, the `MatchBIRS()` check automatically falls back to the existing algorithm path.
...
| Additional scratch memory overhead | Increased memory usage for large messages | Requires `rankSizeY × localStrideSize` scratch space; pre-allocated via `CalcResRequest` |
