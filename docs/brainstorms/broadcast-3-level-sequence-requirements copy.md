...
## 方案选定：Scatter 3 级 + AllGather 3 级（6 步串行）
...
1. **Selector 不检查 level0Topo**（对应 R15）— 原 selector 仅检查 `topoLevelNums==3 && !level2Uboe`，不检查 `level0Topo == MESH_1D`，CLOS 拓扑会路由到 sequence executor。**已缓解**：selector 条件改为 `level0Topo == MESH_1D && !level2Uboe`（见 §修改后的 selector 逻辑）。
...
1. **selector 层（对应 R15）**：selector 条件纳入 `level0Topo == MESH_1D` 检查（见 §修改后的 selector 逻辑），CLOS/MESH_1D_CLOS 拓扑回退 parallel executor。**该缓解已落地到 selector 条件。**
...
### 1. 入口：HcclBroadcast → Selector → HcclExecOp
