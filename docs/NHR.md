...
在规模较大、通信节点数量较多的组网中，由于Ring算法的通信步数与通信节点规模成正相关，所以容易出现较大延迟，因此Ring算法的切分数据块的方式对小数据包场景不友好，更适合大数据包传输场景。在集群规模不是2的整数次幂时，HD算法将退化为RHD算法，会引入额外的通信步数和开销，产生N-1规模集群下的通信性能比N规模集群通信性能还差的现象。此外，由于HD算法每个通信阶段的对象在变化，导致通信链路也发生变化，这在大流量场景可能会引起交换机的流量冲突，从而导致带宽下降。
...
**图 1**  rank size为4时NHR算法通信过程<a name="fig473964014451"></a>  
...
**图 2**  rank size为5时NHR算法通信过程<a name="fig39515544610"></a>  
