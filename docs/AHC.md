...
**图 1**  AHC 基于逻辑同号卡实现AllReduce过程 （5个rank， 2+3两个分组）<a name="fig12416646135910"></a>  
![](figures/AHC-基于逻辑同号卡实现AllReduce过程-（5个rank-2+3两个分组）.png "AHC-基于逻辑同号卡实现AllReduce过程-（5个rank-2+3两个分组）")
...
2.  划分“逻辑同号卡”，基于逻辑同号卡实现组间allredcue。
    1.  将每个group中待执行reduce操作的数据，按照group内各NPU卡间的数据边界进行切分，形成若干不均匀的数据块。
    2.  每个group中的每份数据，在其他所有group中各有一份对应的、大小相同的数据。按照数据对应关系，group间的NPU也存在对应关系。我们称存在对应关系的NPU 为“逻辑同号卡”。
    3.  在逻辑同号卡间执行AllReduce 操作。
