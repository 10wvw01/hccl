...
static constexpr u32 NOTIFY_IDX_DATA_SIGNAL  = 1;
...
HcclResult AiCpuLauncher::SendWrite(const DataInfo &sendInfo, const ThreadHandle &thread)
{
    ...
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyWaitOnThreadDefault(thread, sendChannel.handle, NOTIFY_IDX_ACK, execTimeout)));
    ...
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyRecordOnThreadDefault(thread, sendChannel.handle, NOTIFY_IDX_DATA_SIGNAL)));
    return HCCL_SUCCESS;
}
...
HcclResult AiCpuLauncher::RecvWrite(const DataInfo &recvInfo, const ThreadHandle &thread)
{
    ...
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyRecordOnThreadDefault(thread, recvChannel.handle, NOTIFY_IDX_ACK)));
    u32 execTimeout = ExecTimeoutManager::Instance().GetExecTimeout();
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyWaitOnThreadDefault(thread, recvChannel.handle, NOTIFY_IDX_DATA_SIGNAL, execTimeout)));
    return HCCL_SUCCESS;
}
...
HcclResult AiCpuLauncher::SendRead(const DataInfo &sendInfo, const ThreadHandle &thread)
{
    const ChannelInfo &sendChannel = sendInfo.channel_;
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyRecordOnThreadDefault(thread, sendChannel.handle, NOTIFY_IDX_ACK)));
    u32 execTimeout = ExecTimeoutManager::Instance().GetExecTimeout();
    CHK_RET(static_cast<HcclResult>(
...
HcclResult AiCpuLauncher::RecvRead(const DataInfo &recvInfo, const ThreadHandle &thread)
{
    ...
    u32 execTimeout = ExecTimeoutManager::Instance().GetExecTimeout();
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyWaitOnThreadDefault(thread, recvChannel.handle, NOTIFY_IDX_ACK, execTimeout)));
    for (u32 i = 0; i < repeatNum; i++) {
        const DataSlice srcSlice = srcSlices[i];
        const DataSlice dstSlice = dstSlices[i];
        if (srcSlice.size_ == 0) { continue; }
    ...
    CHK_RET(static_cast<HcclResult>(
        HcommChannelNotifyRecordOnThreadDefault(thread, recvChannel.handle, NOTIFY_IDX_DATA_SIGNAL)));
    return HCCL_SUCCESS;
}
