...
DEFINE_WEAK_FUNC(CcuResult, HcommCcuKernelRegister, CcuInsHandle insHandle, uint32_t dieId,
    const char *kernelFuncName, const void *kernelFunc, const void **kernelArgs,
    uint32_t argNum, CcuKernelHandle *kernelHandle);
DEFINE_WEAK_FUNC(CcuResult, HcommCcuKernelRegisterEnd, CcuInsHandle insHandle);
DEFINE_WEAK_FUNC(CcuResult, HcommCcuKernelLaunch, ThreadHandle threadHandle,
