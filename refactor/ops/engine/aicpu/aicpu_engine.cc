...
static void UpdateAicpuTimeoutCtx(const OpParam &param, AlgResourceCtxSerializable &resCtx)
{
    ...
    HCCL_INFO("[TIMEOUT-TRACE][UpdateAicpuTimeoutCtx] execTimeout=%u waitTimeout=%u fullTimeout=%u hostNotifyTimeout=%u kernelLaunchTimeout=%u hcommDefaultSupported=%u.",
                                   ...
                                   TopoInfoWithNetLayerDetails &topoInfo)
{
    ...
    // 4. 创建 cpuTsThread 并导出到 AICPU_TS（必须在 HcclGetThread 之前，保持与原始 HcclExecOp 一致的顺序）
    ...
    // 5. 创建 AICPU 线程（对应 HcclGetThread）
    CHK_RET(HcclGetThread(comm, param, resReq));


    // 6. 获取主流信息并导出为 exportedCpuTsThread（对应原始 GetMainThreadInfo + HcclThreadExportToCommEngine）
    if (param.engine == COMM_ENGINE_AICPU_TS || param.engine == COMM_ENGINE_CPU) {
    ...
    // 7. 按层级申请 channel（对应 HcclGetChannel + HcclGetChannelImpl）
...
{
    resCtx_.isHcclThreadAcquireWithConfigSupported = HcommIsSupportHcclThreadAcquireWithConfig();
    if (param.engine == COMM_ENGINE_AICPU_TS || param.engine == COMM_ENGINE_CPU) {
        ...
        if (GetUnfoldThreadInfo(comm, param, existingUnfoldThread) == HCCL_SUCCESS) {
            resCtx_.unfoldThread = existingUnfoldThread;
            unfoldReady = true;
            HCCL_INFO("[HcclGetThread] reuse unfoldThread [%lu]", resCtx_.unfoldThread);
        }
        ...
        if (HcommIsSupportHcclThreadAcquireWithConfig()) {
            ...
            threadConfigs[0].notifyNumPerThread = resReq.notifyNumOnMainThread + 1;
            HCCL_DEBUG("[HcclGetThread] AICPU thread[0] notify num[%u].", threadConfigs[0].notifyNumPerThread);
            CHK_PRT_RET(resReq.notifyNumPerThread.size() < threadNum - 1,
                HCCL_ERROR("[HcclGetThread] notifyNumPerThread size[%zu] is less than slaveThreadNum[%u].",
                    resReq.notifyNumPerThread.size(), threadNum - 1), HCCL_E_INTERNAL);
            for (u32 i = 1; i < threadNum; i++) {
                threadConfigs[i].notifyNumPerThread = resReq.notifyNumPerThread[i - 1];
            ...
            if (!unfoldReady) {
                ...
                CHK_RET(HcclThreadAcquireWithConfig(comm, COMM_ENGINE_CPU, 1, THREAD_TYPE_TS,
                    &unfoldThreadConfig, &resCtx_.unfoldThread));
            }
            CHK_RET(SaveMainThreadInfo(comm, param, threads[0], resReq.notifyNumOnMainThread + 1));
        } else {
    ...
    ThreadHandle thread, u32 notifyNum)
{
    ...
    void *ctx = nullptr;
    HcclResult ret = HcclEngineCtxGet(comm, param.algTag, CommEngine::COMM_ENGINE_CPU_TS, &ctx, &size);
    if (ret != HCCL_SUCCESS) {
...
// 对应原始 GetMainThreadInfo：从 HCOMM EngineCtx 获取主流信息
HcclResult AiCpuEngine::GetMainThreadInfo(HcclComm comm, const OpParam &param, ThreadHandle &thread, u32 &notifyNum)
{
    uint64_t size = sizeof(ThreadHandle) + sizeof(u32);
    void *ctx = nullptr;
...
// 对应原始 GetUnfoldThreadInfo：从 HCOMM EngineCtx 获取展开流线程信息
HcclResult AiCpuEngine::GetUnfoldThreadInfo(HcclComm comm, const OpParam &param, ThreadHandle& unfoldThread)
{
    ...
    ThreadHandle unfoldThread)
{
    ...
    void *ctx = nullptr;
    char unfoldAlgTag[ALG_TAG_LENGTH] = {0};
    int snprintfRet = snprintf_s(unfoldAlgTag, sizeof(unfoldAlgTag), sizeof(unfoldAlgTag) - 1, "%s_unfold", param.algTag);
    CHK_PRT_RET(snprintfRet <= 0, HCCL_ERROR("[%s] failed to fill unfoldAlgTag", __func__), HCCL_E_INTERNAL);
    HcclResult ret = HcclEngineCtxGet(comm, unfoldAlgTag, CommEngine::COMM_ENGINE_CPU_TS, &ctx, &size);
    if (ret != HCCL_SUCCESS) {
...
HcclResult AiCpuEngine::HcclAicpuKernelEntranceLaunch(const OpParam &param)
{
    ...
    // 序列化 resCtx_ 并拷贝到 device 可访问内存（对应 src 的 HcclMemcpyCtxHostToDevice）
    HCCL_INFO("[TIMEOUT-TRACE][Serialize] before serialize: resCtx_.waitTimeout=%u resCtx_.fullTimeout=%u",
        resCtx_.waitTimeout, resCtx_.fullTimeout);
    std::vector<char> seq = resCtx_.Serialize();
    uint64_t seqSize = seq.size();
    void *ctx = nullptr;
    uint64_t existSize = 0;
    HcclComm comm = static_cast<HcclComm>(resCtx_.commInfoPtr);
    // 尝试获取已有的 device context，没有则创建
    HcclResult getRet = HcclEngineCtxGet(comm, param.algTag, COMM_ENGINE_AICPU_TS, &ctx, &existSize);
    if (getRet != HCCL_SUCCESS) {
        HCCL_INFO("[AicpuKernelEntranceLaunch] create new EngineCtx, algTag[%s], size[%llu]",
            param.algTag, seqSize);
        CHK_RET(HcclEngineCtxCreate(comm, param.algTag, COMM_ENGINE_AICPU_TS, seqSize, &ctx));
    }
    // 将序列化数据拷贝到 device 可访问内存
    CHK_RET(HcclEngineCtxCopy(comm, COMM_ENGINE_AICPU_TS, param.algTag, seq.data(), seqSize, 0));
    const_cast<OpParam &>(param).resCtx = ctx;
    const_cast<OpParam &>(param).ctxSize = seqSize;
    ...
    HCCL_INFO("[TIMEOUT-TRACE][EngineNotify] execTimeout=%u hostNotifyWaitTime=%u defaultSupported=%u",
        param.opConfig.execTimeout, hostNotifyWaitTime, static_cast<u32>(IsHcommDefaultTimeoutSupported()));
