...
std::unique_ptr<AlgResourceCtxSerializable> DeserializeResCtx(const OpParam *param)
{
    ...
    HCCL_INFO("[TIMEOUT-TRACE][DeSerialize] after deserialize: waitTimeout=%u fullTimeout=%u ctxSize=%zu",
        resCtx->waitTimeout, resCtx->fullTimeout, param->ctxSize);
    return resCtx;
}
} // anonymous namespace
...
extern "C" unsigned int HcclLaunchAicpuKernel(OpParam *param)
{
    ...
    if (!ops_hccl::IsOpsV2(param->algName, param->deviceType)) {
        ...
        if (HcommIsSupportHcommRegOpInfo()
            && HcommRegOpInfo(param->commName, reinterpret_cast<void *>(&opInfo), sizeof(ops_hccl::ScatterOpInfo))
                   != HCCL_SUCCESS) {
    ...
    if (param->opType != HcclCMDType::HCCL_CMD_BATCH_SEND_RECV) {
        ...
        if (cachedResCtxHolder != nullptr && ops_hccl::IsResCtxCacheReusable(*cachedResCtxHolder, *param)) {
            ...
            HCCL_INFO("[TIMEOUT-TRACE][CacheHit] resCtxPtr->waitTimeout=%u resCtxPtr->fullTimeout=%u",
                resCtxPtr->waitTimeout, resCtxPtr->fullTimeout);
        } else {
            ...
            HCCL_INFO("[TIMEOUT-TRACE][CacheMiss] resCtxPtr->waitTimeout=%u resCtxPtr->fullTimeout=%u",
                resCtxPtr->waitTimeout, resCtxPtr->fullTimeout);
            if (isStaleCache) {
    ...
    if (HcommIsSupportHcommSetNotifyWaitTimeOut()) {
        HCCL_INFO("[TIMEOUT-TRACE][Use] calling HcclSetNotifyWaitTimeOut(resCtxPtr->waitTimeout=%u) fullTimeout=%u",
            resCtxPtr->waitTimeout, resCtxPtr->fullTimeout);
