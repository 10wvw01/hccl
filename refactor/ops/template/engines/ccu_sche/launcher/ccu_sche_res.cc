...
{
    // 迁移自 op_common.cc:1564-1599 HcclGetChannelForCcu，与 CcuMsGetChannelForCcu 逻辑一致
    for (CcuKernelInfo &kernelInfo : res.ccuKernelInfos) {
        ...
        if (channelNum > 0) {
            // CCU_SCHE 与 CCU_MS 共用底层 CommEngine::COMM_ENGINE_CCU
            auto ret = HcclChannelAcquire(comm, CommEngine::COMM_ENGINE_CCU, kernelChannelRequest.data(),
                                          channelNum, kernelChannels.data());
            if (ret == HCCL_E_UNAVAIL) {
                HCCL_WARNING("[CcuScheGetChannelForCcu] channel unavailable, channel num[%u].", channelNum);
        ...
        if (kernelArgBase == nullptr) {
            HCCL_ERROR("[CcuScheGetChannelForCcu] kernelArg ptr is err.");
        ...
        HCCL_INFO("[CcuScheGetChannelForCcu] get [%u] channels", channelNum);
    }
    return HCCL_SUCCESS;
}
...
{
    // 迁移自 op_common.cc:1601-1664 HcclGetCcuKernel，与 CcuMsGetCcuKernel 逻辑一致
    CcuInsHandle insHandle{0};
    uint32_t insNum = 0;
    CHK_RET(HcclCommQueryCcuIns(comm, &insHandle, &insNum));
    CHK_PRT_RET(insNum != 1,
                HCCL_ERROR("[CcuScheGetCcuKernel] HcclCommQueryCcuIns fail! insNum is [%u]", insNum),
                HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(totalKernelNum != res.ccuKernelInfos.size(),
                HCCL_ERROR("[CcuScheGetCcuKernel] ccuKernel num not match! totalKernelNum[%u] ccuKernelInfos[%zu]",
    ...
    while (currentResGroup <= maxResGroup) {
        ...
        if (regStartRet != CCU_SUCCESS) {
            HCCL_ERROR("[CcuScheGetCcuKernel] ccu kernel register start failed: ccuRet -> %d", regStartRet);
        ...
        for (u32 i = 0; i < totalKernelNum; i++) {
            ...
            HCCL_DEBUG("[CcuScheGetCcuKernel] kernelFuncName[%s]", kernelInfo.kernelFuncName);
            ...
            if (regRet != CCU_SUCCESS) {
                HCCL_ERROR("[CcuScheGetCcuKernel] ccu kernel register failed: ccuRet -> %d", regRet);
        ...
        if (regEndRet == CCU_E_UNAVAIL) {
            HCCL_WARNING("[CcuScheGetCcuKernel] ccu kernel register end unavailable, try to fallback.");
            return HCCL_E_UNAVAIL;
        } else if (regEndRet != CCU_SUCCESS) {
            HCCL_ERROR("[CcuScheGetCcuKernel] ccu kernel register end failed: ccuRet -> %d", regEndRet);
