...
HcclResult CcuMsLaunchKernel(const OpParam &param, OpsExecutor &executor, AlgResourceCtxSerializable &resCtx)
{
    ...
    if (HcommBatchModeStart(param.algTag) != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuMsLaunchKernel] failed set batch mode, tag is %s.", param.algTag);
        return HCCL_E_INTERNAL;
    }
    ...
    if (ConvertToHcclDfxOpInfo(const_cast<OpParam *>(&param), &dfxOpInfo) != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuMsLaunchKernel] ConvertToHcclDfxOpInfo fail, commName is %s, tag is %s",
    ...
    if (HcclDfxRegOpInfoByCommId(const_cast<char *>(param.commName), reinterpret_cast<void *>(&dfxOpInfo)) != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuMsLaunchKernel] HcclDfxRegOpInfoByCommId fail, commName is %s, tag is %s",
                   param.commName, param.algTag);
        return HCCL_E_INTERNAL;
    }
    ...
    if (HcommProfilingReportKernelStartTask(thread, param.commName) != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuMsLaunchKernel] failed to report MainStream And FirstTask, thread %lu, commName %s.",
    ...
    if (executor.Orchestrate(resCtx) != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuMsLaunchKernel] orchestrate failed for alg:%s", param.algName);
        return HCCL_E_INTERNAL;
    }

    // 6. 上报 device op profiling
    if (HcommProfilingReportDeviceOp(param.commName) != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuMsLaunchKernel] HcommProfilingReportDeviceOp fail, commName[%s]", param.commName);
        return HCCL_E_INTERNAL;
    }
    ...
    if (HcommProfilingReportKernelEndTask(thread, param.commName) != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuMsLaunchKernel] failed to report MainStream And LastTask, thread %lu, commName %s.",
                   thread, param.commName);
        return HCCL_E_INTERNAL;
    }

    if (HcommBatchModeEnd(param.algTag) != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuMsLaunchKernel] failed set eager mode, tag is %s.", param.algTag);
        return HCCL_E_INTERNAL;
    }

    if (HcommReleaseComm(param.commName) != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuMsLaunchKernel] HcommReleaseComm fail, commName[%s]", param.commName);
        return HCCL_E_INTERNAL;
    }
    HCCL_INFO("[CcuMsLaunchKernel] success, tag[%s], algTag[%s], commName[%s]",
