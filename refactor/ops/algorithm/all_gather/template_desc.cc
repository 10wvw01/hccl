...
/**
 * 全局 AllGather TemplateDesc 表。
 * 将 gen_algorithm.cc（重构后为 algorithm_all_gather_aicpu.cc）中重复构造的 TemplateDesc 去重，
 * 统一在此处构造，通过 HcclAllGatherTemplateDescType 枚举下标访问。
 * 数组定义顺序必须与 HcclAllGatherTemplateDescType 枚举顺序保持一致。
 *
 * 去重依据：所有 TemplateDesc 的 hcclCmdType 恒为 HCCL_CMD_ALLGATHER，
 * shotMode 恒为 ONE_SHOT，仅 algType 与 jettyMode 存在差异，共 4 种组合：
 *   - NHR + SINGLE_JETTY        （InsAllGatherNHR / InsAllGatherParallelMesh1DNHRUboe 等）
 *   - FULLMESH + SINGLE_JETTY   （InsAllGatherMesh1D / InsAllGatherMesh1D1DZAxisDetour 共用）
