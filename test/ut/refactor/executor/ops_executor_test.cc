...
TEST_F(OmniPipeTest, CalcRes)
{
    ...
    EXPECT_EQ(req.slaveThreadNum, 9u);

    // notifyNumPerThread (9 entries):
    //   level0 (Mesh, 8 ranks): notifyOnMain+1=7, maxSlave=6 × maxNotifyPerThread=1 → [7,1,1,1,1,1,1]
    //   level1 (NHR, 8 ranks): notifyOnMain+1=1, maxSlave=0 × maxNotifyPerThread=1 → [1]
    //   level2 (NHR, 2 ranks): notifyOnMain+1=1, maxSlave=0 × maxNotifyPerThread=1 → [1]
    ASSERT_EQ(req.notifyNumPerThread.size(), 9u);
    ...
    EXPECT_EQ(req.notifyNumPerThread[7], 1u);  // level1 main
    EXPECT_EQ(req.notifyNumPerThread[8], 1u);  // level2 main
