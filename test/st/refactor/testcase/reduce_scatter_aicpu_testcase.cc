...
TEST_F(ST_REDUCE_SCATTER_AICPU_TEST, st_reduce_scatter_a5_aicpu_mesh_1d_4rank_fp32_16G_data_test)
{
    // 仿真模型初始化
...
TEST_F(ST_REDUCE_SCATTER_AICPU_TEST, st_reduce_scatter_a5_aicpu_NHR_2rank_int64_small_data_test)
{
    // 仿真模型初始化
    TopoMeta topoMeta{{{0}, {1}}}; // 三维数组指定超节点-Server-Device信息
...
TEST_F(ST_REDUCE_SCATTER_AICPU_TEST, st_reduce_scatter_a5_aicpu_NHR_2rank_int8_500M_data_test)
{
    // 仿真模型初始化
    TopoMeta topoMeta{{{0}, {1}}}; // 三维数组指定超节点-Server-Device信息
...
TEST_F(ST_REDUCE_SCATTER_AICPU_TEST, st_reduce_scatter_a5_aicpu_NHR_8rank_fp64_small_data_test)
{
    // 仿真模型初始化
    TopoMeta topoMeta{{{0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}}}; // 三维数组指定超节点-Server-Device信息
...
TEST_F(ST_REDUCE_SCATTER_AICPU_TEST, st_reduce_scatter_a5_aicpu_NHR_4rank_fp32_16G_data_test)
{
    // 仿真模型初始化
    TopoMeta topoMeta{{{0}, {1}, {2}, {3}}}; // 三维数组指定超节点-Server-Device信息
    // 算子执行参数设置
...
TEST_F(ST_REDUCE_SCATTER_AICPU_TEST, st_reduce_scatter_a5_aicpu_seq_2x2rank_int64_8G_data_test)
{
    // 仿真模型初始化
    TopoMeta topoMeta{{{0, 1}, {0, 1}}}; // 三维数组指定超节点-Server-Device信息
    // 算子执行参数设置
    auto recvCount = 256 * 1024 * 1024;                // 单卡数据量
    auto dataType = HcclDataType::HCCL_DATA_TYPE_INT64; // 数据类型
    auto reduceOp = HcclReduceOp::HCCL_REDUCE_SUM;      // 归约类型
    RunReduceScatterAicpuA5(topoMeta, recvCount, dataType, reduceOp);
}
...
TEST_F(ST_REDUCE_SCATTER_AICPU_TEST, st_reduce_scatter_a5_aicpu_parallel_2x4rank_int64_4G_data_test)
{
    // 仿真模型初始化
...
TEST_F(ST_REDUCE_SCATTER_AICPU_TEST, st_reduce_scatter_a5_aicpu_concurrent_mesh1dclos_fp16_512mb_data_test)
{
