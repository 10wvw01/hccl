    ...
    const HcclReduceOp &reduceOp, const uint32_t root)
{
    ...
    for (auto rankId = 0; rankId < rankSize; ++rankId) {
        threads.emplace_back([=]() {
            ...
            uint32_t topoLevelNum = 0;
            CHK_RET(HcclRankGraphGetLayers(comm, &netLayers, &topoLevelNum));
            HCCL_INFO("[ST_REDUCE_2LEVEL_TEST] rank[%u] topoLevelNum[%u]", rankId, topoLevelNum);
...
{
    TopoMeta topoMeta{{{0, 1, 2}, {0, 1, 2}, {0, 1, 2}, {0, 1, 2}, {0, 1, 2}}};
    auto recvCount = 262;
    auto dataType = HcclDataType::HCCL_DATA_TYPE_INT8;
    auto reduceOp = HcclReduceOp::HCCL_REDUCE_SUM;
    uint32_t root = 8;
